...
Engine-side weight computation fires independently via `(*Player).updateInvs` inv-listener loop at `modules/world/player.go:832-837`: `runWeightChanged` flag → `p.calculateRunWeight()` → `sendUpdateRunWeight`. This is the NAI-136 wiring (commit 1b5ae51).
...
| OpUpdateRunWeight (game prot) | modules/world via sendUpdateRunWeight | WIRED | NAI-136 commit 1b5ae51: "Adds OpUpdateRunWeight (opcode 22, payload 2) to game server prot." |
