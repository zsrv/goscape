...
NAI-138 commit 4918d37 fixed varp configs loading both server+client streams. Varp 4 (`tutorial`) and varp 173 (`option_run`) are loaded from the same `VarPlayerType` config path. The `writeVarp` probe test (player_varp_probe_test.go:27,110,133,179) covers this path directly with varp 173; the same `writeVarp` call is used for all varp IDs including tutorial varp 4.
