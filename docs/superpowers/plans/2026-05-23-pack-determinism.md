...
The resume doc lists helpers at `/tmp/claude/decode_script_n.go` (decodes script id N from a .dat) and `/tmp/claude/diff_set_a_vs_b.go` (diffs the set of differing scripts). If those scripts have been purged, recreate from the resume's commit `4b069bc8` reference.
