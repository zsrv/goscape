...
Create `/home/owner/.claude/projects/-home-owner-Code-github-com-zsrv-goscape/memory/post_friends_arc_cleanup_b3_close.md` with this content (replace the commit-hash placeholders with actuals from `git log --oneline 5216cc79..HEAD`):
...
Replace `N`, `<first>`, `<last>` with actuals from `git log --oneline 5216cc79..HEAD | wc -l` and the first/last short hashes.
