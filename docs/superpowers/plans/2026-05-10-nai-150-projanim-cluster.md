...
Expected: count of dispatch entries equals (count at HEAD `f224216`) + 3. The exact pre-count depends on HEAD, but the delta must be +3 vs `git show f224216:pkg/script/handlers.go | grep -cE '^\tOp[A-Z][a-zA-Z]+:\s+handle'`.
...
NAI-150 spec at `docs/superpowers/specs/2026-05-10-nai-150-projanim-cluster-design.md` (commit `983b799`); plan at `docs/superpowers/plans/2026-05-10-nai-150-projanim-cluster.md` (commit `<plan-commit-sha>`). No deferred items.
