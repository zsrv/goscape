...
Run: `git log --oneline 00e7e242..HEAD`

Expected: 9 commits — spec (df7d07a7) + T1..T8 in order. Each commit message references its task number except the spec.
...
Write `friends_server_slice4b_close.md` with this content (substitute `<HEAD>` with the actual HEAD SHA from `git rev-parse --short HEAD` and `<COMMIT-RANGE>` with `df7d07a7..<HEAD>` after T9 passes):
