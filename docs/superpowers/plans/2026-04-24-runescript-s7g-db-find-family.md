...
**Tech Stack:** Go 1.26+. `GOPATH=$TMPDIR/go GOCACHE=$TMPDIR/go-cache go test ./...` for verification. `git commit --no-gpg-sign` for commits. Branch: `main` (project convention — no worktree needed per established S-series workflow). Spec source: `docs/superpowers/specs/2026-04-24-runescript-s7g-db-find-family-design.md` (commit `5b91e82`).
