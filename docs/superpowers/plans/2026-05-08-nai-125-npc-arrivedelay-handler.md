...
Locate the alphabetic-by-name NPC mutating-ops block in `pkg/script/handlers.go:402-414` (verified at HEAD `d70d822`):
...
Expected: all packages PASS. Pre-existing modernization warnings (S1001 / minmax / rangeint per NAI-124 close memo at `nai_followups.md:6313`) are NOT introduced by this bundle — confirm any failures pre-date `d70d822` via `git stash && go test ./... && git stash pop` if uncertain.
