...
**Spec:** `docs/superpowers/specs/2026-05-12-nai-186-supermod-cheats-design.md` (commit a71fe94).
...
These were re-greped at plan-write against HEAD (commit a71fe94). The plan-author/controller may re-confirm before each dispatch, but the answers are baked in below.
...
**Context:** The NAI-185 close commit (92aec51) left a 15-line comment block at L367-381 listing 10 unported cheats in 3 clusters. NAI-186 completes the super-mod cluster — drop those 4 cheats from the listing; supersede the tag.
...
(Empty close commit is OPTIONAL — per recent close-commit precedent at 92aec51 / 25ee29d, a close-commit is conventional but not required if Task 6's commit already captures the cluster-retirement narrative. Use `--allow-empty` only if there's no content to commit; otherwise let Task 6's commit BE the close commit and skip this step.)
