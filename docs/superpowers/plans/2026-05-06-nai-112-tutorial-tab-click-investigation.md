...
**Purpose:** Re-verify spec §9 premises against current HEAD before dispatching audit work. The spec was committed at `cbbc977`; pre-flight observations were captured at `e3eecbe`. HEAD has moved one commit. Stale citations cause wasted audit cycles (`controller_preflight`, `spec_followup_tracker_freshness`).
...
Expected: `cbbc977 docs(spec): NAI-112 …` at HEAD; `e3eecbe chore(close): NAI-110 …` at HEAD~1; `c4706db feat(script): NAI-110 T2 …` at HEAD~2. If unexpected commits appear, halt and reconcile.
...
- After the NAI-110 close (commit e3eecbe), user-launched smoke (2026-05-06) reported: clicking the inventory tab on Tutorial Island (post `tut_flash(^tab_inventory)`) advances neither the chatbox nor displays the inventory side panel. No warn log was reported.
...
**Purpose:** Verify every "Verified at HEAD" claim from the audit report against current HEAD (`cbbc977` or later). Per `audit_subagent_fabrication`, any unverifiable claim disqualifies that hypothesis from binding.
...
Expected: `<new-sha> docs(investigation): NAI-112 Stage 1 …` at HEAD; `cbbc977 docs(spec): NAI-112 …` at HEAD~1.
...
```bash
git diff cbbc977 -- modules/world/handler_interface.go pkg/script/provider.go
```
...
Instrumentation reverted to pre-Bundle-1b state (byte-diff vs cbbc977 = empty).
...
4. **No invented symbols:** all referenced functions / files exist at HEAD `cbbc977` per Bundle 0 pre-flight.
