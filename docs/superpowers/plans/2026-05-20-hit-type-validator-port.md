...
Expected: HEAD shows `431ad5d6 spec(script): HitType validator port + NpcStat read-path validator coverage` (or a more recent commit on top of it). `git status` shows only `config.yaml` modified plus standing untracked noise (`.bash_profile`, `.claude/`, `.vscode`, etc.). **Do not stage or modify any of that noise.**
