...
**HEAD at plan-write:** `d789276` (after spec polish commit `d789276 docs(spec): NAI-26 spec — self-review polish`). Spec's predecessor pin remains `8eec4c1` (NAI-25 close).
...
**Pre-flight context:**
- HEAD `d789276` at task dispatch. Verify all line numbers via re-grep at task time per `controller_preflight` memory. The two spec commits (`be17d0c`, `d789276`) are docs-only — production lines have not drifted from the spec's `8eec4c1` claims.
...
**Spec-followup-tracker-freshness** (per `spec_followup_tracker_freshness` memory): tracker entry assertions (STRONGQUEUE site at handlers.go:629, P_DELAY site at handlers.go:589, enqueueTyped helper at handlers.go:599-619) verified at HEAD `d789276`. All assertions held at HEAD. Re-verified at task dispatch via Step 1 grep commands.
