...
// HEAD=7a1ef6a). No client packet is sent. The TestPlaySongNoWriteOut
...
For each non-`newPlayer` `Player{...}` literal, confirm the literal intent does not need `lowMemory`. Known sites from HEAD=7a1ef6a:
...
No spec edit is needed unless the list drifted — the spec is already committed as `e329a6a`.
