...
- Bundle 0 findings: `docs/superpowers/investigations/2026-05-07-nai-120-bundle0-findings.md` (commit `cd5fcc2`).
- Bundle 1 audit: `docs/superpowers/investigations/2026-05-07-nai-120-bundle1-audit.md` (commit `e5c55b4`).
...
**Before EVERY handler dispatch:** Re-grep at HEAD per `controller_preflight`. Do not trust this plan's line numbers blindly — they are correct at HEAD `e5c55b4` but may shift if intervening commits land. If a sibling pattern moved, follow the actual code at HEAD, not the plan's quoted line.
...
**On Bundle pre-flight grep:** Each bundle starts with a "Pre-flight" task that re-greps the audit's claimed-absent symbols at HEAD. If any are present (added since `e5c55b4`), merge with existing rather than duplicate.
