...
Pre-flight at HEAD `0259d56` confirmed the function shape: `newPlayer` at `player.go:343` uses `p := &Player{...}` (struct literal at lines 344-430), then a `for i := 0; i < 21; i++` stats-init loop, then `p.afkZones[0] = packAfkCoord(...)`, then `return p` at line 440.
