...
4. **Signal H7 — StepValidator divergence.** With H6 passed, walk `route.Waypoints` consecutively. For each `(prev, curr)` pair (prev = prior waypoint accessor `prev.X()/prev.Z()`, or `(srcX, srcZ)` for index 0), compute `offsetX := curr.X() - prev.X(); offsetZ := curr.Z() - prev.Z()`. Call `gm.CanTravel(0, prev.X(), prev.Z(), offsetX, offsetZ)` — actual signature per `pkg/gamemap/gamemap.go:97-100`. Any false → fail with `"H7 FIRES at step %d/%d: BFS produced (%d,%d)→(%d,%d) offset=(%d,%d) but CanTravel=false. Route=%+v"`. **Plan-author must verify at HEAD:** (a) whether `route.Waypoints[0]` is the source tile or the first step (affects prev=src bootstrapping); (b) whether each `(offsetX, offsetZ)` lands in {-1, 0, 1} (BFS expansion is single-tile-step in `routeFindSize1`, so it should — but skip-pin and escalate if not).
...
- **`route.Waypoints` indexing semantics.** Phase 1 §3.2 Step 4 assumes `Waypoints[0]` is the first step (with `(srcX, srcZ)` bootstrapping prev). If the actual semantics put the source as `Waypoints[0]`, the H7 walk needs a different bootstrapping. Plan-author verifies at HEAD via Read of `pkg/pathfinder/routefinder/routefinder.go` waypoint-construction loop (~`:130-153`); if uncertain, add a one-tile probe before the H7 walk.
