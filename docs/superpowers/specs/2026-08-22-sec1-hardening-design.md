...
| SEC1-D3 | Outbound socket writes happen on a per-client writer goroutine behind a bounded queue (`maxOutboundQueueSlots`=512 frames, `maxOutboundQueueBytes`=1 MiB). Queue overflow or a write exceeding `tcp_server_write_timeout` closes the connection; with `tcp_server_write_timeout` <= 0 there is no budget to bound a drain, so close is an immediate hard close. TS `socket.write` is async with an unbounded Node buffer and no disconnect. | Node's async write never stalls the event loop; Go's blocking `net.Conn.Write` on the tick goroutine did. The cap is the goscape equivalent of "never block the tick" plus protection against unbounded memory. Bytes is the real limiter: the slot count is generous because the OnDemand pump lost its only backpressure when writes went async and emits one ~508-byte chunk per send, so a tight slot cap would disconnect a legitimately-downloading client after a few tens of KiB. | Permanent. Cap values are constants; revisit if legitimate clients hit them (log line `outbound queue overflow`). |
