...
producer-side yield is expressed in `onDemand.cycle`/`sendBatch`: a client
whose `backlogged()` reports the soft high-water mark has its pending requests
put back at the head of their queue (untouched, in order) and retried on the
next 50 ms cycle, instead of `serveClient` ending a slice and `run` re-arming
after `backloggedRetryInterval` (which this branch does not need — the ticker
...
everything else in the batch are identical. The adaptation was inherited
through the rev-254 → rev-245.2 back-port chain: this branch's pre-port
`ondemand.go`, `ondemand_test.go`, `server_accept.go` and `req.go` were
byte-identical to both siblings', and `tick.go`, `client.go`'s writer wiring,
`config.go`, `player.go`, `server_login.go`, `server_test.go`, `_helpers.tpl`
and `cmd/goscape/app/modules.go` merged from rev-245.2's ported versions
without conflict.
