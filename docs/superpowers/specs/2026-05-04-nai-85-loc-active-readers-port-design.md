...
**Predecessor**: NAI-84 (HEAD `9afcbba` — REBUILD_GETMAPS byte-source fix +
rebuildZones port; door-click cache freeze silenced).
**Smoke binding (post-NAI-83)**: door-click smoke at HEAD `9afcbba` confirmed
...
Tutorial Island door-click smoke at HEAD `9afcbba` reveals the
...
### 5.1 Pre-flight verification (against HEAD `9afcbba`)
