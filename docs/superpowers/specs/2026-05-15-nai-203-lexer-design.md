...
NAI-202 (closed at b9a4661) shipped `pkg/pack/compiler.BuildSymbols(srcDir, dataPackDir)` — the 32-key symbol table TS `Compiler.ts:330-365` hands to `CompileServerScript`. After NAI-202, the **only** thing blocking end-to-end `.rs2` → `script.dat/idx` compilation is the external `@lostcityrs/runescript` compiler itself.
