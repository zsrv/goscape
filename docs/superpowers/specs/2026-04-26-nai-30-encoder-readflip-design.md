...
- **Sub-spec**: NAI-30
- **Date**: 2026-04-26
- **Scope label**: B (Second sub-spec of the four-sub-spec series aligning goscape's `pkg/rsbuf` to the upstream `@2004scape/rsbuf` Rust crate's stateful API; replaces the existing interface-based `Encode`/`EncodeNpc` package functions with `*Buf`-attached `PlayerInfo`/`NpcInfo` structs that read state directly from the NAI-29-landed `*Buf` slot tables; reconciles the two NAI-29 parallel-write divergences flagged inline in `tick.go`; ports the NPC-side spatial-discovery primitives from upstream; retires `pkg/grid/`, `pkg/buildarea/`, `pkg/rsbuf/npc_observers.go`, plus `Player.AppearanceHash` method + the `AppearanceHash()` member of the `PlayerSource` interface (the rest of `PlayerSource`/`NpcSource` remains because the renderer + mask_payload layer still consumes them; full retirement waits for NAI-31 renderer port). Touches `pkg/rsbuf/` (rewrites `playerinfo.go` + `npcinfo.go`; extends `buildarea.go`, `player.go`, `npc.go`, `buf.go`; trims 1 line from `source.go`) + `modules/world/` (extends `Player` + `Npc` with orientation + `lastAppearance` fields; flattens `pkg/buildarea` scenery fields onto `Player`; swaps encoder callers in `player_info.go` + `player_npc_info.go`; deletes the placeholder comments at `tick.go:373,376,412`; deletes `Player.AppearanceHash`); ~700 LOC production + ~700 LOC tests across 4 bundles; introduces 1 new deviation tag (NAI-30-D1: orientation field plumbed without producer); net deviation count 13 → 14)
- **Predecessors**: NAI-29 (rsbuf stateful core + entity model + parallel-write caller hooks) — last on `main` as `484cd98`
- **Source root**: `2004scape/rsbuf` branch `225` at `$HOME/Code/github.com/2004scape/rsbuf/` (HEAD `1cbb2ce`)
...
- Deleted files:
  ...
  - `(p *Player) AppearanceHash()` method in `modules/world/player_source.go` + `appearanceHash` helper in `modules/world/appearance.go` (if package-level helper becomes dead after method retires); B4 T4.6
- **NOT deleted in NAI-30** (deferred to NAI-31, the renderer port):
  - `pkg/rsbuf/source.go` — `PlayerSource` interface remains consumed by `pkg/rsbuf/renderer.go` (`ComputePlayers`, `buildPayload`) and `pkg/rsbuf/mask_payload.go` (every `writeAnim`/`writeSay`/`writeExactMove`/`writeFaceEntity`/`writeFaceCoord`/`writeSpotAnim`/`writeAppearance`/`writeDamage` helper). NAI-31 ports the renderer to consume `*Buf.players[pid]` directly; source.go retires then. NAI-30 only removes the now-dead `AppearanceHash()` member from the interface (1-line interface trim) since the new encoder no longer reads via PlayerSource.AppearanceHash.
  - `pkg/rsbuf/npc_source.go` — `NpcSource` interface remains consumed by `pkg/rsbuf/renderer.go` (`ComputeNpcs`) and `pkg/rsbuf/npc_mask_payload.go`. Same NAI-31 deferral.
  - `modules/world/player_source.go` — the `Player` methods that satisfy `PlayerSource` (Visibility, Active, Slot, Coords, Masks, EntityMask, AppearanceBytes, AnimID, AnimDelay, FaceEntity, SayText, etc.) remain because the interface they satisfy still exists. After NAI-31 retires `PlayerSource`, this file's methods either become dead (delete then) or get repurposed as direct accessors. NAI-30 only deletes the `AppearanceHash()` method (the one accessor that's been replaced by `Player.lastAppearance int`).
  - `modules/world/npc_source.go` — same pattern; satisfies `NpcSource` which is still alive. No deletions in NAI-30.
...
- **T4.6** — Verify-and-delete dead packages/files. Plan-author lists each grep command with expected zero-match output:
  ...
  - `rg "AppearanceHash\b\(" pkg/ modules/ cmd/` → 0 → delete `(p *Player) AppearanceHash()` method in `modules/world/player_source.go` AND remove the `AppearanceHash()` member from the `PlayerSource` interface in `pkg/rsbuf/source.go` (1-line interface trim only — the rest of the interface stays for renderer use). Delete the `appearanceHash` helper in `modules/world/appearance.go` if present and dead. Note: `PlayerSource` and `NpcSource` interfaces remain (consumed by `pkg/rsbuf/renderer.go` + `pkg/rsbuf/mask_payload.go` + `pkg/rsbuf/npc_mask_payload.go`); they retire wholesale at NAI-31 alongside the renderer port.
  ...
  - `modules/world/player_source.go` and `modules/world/npc_source.go` remain as-is (their methods satisfy `PlayerSource` and `NpcSource`, which are still alive — see "NOT deleted in NAI-30" note in tech-stack section). Only `(p *Player) AppearanceHash()` is removed (the one method whose interface contract was retired by the lastAppearance tick port).
