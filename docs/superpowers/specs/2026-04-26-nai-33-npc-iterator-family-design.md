...
- **Predecessors**: NAI-32 (renderer dual-cache CHAT port + rs-server-225 citation sweep + 4-stage Bundle 3 layered-bug investigation) — last on `main` as `5169689`
- **Source root**:
...
2. Confirm `*Npc` satisfies both `pkg/zone.NpcLike` AND `pkg/script.ActiveNpc` at HEAD `5169689` — grep for `var _ pkg/zone.NpcLike = (*Npc)(nil)` and `var _ pkg/script.ActiveNpc = (*Npc)(nil)` (or equivalent).
