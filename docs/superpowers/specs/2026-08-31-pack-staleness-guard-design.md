...
**Status:** implemented
**Date:** 2026-08-31
**Branch:** originated on `rev-274`; back-ported to `rev-245.2`
**Deviation ID prefix:** `PSG-D{n}`

---

> **Back-port note.** This document is `rev-274`'s, kept verbatim below. The
> failure in §1 is the one that prompted the guard and is specific to that
> branch's engine sync, but nothing about the hazard is: incremental freshness
> is keyed on source mtimes on every branch, so any change to a packed byte
> reaches the same trap. Two things differ at this pin: `FormatVersion`'s
> history line records this branch's Engine-TS and Content pins, and
> `TestPackFormatVersionGolden` covers the fields this revision actually has
> (no `check_invcat`, no seq `reachforward`/`duplicatebehaviour`) with its own
> golden hash.
