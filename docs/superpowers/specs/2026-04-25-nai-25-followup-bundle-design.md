...
- **Predecessors**: NAI-24 (follow-up bundle) — last on `main` as `0f8bb4e`
...
Math: PlayerOps.ts has 56 NumberNotNull; NAI-24 Bundle 1 audited **47 popInt sites** in `handlers_player.go` (per commit `edd9c88` audit table); the **9-site delta** lives in PlayerOps.ts opcodes that goscape dispatches from a different file (or whose TS counterpart is not a popInt-equivalent in goscape).
...
2. Cross-reference NAI-24 Bundle 1's audit table in commit `edd9c88` for the 47 wrapped/skipped sites.
...
Embedded in each Bundle 2 commit message (per-file commits and rollup commit). Per-file format mirrors NAI-23 Bundle 4c (commit `ab9c681`):
...
- **`spec_followup_tracker_freshness` discipline.** Per memory: tracker entries silently rot. Spec-write-time re-greps verified the From-NAI-24 entry assertions (zero production callers passing -1; dispatch site at `:471-479`; doc-comment at `:632-633`; interface contract at `:277-283`); the From-NAI-23 entry assertions (per-file enumeration; priority order). All assertions held at HEAD `0f8bb4e`.
