...
**Predecessor:** NAI-181 (LAST_LOGIN_INFO) closed at `3d9279c`.
...
## 2. Affected sites (pre-flight verified at HEAD `3d9279c`)
...
**Import audit at HEAD `3d9279c`:** `strconv` and `strings` already imported (lines 5-6). `math` is NOT imported — add it.
...
3. **dskit Manager graceful-exit handshake.** Verified at HEAD `3d9279c`: `world.go:99-107` runFn returns `fmt.Errorf("server stopped unexpectedly")` on nil-error from `Run()`. Spec adds `s.shutdownGraceful bool` and modifies runFn to return nil when that flag is set. **Implementer task:** also ensure `Server.Run()` returns nil (not `fmt.Errorf(...)`) when the tick loop exits via the `shutdownGraceful` path. Re-grep `Run() error` body before changing.
