...
Stage 3 instrumentation (commits `90a855a` Provider.Names accessor, `10efece` boot-time `[opheldu,*]` registry log, `51e2418` 6 inline DEBUG lines in `handleOpHeldU` L370-L418) shipped and was smoked.
...
- **`90a855a` Provider.Names() accessor.** Stays — long-term API.
...
3. `revert: chore(debug): NAI-114 Stage 3 — opheldu trigger-lookup hit-trace` ← `git revert 51e2418`.
4. `revert: chore(debug): NAI-114 Stage 3 — boot-time opheldu script-registry log` ← `git revert 10efece`.
