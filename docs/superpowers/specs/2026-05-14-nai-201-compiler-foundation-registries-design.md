...
**Date**: 2026-05-14
**Predecessor**: NAI-200 (`CompilerTypeInfo` foundation; closed at `ab74814`). NAI-200 §12 enumerated four "NAI-201 prerequisites" — this sub-spec lands those four as data registries with zero production consumers, deferring the `runServerCompiler` driver port that consumes them to NAI-202.
...
The original NAI-201 framing in `NAI-200 §12` and the dispatch prompt was "port `runServerCompiler` driver (TS `tools/pack/Compiler.ts:109-367`)". Pre-flight against HEAD `ab74814`:
