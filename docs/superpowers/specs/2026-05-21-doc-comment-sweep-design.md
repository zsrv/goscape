...
- Commit `6c1d908d Run gofmt` (between [[combat-sub-spec-framing-doc-cleanup-close]] and [[hero-points-lifecycle-clear-close]]) had already normalized the alignment drift.
...
- **Explicit false-positive retirement in close memo:** the close memo MUST state that anchors #2/#3/#4 from predecessor memo (gofmt drift in `active.go` / `handlers_npc_test.go` / `handlers_player_test.go`) were false-positives normalized by commit `6c1d908d Run gofmt`, and that the visible "two-tier" alignment in `mockActiveNpc` is gofmt's natural blank-line-separated alignment behavior NOT drift. Future close memos must NOT regenerate this claim.
