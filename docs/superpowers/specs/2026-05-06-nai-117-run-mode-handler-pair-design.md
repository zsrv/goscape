...
**Routing:** Adjacent residuals from NAI-116 close-commit smoke (`87e197c`); routed per `smoke_surfaces_adjacent_divergences` because each handler-port exceeds the 30-LOC in-scope-stretch threshold once tests are included.
...
Per `controller_preflight` memory; verified against HEAD `87e197c`.
...
enumeration. (Verified at HEAD `87e197c`: no second parallel
...
- `controller_preflight` — file-paths and line-numbers in §3 verified
  against HEAD `87e197c` before plan dispatch.
