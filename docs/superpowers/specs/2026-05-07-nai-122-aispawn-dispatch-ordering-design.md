...
**Predecessor:** NAI-121 (closed `9be10a2`); Bundle 2 V-PARTIAL audit `e60ca17`.
...
- `TestAddNpc_FreshSpawn_PlayerUidVarnReadsMinusOne` (existing, NAI-121-T3 commit `e6644b1`) must still pass. The order `resetEntityForRespawn` → AI_SPAWN dispatch is preserved (lines 79 → 88 of `npc_registry.go`); sync replacement keeps that order. No code change in this task; verification only. Bundle 1 reviewer flags green.
