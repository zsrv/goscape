...
The NAI-186 carryforward described the admin cluster as "blocked on dynamic Loc/Npc spawn + interface routing." Per memory `tracker_entry_framing_can_be_incomplete` and `risk_register_premise_grep`, I re-derived from primary sources at HEAD `71b13ac`. The framing was stale: every dynamic-spawn / interface-routing primitive already exists.
