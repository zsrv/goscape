...
- **Predecessors**: NAI-25 (follow-up bundle) — last on `main` as `8eec4c1`
...
1. **`modules/world/player_script.go`**:
   - `playerQueueRequest` struct (`:25-30` per HEAD `8eec4c1`): `IntArg int` (line `:28`) → `IntArgs []int` + `StringArgs []string`. Verify struct location at controller pre-flight via `grep -n "type playerQueueRequest" modules/world/`.
...
6. **`modules/world/script_test.go`** (6 call sites):
   ...
   - **Plan-time verification correction**: each call site enqueues a script that is **registered** in the surrounding test fixture (verified at plan-write commit `0e0b9d9`). The migration is purely a signature update — the test scripts hit the happy path, not the missing-script path. Bundle 2's per-site evaluation is a no-op confirmation pass; no functional migration is needed.
...
- **Bundle 2 6-site `script_test.go` migration semantic mismatch.** **Resolved at plan-write (commit `0e0b9d9`)**: all 6 sites enqueue registered scripts; no test pins the silent-no-op behavior. The Bundle 2 error-return activation has no observable effect on these tests. No deviation needed.
...
- **`spec_followup_tracker_freshness` discipline.** Per memory: tracker entries silently rot. Spec-write-time re-greps verified the From-NAI-25 entry assertions: `STRONGQUEUE` site at `handlers.go:629` (verified at HEAD `8eec4c1`); `P_DELAY` site at `handlers.go:589` (verified); `enqueueTyped` shared helper at `handlers.go:599-619` (verified). All assertions held at HEAD.
