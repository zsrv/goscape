...
**Predecessors:** NAI-190 (`World.reload()` port + `::reload` cheat). NAI-190 close commit `a564013` notes `::rebuild` as the lone remaining `ClientCheatHandler.ts` carryforward, blocked on the pack-pipeline arc opened here.
**HEAD at spec-write:** `a564013`
...
Per `controller_preflight` and `risk_register_premise_grep`, every premise below was re-verified against HEAD `a564013`.
...
- Predecessor close: NAI-190 (`a564013`) — `World.reload()` port + `::reload` cheat.
