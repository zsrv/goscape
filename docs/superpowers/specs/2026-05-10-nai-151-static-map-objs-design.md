...
- **NAI-151-D3: `Obj.IsActive` field added** — `pkg/entity/obj.go` previously lacked an `IsActive` flag; T1 added it to mirror the pre-existing `Loc.IsActive` field at `pkg/entity/loc.go:16`. **Why:** TS `Zone.addStaticObj` sets `obj.isActive = true` (Zone.ts:214); without the field, the static-loc parity assertion in T4 (`TestAddStaticObjSetsIsActive`) cannot be expressed. **Risk:** none — purely additive, no callers depend on the prior absence. The field has no production reader yet (parallels `Loc.IsActive`'s reader pattern at `writeFullFollows`); future obj-replay tightening may consume it.
- **NAI-151-D4: `populateStaticObjsIntoZones` log-pin test omitted** — spec §6.3 item 3 prescribed `TestPopulateStaticObjsIntoZones_LogsCount` to pin the boot-log message; T5 shipped `TestPopulateStaticObjsIntoZones_EmptySpawnsNoOp` instead. **Why:** the existing locs counterpart `TestPopulateStaticLocsEmptyIsNoop` (modules/world/server_zone_static_test.go:28) follows the same no-panic-only pattern; adding a log-capture test for objs but not locs would create asymmetric pinning precedent. **Risk:** boot-log message string change goes unnoticed by tests. Mitigation: post-smoke logs are inspected manually per §6.5.
