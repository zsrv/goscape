...
- **R5 (low) — Trailing-clear breaks an existing player-info test that depends on stale `faceEntity`.** Grep at spec-write: `player_masks_test.go:91` notes `// Persistent (animID, faceEntity, levels[3], baseLevels[3]) should stay.` — this test asserts persistence WITHOUT the `target == nil` precondition (i.e. with `p.target` not set). The trailing-clear DOES fire when `target == nil && faceEntity != -1`. Implementer must verify the existing test sets `p.target` to a non-nil value OR sets `faceEntity == -1` initially. If neither, the trailing-clear flips the test. Pre-flight at plan-write (controller pre-flight per `controller_preflight.md`). Disposition: if the existing test breaks under the new behavior, REWRITE the test to explicitly set `p.target = &Npc{}` for the persistence assertion; the new persistence claim is "faceEntity persists when target is present" (which IS the correct TS-faithful invariant).

- **R6 (low) — `p.target` field assignment in tests may not be straightforward.** `p.target` has type `Entity` (interface). Tests can assign concrete `*Npc{}` or `*Player{}` instances. NPC-side test at `npc_masks_test.go:255-265` uses `n.target = &mockEntity{...}` or similar pattern — implementer mirrors that pattern.
