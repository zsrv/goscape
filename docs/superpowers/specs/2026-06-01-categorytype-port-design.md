...
3. **Mock cascade.** Compile errors at every `Configs`-implementing test
   ...
   from `h-core-3`'s `VarsType` addition (`med-bundle-15` `8077b90f`)
...
- **Docs to update:**
  - `docs/PORTING-CLOSED.md` — promote the existing `✅ EXCEPTION-DOCUMENTED` row (line 74, `fix/med-bundle-18` `3e9141d3`) to `✅ FIXED` citing the new commit SHA. Body updated to reflect the actual port (loader + Provider + reload arm + tightened validator) rather than the pre-port deferral rationale.
