...
**True unported cheat count at HEAD `868a25a` = 17 + 6 + 5 = 28.** NAI-182 D3 listed 25, missing `setvar`, `getvar`, `getvarother` (wait — D3 does include getvar and getvarother; let me recount the D3 list).
...
### 2.5 Goscape state at HEAD `868a25a`
...
- [ ] Re-grep TS L52-617 at plan-write time to verify the §2.1 outer-block enumeration hasn't drifted (the TS pin at commit `868a25a`'s submodule head should be stable, but verify).
