...
Per memory `spec_followup_tracker_freshness`: every "TS does X" assertion above re-grep+Read at HEAD `ba1b03f` 2026-05-09. All confirmed at the cited line numbers.
...
- `spec_followup_tracker_freshness` — every cross-reference re-grep+Read at HEAD `ba1b03f` 2026-05-09; flagged stale claims (D3 `isMulti API needs porting`, D2 `[mapzone_exit,...]` underscore).
