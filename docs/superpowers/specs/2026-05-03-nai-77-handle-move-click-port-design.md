...
**Status:** Spec
**Branch:** main
**HEAD at spec-write:** `452cdf0` (NAI-76 close)
...
|---|------|--------|------------|
| R1 | `ClearPendingAction → CloseModal(true)` semantics: TS uses immediate close (not deferred via `requestModalClose`). Goscape's `CloseModal` (player_script.go:674) IS immediate per inspection at spec-write — modalState set to None synchronously and IF_CLOSE triggers fire inline. | **VERIFIED at spec-write** (HEAD `452cdf0`) | Re-verify at plan-author + implementer-dispatch. No code change. |
...
None at spec-time. All premises grep-verified at HEAD `452cdf0`.
...
- `controller_preflight.md` — all premises grep-verified at HEAD
  `452cdf0` BEFORE spec-write commit, not just at plan dispatch.
