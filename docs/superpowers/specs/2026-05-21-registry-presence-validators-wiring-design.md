...
All line numbers verified at brainstorm time against `HEAD=99f9effc`. Per the predecessor `[[doc-comment-sweep-close]]` non-obvious finding #1, re-verify before commit if any in-flight edit lands first.
...
**`TestCheckObjType`** in `handlers_obj_test.go` — sibling-location to `checkObjType` per file-pairing convention (predecessor T1 reviewer Minor finding `6d3de571` corrected validator unit tests to their def-file's paired test file).
