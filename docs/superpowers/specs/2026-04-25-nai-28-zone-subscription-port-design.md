...
- **Predecessors**: NAI-27 (player timer family + VARARG opcode port + NPC queue audit memo) — last on `main` as `be0aa13`
...
- **`spec_followup_tracker_freshness` discipline.** All assertions in this brainstorm (file paths, line numbers, method signatures, NAI-19-D1 inline comment sites, current `pkg/grid` consumer count) verified at HEAD `be0aa13` at brainstorm-write. Plan author re-verifies at plan-write; controller re-verifies before each dispatch.
