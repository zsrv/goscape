...
## 3. Goscape state at HEAD `ce0cef2`
...
// EarlyReturnsBeforePathing (NAI-155, commit 575d4cf).
if p.delayed && s.currentTick < p.delayedUntil {
    return
}
```
...
**Pre-T1 baseline:** `GOPATH=$TMPDIR/go GOCACHE=$TMPDIR/go-cache go test ./modules/world/... -run 'CanAccessGate'` green at HEAD `ce0cef2`.
