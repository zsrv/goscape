...
**No changes.** `insertSession` in `modules/login/db.go:158-168` already passes a `uuid.NewString()` value (from `handler.go` slice-7 code, see `e67a521b`). The CHECK is purely defensive against future regressions.
