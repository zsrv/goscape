...
NAI-96 closed pkg/gamemap/ collision-write divergences at `76dc3f7`. User-launched smoke (2026-05-05 ~12:18 EDT) confirmed:
...
| H3 | `LayerGroundDecor` write-side divergence vs TS `GameMap.ts` — NAI-96 pinned `LayerGround` width/length angle-swap (e84629b/2f4c8e9); `LayerGroundDecor` branch + `ChangeFloor` are single-tile, untouched. TS may swap, may write blockrange, may use a different angle offset. | Low — code diff | **Stage 1.4**: read `pkg/gamemap/gamemap.go` `LayerGroundDecor` branch + `ChangeFloor` callees vs TS `GameMap.ts changeLocCollision` GroundDecor branch; enumerate every divergence. | Per-divergence fix in `pkg/gamemap/gamemap.go`. |
