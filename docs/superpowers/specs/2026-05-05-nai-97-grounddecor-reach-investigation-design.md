...
NAI-96 closed pkg/gamemap/ collision-write divergences at `27dbfbf`. User-launched smoke (2026-05-05 ~12:18 EDT) confirmed:
...
| H3 | `LayerGroundDecor` write-side divergence vs TS `GameMap.ts` — NAI-96 pinned `LayerGround` width/length angle-swap (e95b0ea/b1e8533); `LayerGroundDecor` branch + `ChangeFloor` are single-tile, untouched. TS may swap, may write blockrange, may use a different angle offset. | Low — code diff | **Stage 1.4**: read `pkg/gamemap/gamemap.go` `LayerGroundDecor` branch + `ChangeFloor` callees vs TS `GameMap.ts changeLocCollision` GroundDecor branch; enumerate every divergence. | Per-divergence fix in `pkg/gamemap/gamemap.go`. |
