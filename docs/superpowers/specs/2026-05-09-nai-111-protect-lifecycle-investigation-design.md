...
This line was introduced by **NAI-53 T3** (commit `5af531b`, 2026-05-01) under the **NAI-52 convergence** premise: "TS `this.protect` ↔ goscape `activeScript.Protect`". The convergence collapses two TS fields whose lifecycles drift:
...
- NAI-53 T3 introducing the bug: commit `5af531b`, spec at `docs/superpowers/specs/2026-05-01-nai-53-closemodal-full-port-design.md`.
...
- NAI-133 refactor that renamed `s.Protect bool` → `Pointers&PtrProtectedActivePlayer`: commit `60e5851`. Behavior-preserving rename; did not change the bug.
