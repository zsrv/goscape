...
**Cascade-tail context:** missing-handler audit at HEAD `d865ba1`, **using the tightened regex** `Op[A-Za-z][A-Za-z0-9]*` (per `missing_handler_audit_regex_flaw.md` — the original `Op[A-Za-z]+` regex collapsed `OpBusy` into the dispatched `OpBusy2`), reports **4 unhandled opcodes**:
...
**Re-confirmed at HEAD `d865ba1` via direct re-read of TS source per `spec_ts_source_read.md`:**
