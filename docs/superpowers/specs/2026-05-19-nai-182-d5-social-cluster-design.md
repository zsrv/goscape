...
- **DEVIATION-NAI-182-D5-CHAT-FILTER-NO-RESTORE** — `sendChatFilterSettings(p, p.publicChat, p.privateChat, p.tradeDuel)` emits zeros at fresh login because no save/restore subsystem populates these fields from disk yet. Same shape as `DEVIATION-NAI-182-D1-RECONNECT-NO-RESTORE` from the misc cluster. Retires when PlayerLoading restore lands (or earlier, if a chat-filter-write client packet handler lands and populates between login and first emit).
