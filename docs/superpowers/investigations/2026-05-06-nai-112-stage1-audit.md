...
**Audit subagent:** Sonnet (Explore, read-only); dispatched per plan §2.2 at HEAD `5b1af2f`.
...
Goscape (verified by controller at HEAD `5b1af2f`):
...
## Stage 2.1 — runtime evidence (smoke 2026-05-06 at HEAD `86b004a`)

User-launched smoke against the Bundle 1 instrumentation (`86b004a`). Java client rev-225, fresh account (`LOGIN_RESULT_NEW_PLAYER`), walk through Survival Expert dialog to "Click on the flashing backpack icon to the …", click inventory tab.
...
## Bundle 1.5 instrumentation + second final smoke (2026-05-06 at HEAD `6669c32`)

Re-instrumented four sites for one disambiguation cycle: `handleTutClickSide` entry/lookup/postScript; `Player.OpenTutorial` post-writeOut; `Player.CloseTutorial` post-writeOut; `Player.IfSetTab` entry. Reverted at `a7ae30e` after the smoke trace bound the residual.
...
**NAI-112 PRIMARY (chatbox-advance / `[tutorial,_]` branch dispatch correct, TUT_OPEN wire suppressed): H6.c — TUT_OPEN unconditional re-emit divergence.** Smoke-confirmed at HEAD `1d7d259` second smoke 2026-05-06 → chatbox advances to "Cut down a tree" after inventory-tab click.
