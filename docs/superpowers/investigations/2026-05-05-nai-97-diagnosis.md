...
**HEAD at audit start:** 7d08dd6 (NAI-97 plan commit)
...
```
NAI-97 Bundle 0 pre-flight at HEAD 7d08dd6:
...
|---|---|---|
| `TestNAI97_LocWalkDump_Lumbridge` | PASS (dump-only, 100 lines `-v` output captured) | always passes; output captured at Stage 1.1 (commit `871ef5d`) |
| `TestNAI97_NPC943_PathAroundFountain` | SKIP (anomaly reproduces on empty FlagMap; `{Waypoints:[] Alternative:true Success:true}` pinned) | skipped per plan §8.4; commit `f127d37` |
| `TestNAI97_NPC3_MidRouteAbandonment` | SKIP (same shape, same pin) | skipped per plan §8.4; commit `f127d37` |
...
- Stage 1.1 dump (commit `871ef5d`, captured at `$TMPDIR/nai97_dump.log` at audit time)
...
- Stage 1.1 FlagMap dump (commit `871ef5d`) shows expected per-layer flag writes
...
- Repro A/B commit `f127d37`, captured Route value pinned in `t.Skip`
