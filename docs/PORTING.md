...
|---|---|---|---|---|---|
| ℹ LOW | `pkg/gamedb/migrations/{sqlite,postgres}/000002_drop_chat.up.sql` + `modules/world/handlers_game.go` (`MESSAGE_PUBLIC` handler) + `modules/friends/handler.go` (`PrivateMessage` handler) | `FriendServer.ts:291-306` @3c16994c (public chat persist) + `FriendServer.ts:260-290` @3c16994c (PM persist) | ✅ EXCEPTION-DOCUMENTED | — | **Chat persistence (public_chat / private_chat) — DIVERGENCE.** TS persists public chat (`FriendServer.ts:291-306`) and PMs (`FriendServer.ts:260-290`) to central-DB tables. goscape retires both tables (migration `000002_drop_chat`) and emits `PublicChatEvent` (world module, `MESSAGE_PUBLIC` handler) / `PrivateChatEvent` (friends `PrivateMessage` handler) telemetry events instead — Kafka-only, fire-and-forget. The TS resolve-both-accounts silent-drop for PMs is preserved (`ResolvePrivateMessageEndpoints`). A deployment without a telemetry emitter records chat nowhere. ARCH-1 CLOSED 2026-06-13 (tick error-recovery TS-faithful, backported from rev-274; see backlog note below). Approved in `docs/superpowers/specs/2026-07-07-chat-kafka-only-design.md`. |
...
- 🚧 **friends `public_chat` account_id resolution NO-LANDING-SITE**
  ...
  **RETIRED (2026-07-06):** DB-2 federation is retired — see the "DB-2
  ...
  shared `account` table (`LogPublicMessage`'s new `accountIDByUsername`
...
- ℹ **FriendServerRepository internals NO-OP/N-A** — the orderBy
  ...
  **RETIRED (2026-07-06):** DB-2 federation is retired — see the "DB-2
  ...
  form — but unlike rev-254/274, this branch's own TS pin (`3c16994c`)
  has no members-aware cap split yet: `friendListLimit` stays a flat
  `100` for both `AddFriend`/`AddIgnore`, matching
  FriendServerRepository.ts:233/268 exactly (no `account.members` read
  anywhere in the TS repository at this pin).
...
- Strange Plant (triffid) `opnpc`/`apnpc` `npc_delay` continuation fix (2026-06-14): the player-side `resumeOrFinish` switch was missing a `case script.NpcSuspended` arm, so a player-anchored script that suspends via `npc_delay`/`npc_arrivedelay` (every `opnpc`/`apnpc` carries an ActiveNpc) had its continuation DROPPED at the `default` "unsupported execution state" arm. `macro_event_triffid.rs2`'s `[opnpc1]` pick handler ends `npc_delay(22); npc_del;` — the dropped `npc_del` left the picked plant alive, so its hostile `ai_timer` resumed and attacked the player after they had picked the fruit. Fix stores the continuation on the active NPC (`state.ActiveNpc.StoreActiveScript`), mirroring TS `Player.executeScript` (`script.activeNpc.activeScript = script`); pin test `TestOpNpcScriptNpcDelayStoresContinuationOnNpc`. Verified faithful vs rev-245.2 pin `3c16994c` (`Player.ts:2136-2137`; NPC_SUSPENDED arm unchanged since `9d771f26`), so this restores fidelity rather than forward-porting. Backported from rev-274 `59d007f1`; local commit `43fca517`.
- NPC-dispatcher player-suspend continuation fix (2026-06-14): sibling of the Strange Plant fix on the NPC side, found by auditing all three post-`Execute` dispatch sites for this bug class. An NPC-anchored script that carries an active player (`buildNpcScriptState` binds an ActivePlayer target → `state.Self`, e.g. `ai_opplayer`/`ai_applayer`) and suspends ON THAT PLAYER had its continuation DROPPED in `resumeOrFinishNpc`'s `default` arm (clear+warn). TS `Npc.executeScript` stores it on the active player (`Npc.ts:223-224 @3c16994c`: `} else { script.activePlayer.activeScript = script; }`); fix mirrors that (store on `state.Self`, else clear defensively). LATENT — unreachable in pinned content (the player-suspend opcodes require ProtectedActivePlayer, which NPC-anchored scripts don't grant; combat `ai_opplayer` uses `npc_delay`→NpcSuspended), mirrored for fidelity. Pin test `TestResumeOrFinishNpcStoresPlayerSuspendOnActivePlayer`. Origin rev-274 `5d3dcf4d`; local commit `2431d840`.
- NPC-dispatcher protect-cleanup tail (2026-06-14): completes the `resumeOrFinishNpc` TS-fidelity port (follow-on to the player-suspend fix above). Ported the tail of TS `Npc.executeScript` (`Npc.ts:230-237 @3c16994c`) that releases protected access held on the active player(s) — `script._activePlayer[2].protect = false` + `pointerRemove(ProtectedActivePlayer[2])` — so a stale protect flag (protectedScriptActive) can't block the player's later interactions. The player dispatcher needs no equivalent (it clears the protagonist's own protect, which IS the active player there; TS `Player.executeScript` has no such tail — verified, no parallel gap). LATENT — `buildNpcScriptState` never sets the ProtectedActivePlayer pointer in npc context; mirrored for fidelity. Pin test `TestResumeOrFinishNpcReleasesProtectedActivePlayer`. Origin rev-274 `54ecab8a`; local commit `11dde6b2`.
- Arc 28 fidelity backport (2026-07-02): five goscape-vs-TS fidelity fixes ported from rev-274's arch-28 fix arc (`dcb04092..25f554eb`), all restoring behavior the TS engine had that goscape lost. **28.1 sqlite** — `db.SetMaxOpenConns(1)` + DSN-encoded `busy_timeout(5000)`/`foreign_keys(1)` pragmas on both login and friends `openDB`, matching TS better-sqlite3's single-connection posture (origin `2320a18c`; local `87c6d922`, byte-identical apply for friends/db.go, hand-applied to login/db.go since this branch still carries the pre-rev-254 `countRecentLoginAttempts`/`insertLoginAttempt` functions rev-274 had already retired). **28.2 exit code** — `failedServicesError` restores the post-`AwaitStopped` `ServicesByState[Failed]` check so a crashed module exits non-zero (origin `73ba3977`; local `01c6a00b`, byte-identical apply). **28.3 WS recover** — `HandleConn` now gates admission under a mutex and routes through `serveConn` (owns the panic recover + `tcpWg.Done`) instead of calling `handleTCPConn` directly (origin `ec60a488`; local `0d0e2c38`; field named `admissionGateMu` directly, skipping the intermediate `wsGateMu` name rev-274 later renamed). **28.4 teardown cluster**, four commits folding in rev-274's two post-review fix waves (`8733646d`, `668e273a`) into their thematically-matching canonical commit: (a) guaranteed non-lossy `removalQueue`/`drainRemovals` for disconnect removals, run before the lossy relay queue each tick (origin `78d9b0d9`; local `eec3eff9`); (b) `client.teardownRefs` atomic refcount + `dropConnRef`/`dropTickRef` so pooled conn/tick buffers return to their pools only once both owners are done (origin `2ea5e1de`; local `28cbf158`; the rev-274 hunk's "Task 22a" OnDemand `client_closed` round-robin cleanup — `connID` field, `onDemand.clientClosed` call — has no landing site on this branch, which predates that subsystem, and was omitted, see residual note in the commit); (c) `liveConns`/`liveConnsMu` registry + `closeLiveConns()` so `Shutdown` can force-close a chatty connection instead of hanging on `tcpWg.Wait`, plus `worldServiceFns` extraction so `Run` is spawned from `startingFn` (fixing a stop-during-Starting deadlock), with the fix-wave's synchronous `trackConn`-at-admission-site and quit-gated `serveTCP` accept loop folded in (origin `e4141eb7` + `8733646d` + `668e273a`; local `539826d5`); (d) 2s default `TCPServerWriteTimeout` (was 30s) + `flushWriteOrClose` closing the conn on a tick-goroutine flush error (origin `418bdc8a`; local `a1dccfa6`). **28.5 logout retry** — `sendPlayerLogoutWithRetry` bounds a graceful-logout `PlayerLogout` RPC to 3 attempts over `logoutSaveRetryDelay` (2s default), aborting early on `bridgesCtx` cancellation, closing a loss window a login-service restart could otherwise turn into an M25 DataLoss reject; folds in the `9f644730` final-review polish relevant to this branch (tick ref taken before `appendNewPlayer` in `sendLoginOK`; singular `"attempt"` slog key on both retry-loop branches; accept-gate refusal now logs "refusing connection accepted during shutdown") (origin `c2836731` + `9f644730`; local `c39bf88f`). Not backported: arch-28.6 (CI workflow — repo tooling, not fidelity) and the gofmt-stable doc-comment commit `25f554eb` (rev-274-specific file states). `docs/PORTING.md` (this note) is the only other change (origin `43c0afe3`+`3676a46f`, folded here rather than reproduced verbatim). Full world suite (`go test -count=1 ./modules/world/`) ran clean in ~36s with no drainConn hang, so the rev-274 `85876fa7` cherry-pick noted as a contingency in the backport brief was not needed. Gates: build, compile-all, per-fix targeted tests, and `-race` on `modules/login`, `modules/friends`, and the world subset all green. Arch-28.6 (CI workflow), deferred above as repo tooling, landed separately on 2026-07-02: rev-274's final `.github/workflows/go.yml` (`12cf33a2` + `9f644730`/`25f554eb` polish) was copied verbatim, preceded by a gofmt sweep (5 of 29 dirty files hand-restructured to survive Go 1.26's doc-comment reformatter) so the workflow's format gate passes on first push.
- Arc 29 fidelity backport (2026-07-02): five goscape-vs-TS fidelity fixes ported from rev-274's arch-29 fix arc, all restoring behavior the TS engine had (or closing a goscape-only concurrency/integrity gap the arch-28 backport had documented as a residual). **29.1 OnDemand pump transient ref** — closes the arch-28 residual noted in `client.go`'s `teardownRefs` doc comment (previously "tracked as an arch-28 residual — see PORTING.md Arc 28"): `client.tryRef()` (CAS acquire-if-live) lets the OnDemand pump (this branch's chain: `onDemand.cycle` → `onDemand.send` → `clientODAdapter.send`) take a transient buffer ref around each write+flush, so `handleTCPConn`'s teardown can never pool-return `bufr`/`bufw`/`c.in` while a pump send is in flight; a refused `tryRef` returns `net.ErrClosed`, which `onDemand.send` already treats like any other per-chunk send error (stops sending further chunks; does not close the connection — verified by reading `ondemand.go`, no incidental change needed there). `handleTCPConn`'s pre-login flush is also skipped for `ClientStateOndemand` conns (nothing meaningful to flush once the pump owns the connection). New `client_teardown_test.go` coverage: send-refused-after-teardown, a `-race` send-vs-`dropConnRef` stress loop, and `tryRef`'s acquire/refuse lifecycle (origin `47d72a4b`; local `68011a26`). **29.2 gRPC keepalives** — new `modules/world/grpc_keepalive.go` (byte-identical copy: `clientKeepaliveParams`/`worldClientKeepalive`, 30s probe / 10s timeout / `PermitWithoutStream=true`) wired into both `NewLoginClient`/`NewFriendsClient` dial options; matching `KeepaliveEnforcementPolicy` (15s `MinTime`, `PermitWithoutStream=true`) added to both `modules/login/server.go` and `modules/friends/server.go`'s `newGRPCServer` (origin `9a4a55cf`; local `3a23b925`). **29.3 WorldStartup/WorldConnect retry + login gate** — `LoginClient.WorldStartup`/`FriendsClient.WorldConnect` now return `error` (ripples through both fakes plus `logout_save_retry_test.go`'s `flakyLoginClient` stub — compile-all was the proof, all already TS-faithful no-op/pass-through shapes); `Server.retryBridgeRegistration(name, call)` retries an idempotent registration call every `bridgeRetryDelay` (5s default) on a goroutine parented to `bridgesCtx`, tracked in the new `bridgeWg` (`WaitGroup.Go`) so `Shutdown` joins it right after `bridgesCancel`; `world.go`'s `startingBody` calls it instead of blocking inline. Folds in the same arc's reviewer-caught login-integrity fix: `Server.worldStartupDone` (`atomic.Bool`) gates `callPlayerLoginRPC` — while closed, logins get this branch's own byte-identical login-server-offline reject (`loginresp.OpLoginServerOffline.Opcode`, opcode 8), since WorldStartup's blanket `logged_in=0` wipe must complete before any login is admitted or an eventually-successful retry could erase a live session's flag; `worldStartupCall` opens the gate only on a nil `WorldStartup` error (strictly after the wipe); `initLoginGate` opens it immediately for standalone worlds (nil login client). New `world_startup_retry_test.go` (byte-identical copy — no branch-specific content) plus `TestLoginGateBlocksUntilWorldStartup`/`TestLoginGateOpenWhenStandalone` added to `login_client_test.go` (this branch has no hop-timer caching in `callPlayerLoginRPC`, so those origin tests' HopTimer-adjacent context doesn't apply — the two gate tests stand alone); `newTestServer` now seeds `worldStartupDone.Store(true)` so pre-existing login tests keep the gate open by default (origin `b7ff6a5f` + `750ded55`, both fix waves folded into one commit here; local `8318ce3d`). **29.4 friends shutdown** — `(*subscriptions).closeAll`/`(*worldSubscriptions).closeAll` (byte-identical apply to `subscriptions.go`; hand-applied to `world_subscriptions.go` to preserve this branch's own pre-existing `worldSubscriptions` doc comment, which cites a different TS line range/porting-history note than rev-274's current version of that same comment — an unrelated textual divergence, not a fidelity gap) close every registered subscriber's `done` channel and empty the registry under one lock; `Friends.running`'s `ctx.Done` path calls both before `f.srv.shutdown()` (byte-identical apply to `friends.go`); `grpcServer.shutdown` races `GracefulStop` against a `grace` window (`defaultGracefulStopBound` = 5s in production, injectable for tests) and forces `Stop()` if it elapses (hand-applied to `server.go`, preserving this branch's 29.2 `KeepaliveEnforcementPolicy` wiring already in `newGRPCServer`). New `friends_shutdown_test.go` + `server_test.go` (byte-identical copies, taking the origin's final tightened-2s-bound state from the follow-up commit rather than its first commit's looser 6s bound) needed one adaptation: this branch's `friends.Config` has no `Profile` field (a pre-multi-profile-config shape, unrelated to this fix), so the ported fixtures drop that field from their `Config{}` literals; `subscriptions_test.go`/`world_subscriptions_test.go`'s new `TestSubscriptions_CloseAll`/`TestWorldSubscriptions_CloseAll` cases are byte-identical applies (origin `f1955711` + `d061d9b9`, both folded into one commit here; local `2c2cad90`). **29.13 friends FIFO dispatch** — new `friends_dispatcher.go`/`friends_dispatcher_test.go` (byte-identical copies of the two new origin files): `friendsMutationDispatcher` is a single global unbounded FIFO queue served by exactly one worker goroutine, restoring TS's `World.friendThread.postMessage` global ordering guarantee that this branch's per-RPC goroutine fan-out had lost — a login/logout pair for one player could previously apply to the friends server as logout-then-login (leaving the player permanently "online" against its world cap), and an add-then-delete pair could apply as delete-then-add. `grpcFriendsBridge`'s seven mutation methods (`bridges.go`) now enqueue a closure on the dispatcher instead of firing their own goroutine (the admin Relay* bridge is untouched — opposite direction, out of scope); the inline `PlayerLogin` call in `tick.go`'s `processLogins` and the inline `PlayerLogout` call in `server.go`'s `removePlayerOnTick` likewise enqueue (the login-side `sendPlayerLogoutWithRetry` save/retry path — a different RPC to a different server — is untouched); the worker starts from `world.go`'s `startingBody` via `bridgeWg.Go`, mirroring this branch's existing 29.3 `retryBridgeRegistration` acquisition-in-starting convention, so `Shutdown`'s existing `bridgeWg.Wait()` join covers it with no separate wait; `friends_client.go`'s `PlayerLogin` doc comment gets the callback fast/non-blocking contract note (the callback runs synchronously on the dispatcher's worker, outside the per-call ctx's cancellation reach). Naming kept as `friendsMutationDispatcher` (not the plan's `friendsDispatcher`) for cross-branch consistency with origin — this branch also has a pre-existing `Server.friendsDispatcher` field (the opposite-direction friends-server → world push sink), so the same identifier collision origin avoided applies here too. The two ordering tests, `TestFriendsDispatchPreservesLoginLogoutOrder`/`TestFriendsDispatchAddThenDeleteOrder`, were proven RED against a temporary per-call-goroutine stub standing in for the pre-fix fan-out (captured: `got [logout login]`/`got [del add]`) before converting the call sites, then GREEN after with the real FIFO dispatcher restored (origin `adc2fecf`; local `9e862e4a`). Not backported: arch-29.5's config wiring of `friends.graceful_shutdown_timeout` (not fidelity-tagged; this branch already has its own pre-existing, still-unwired `GracefulShutdownTimeout` config key, left untouched). **N/A verdict check**: rev-225 (no OnDemand conn integration per its own arch-28 backport note) was out of scope for this dispatch; this branch (rev-245.2) HAS the OnDemand pump (confirmed by grep for `clientODAdapter`/`onDemand.cycle` before starting), so 29.1 applies here and was NOT skipped. Operator note for 29.3: a persistently-failing WorldStartup registration now blocks ALL logins by design (the offline reject, opcode 8) until the login server recovers and the background retry succeeds — this is a deliberate behavior change from the pre-fix silent-swallow posture, closing a login-integrity gap at the cost of failing loud instead of failing open. Gates: build, compile-all (`go test -run '^$' ./...`), per-fix targeted tests, `-race` on the targeted world/friends sets plus the full friends package, full `go test -count=1 ./modules/world/ ./modules/friends/` (world ~37-41s), and `gofmt -l .` all green throughout.
- USER-DIRECTED follow-up forward-port (2026-07-03): four rev-274-only operational/lifecycle/config/doc improvements ported onto rev-245.2 at the user's explicit request ("port it too"), mirroring the rev-254 pilot. **This is a deliberate DEPARTURE from goscape's normal no-forward-port policy** — unlike the arch-28/arch-29 entries above, NONE of these restore TS fidelity; they replicate later-rev engine behavior onto an older branch on request. (FFP-5) `docs(world)`: the `grpcLoginClient.PlayerForceLogout` wrapper + its `LoginClient` interface entry are documented as retained-but-uncalled (the disconnect-without-save caller was removed for TS parity; the stale "used on disconnect" comment is corrected to say it is kept to mirror the login RPC surface); comment-only, origin rev-274 `707f5b12`, rev-254 template `c0de0527`, local `50b2eb17`. (FFP-2) `fix(ondemand)`: `initOnDemand` binds the HTTP listener via `server.New` then constructs the OnDemand module; when `ondemand.New` errored the bound socket leaked until process exit (no service ever runs to `Shutdown` it). Added `server.Server.Close()` (releases the bound listener for the construct-then-abort path) and call it on the `ondemand.New` error branch; red-green pinned by `TestServerClose_ReleasesListener` (RED: post-close rebind failed "address already in use" with `Close` stubbed to return nil). Origin `086df9c2`, template `a06a0024`, local `78a0393a`. (FFP-3) `feat(friends)`: `friends.graceful_shutdown_timeout` was declared but UNWIRED — `newGRPCServer` hardcoded `defaultGracefulStopBound` (5s) and the flag defaulted to 30s. Wired `newGRPCServer` to consume `Config.GracefulShutdownTimeout` (coercing a non-positive value to `defaultGracefulStopBound`), changed the flag default to `defaultGracefulStopBound` so an unset key stays 5s (**behavior-preserving: effective default shutdown timing is unchanged**), and added a `Validate` rule rejecting a non-positive `graceful_shutdown_timeout`; the one enabled-config test that built a zero-grace `Config` (`TestFriends_Running_StopsWithOpenSubscriberStream`) now sets it explicitly to `defaultGracefulStopBound`. New `config_test.go` red-green pins the rule (RED: `TestConfigValidate_RejectsNonPositiveGrace` failed with the Validate rule removed). **rev-245.2 divergence from rev-254**: `newGRPCServer` already took `cfg Config`, so no signature/call-site threading was needed; and this branch's friends `Config` has no `Profile` field, so `config_test.go`'s `validConfig()` omits it. Origin `fc19df09` (grace-wiring slice) + `c0b80a98` (Validate rule), template `0a42a4e6`, local `e9aca720`. (FFP-1) `feat(observability)`: the full arch-29.6 `/healthz` (tick-liveness) + `/debug/status` feature with the boot deadline. `world.Server` gains three atomics (`lastTickNano`/`currentTickAtomic`/`lastCycleMillis`) stamped once per tick by `stampTick()` right after `s.currentTick++`, plus `HealthSnapshot()`; `modules/ondemand/health.go` defines a mirrored `ondemand.HealthSnapshot` + `RegisterHealthRoutes(mux, snap)` (import-free of `modules/world`), with the app wiring a field-by-field adapter in `initOnDemand` (coexisting with the FFP-2 `serv.Close()` change in the same function). The boot deadline treats a non-positive `LastTick` as "starting" within `healthzBootGrace` (30s), only flipping to 503 once a real timestamp goes stale (>10s) or the grace elapses with no first tick. Helm `_helpers.tpl` swaps the readiness probe from `tcpSocket` to `httpGet /healthz` on `ondemand-http` for SingleBinary/World; Management keeps `tcpSocket` on `login-grpc` (no HTTP port) — verified by `helm template` for all three modes. **rev-245.2 divergence from rev-254**: this branch's `playerList` is the simpler pid-keyed round-robin registry with a plain-`int` `count`, whereas rev-254's is a later structure (playerLoop buckets + `atomic.Int32` count); `count` was promoted to `atomic.Int32` (all readers updated: `getTotalPlayers`, `PlayerCount`, and the count-reading tests) so `HealthSnapshot`'s cross-goroutine `PlayersOnline` read is race-free — this also removes `getTotalPlayers`'s pre-existing lock-free plain-int read. Origin `8fe1a457` + `eb43c055` (boot deadline) + `384701c2` (review fixup), template `a91c4795`, local `5fe4fd2d`. Gates: `go build ./...`, compile-all (`go test -run '^$' ./...`), `go test ./modules/{ondemand,world,friends}/ ./pkg/dskit/server/ ./cmd/goscape/app/`, `-race` on the touched non-world packages, and `gofmt -l .` (outside `.claude/`) empty — all green.
- Tech-debt cleanup batch (2026-07-03), ported from rev-274's three-agent debt audit of the mature post-Arc-31 tree, mirroring the rev-254 pilot. **Behavior-preserving** — no wire/gameplay/cache-output change; A7 (the `fire{Op,Ap}Trigger{Npc,Loc,Obj}` consolidation) was reviewed and **deliberately skipped** at the rev-274 go/no-go (~180 LOC not worth the hot-path fidelity risk). Items A1-A6 and A8-A10 landed per the rev-274 template. **rev-245.2 divergences**: A4 — 18 `packAndSave*` funcs / 8 in the transmitted set (no `varbit`); `readAndValidate` has one fewer arg than rev-274 (no `transmitted bool`, predates the gate). A5 was a pure move (a couple of import-only bugs were caught and fixed during derivation, not behavior changes). Merge tip `684eff7d`. NOT pushed. Cross-ref rev-274 `1953b743` for the full 5-branch record.
- ✅ **DB-2 federation RETIRED (2026-07-06)** — login+friends now share one central database (`pkg/gamedb`, spec `2026-07-05-central-db-consolidation-postgres`), ported to rev-245.2 per the phase audit (`.superpowers/sdd/audit-port2452.md`, `3c16994c` pin — rev-245.2's OWN TS pin, NOT rev-254's `2e3bcf43` or rev-274's `dee467c8`; `merge-base --is-ancestor` confirms neither pin is an ancestor of the other). **Audit finding: unlike rev-254 (whose pin was byte-identical to rev-274's friends surface), THIS pin has real behavioral deltas from rev-274's already-ported contract.** (1) **No members-aware cap** — `addFriend`/`addIgnore` both select `id`-only from `account` and cap at a flat literal `100` (FriendServerRepository.ts:222/233 addFriend, :258/268 addIgnore, all @3c16994c); the `account.members ? 200 : 100` split rev-254/274 have does not exist yet at this pin. (2) **`public_chat` is still the OLDER 6-column account_id+profile+world-keyed shape**, not the LATER 4-column `session_uuid` shape rev-254/274 target — `LogPublicMessage` resolves the wire `username` (a raw string on this branch, unlike every other Repository method's username37) to `account_id` via a new `accountIDByUsername` helper mirroring TS's `executeTakeFirstOrThrow` (FriendServer.ts:294); a missing account surfaces `errAccountMissing`, mapped by the `PublicMessage` handler to a silent success — matching TS's throw + outer per-connection try/catch drop. (3) rev-245.2's own multi-profile `FriendServer` architecture (per-profile repositories/`req.Profile` wiring throughout `handler.go`) is **preserved, not collapsed** — TS @3c16994c is itself still multi-profile; the single-`Environment.NODE_PROFILE` collapse + `WORLD_CONNECT` profile-mismatch reject are LATER TS changes (present at 2e3bcf43/dee467c8), out of scope for this port. `pkg/gamedb` (sqlite + postgres backends, pgx/v5) copied from rev-274 (source tip `311e0502`; landed here as `b2eb6388`), with one schema delta applied at copy time: `public_chat` gets the 6-column shape above (FK + ON DELETE CASCADE, `idx_public_chat_account` replacing rev-274's `idx_public_chat_session`) instead of rev-274's session_uuid shape; `friendlist`/`ignorelist`/`private_chat`/`login`/`account` and every other table are unchanged, verbatim from rev-274 (audit confirms identical shapes across pins). Restored TS behaviors previously excepted: `addFriend`/`addIgnore` owner+target account resolution (FriendServerRepository.ts:210-247/249-294 @3c16994c) with the flat-100 cap noted above (no profile filter on the COUNT, matching TS exactly); PM account-existence check (FriendServer.ts:275-276 @3c16994c — the `NAI-S4A-D-FED-NO-ACCOUNT-EXISTENCE-CHECK` exception blocks in `repository.go`/`handler.go` are deleted, `LogPrivateMessage` now returns `errAccountMissing` and `PrivateMessage` maps it to a silent success matching TS's throw-inside-catch drop); `public_chat` account_id resolution described above. Deliberate non-FKs (matching TS): `ignorelist.value` (TS's `addIgnore` never resolves its target `value37` against `account` at any pin — no target-account resolution exists in `addIgnore`, so the raw username string is stored as-is). Deletes the federation-era `modules/friends/{db.go,db_test.go,migrations/*.sql}` (superseded by `pkg/gamedb/migrations/`); `friends.go`/`config.go` retire `SQLiteDSN` in favor of `gamedb.Config` (independent-clients model, same posture as login); `cmd/goscape/app/modules.go`'s `initFriends` now passes `g.cfg.Database`. Supersedes the "friends `public_chat` account_id resolution NO-LANDING-SITE" and "FriendServerRepository internals NO-OP/N-A" rows above, both marked retired with a pointer back to this entry. Full audit/citation/test record: `.superpowers/sdd/audit-port2452.md`, `.superpowers/sdd/port2452-t2-report.md`..`t6-report.md`. Phase commits `b2eb6388`..`37241640` (direct on rev-245.2, no merge). NOT pushed.
- Reboot process-exit parity fix (2026-07-06, ORIGIN): a production `::reboot`/`::slowreboot` that finishes draining players must stop the WHOLE process — TS `World.ts` `processShutdown` calls `process.exit(0)` once the last player drains (`src/engine/World.ts:1242` @3c16994c), so an external supervisor restarts a fully-dead node. goscape's world runFn returned `nil` on the graceful-exit branch, so the world service Terminated cleanly and left its sibling modules (ondemand/login/friends) running half-alive under `target: all`. Fixed by returning `modules.ErrStopProcess` from the graceful branch (`modules/world/world.go`) — the service manager's Failure listener turns that sentinel into `sm.StopAsync()`, tearing down every module, while `app.go`'s `failedServicesError` still classifies it as a requested shutdown (process exits 0, logged info not error). `TestWorldServiceFnsRunFnOutcomes` updated from the old nil contract to pin the `ErrStopProcess` return (RED→GREEN). Landed on rev-245.2 in this commit; ported to all five rev branches.
- ➕ **Hiscores read API (2026-08-20, EXTENSION — no Engine-TS counterpart)**: `modules/hiscore` serves public JSON player-card and leaderboard reads over the `hiscore`/`hiscore_large` central-database tables that `modules/login`'s `updateHiscores` already populates on logout (write path ported at `login-server-9`, `ffa2c2c2`/`39172737`). Engine-TS has no hiscore serving endpoint at any pinned revision — `hiscore` appears there only in `db/types.ts`, the Prisma migrations, and the write path in `LoginServer.ts`. This subsystem therefore sits **outside the TS fidelity ledger**; a future parity audit must not report it as an unclosed divergence. The only central-database change is additive indexes — migration `000004_hiscore_indexes` (both `sqlite` and `postgres` variants under `pkg/gamedb/migrations/`), which adds `idx_hiscore_rank`/`idx_hiscore_account`/`idx_hiscore_large_rank`/`idx_hiscore_large_account` and changes no existing table shape. `modules/login`'s write path is untouched by this backport. **Revision-specific note**: on rev-245.2 the write path (`modules/login/hiscore.go`) skips only staff and does NOT gate on `banned_until` — this branch reverts 244-only `ccc263c7` (see the `banned_until gate REMOVED` correspondence-table row above, commit `8046cd75`), so a banned account's stats are exported normally at logout. The read-time visibility filter in `modules/hiscore/store.go` (`staff_mod_level <= 1 AND (banned_until IS NULL OR expired)`) is therefore the **only** place a ban is honoured for hiscores on this branch — unlike rev-274, whose write path also gates on `banned_until`, here a ban is enforced exclusively at read time, and the store's doc comment is worded accordingly. Backported from rev-274 (`a4283a78`); `modules/hiscore` (all 15 files), both `000004_hiscore_indexes` migrations, and the Helm Kong/values/service/networkpolicy templates ported verbatim (every package `modules/hiscore` imports — `pkg/objtype`, `pkg/util/jstring`, `pkg/dskit/middleware`, `pkg/dskit/services`, `pkg/gamedb`/`gamedbtest` — is byte-identical across branches). Hand-adapted: `cmd/goscape/app/{config.go,modules.go,app.go,modules_test.go}` (app wiring — `initHiscore`'s disabled branch follows this branch's own `services.NewIdleService(nil, nil)` convention, matching `initOnDemand`/`initAccount`/etc. here, rather than rev-274's later `arch-29.8` "contribute no service" convention, which this branch's other init funcs don't use either); `production/helm/goscape/templates/_helpers.tpl` (this branch's `goscape.baseConfig` has no `account:` block — same as rev-274 at this point — so the `hiscore:` section was inserted after `friends:`, before `world:`, matching rev-274's ordering); `examples/{bundled/goscape.yaml,full-config-reference.yaml}` (the `hiscore:` block content is identical to rev-274's; only its slot in each branch's existing file differs). Not copied: `docs/hiscores-api.md` (published separately from the docs site, out of scope for this backport) and the `docs/superpowers/specs/2026-08-19-hiscore-api-design.md` spec (rev-274-only planning artifact). Gates: `go build ./...`, `go test ./...` (full suite, 0 FAIL), `go test -race ./modules/hiscore/...`, `gofmt -l` clean, `go vet ./modules/hiscore/...`, both example configs pass `--config.verify=true` (strict YAML decoding), and the three documented `helm template`/`helm lint` gates (default render has zero Kong objects; `hiscoreGateway.createGatewayConfig=true` without the Kong CRD API fails with the prerequisite-guard error; with `--api-versions configuration.konghq.com/v1` it renders 4 `KongPlugin` + 1 `KongConsumer` + 1 `Ingress`).
- 🔒 **SEC1 — Security hardening batch 1 (2026-08-22, back-ported from `rev-274` @ `33f61845`)**: five independent hardening fixes, user-directed onto this branch (spec `docs/superpowers/specs/2026-08-22-sec1-hardening-design.md`, which carries a rev-245.2 back-port note). M-7: `GameLogin.LogValue` redacts the password, ISAAC seed and CRC table from the login debug log (`7c463f16`). M-2: outbound socket writes move off the tick goroutine onto a per-client `outboundWriter` (`modules/world/outbound.go`) behind a bounded async queue, with `client.closeConn()` draining-then-closing at every `*client` teardown site; caps are 512 frames / 1 MiB, and a zero/negative `tcp_server_write_timeout` stamps a 250 ms `fallbackDrainTimeout` on the socket instead of leaving per-write deadlines off indefinitely (`850804e1`, DEVIATION SEC1-D3). **rev-245.2 adaptation**: this branch's OnDemand is the pre-274 scheduler (`OnDemand.ts` @9aadcec4 — a fixed 50 ms cycle draining urgent/extra/ingame in full), not 274's round-robin pump, so the producer-side yield lives in `onDemand.cycle`/`sendBatch`: a client whose `odClient.backlogged()` reports the soft high-water mark has its pending requests put back at the HEAD of their queue, untouched and in order, and retried on the next 50 ms cycle — the connection is never closed for being slow. 274's `serveClient` slice-end, `pump` per-pass progress bound and `backloggedRetryInterval` have no counterpart here (the ticker already re-arms); the check is per request, never mid-file, because this scheduler has no resumable per-client cursor. This adaptation (and the `outbound.go` comment wording that goes with it) was taken over verbatim from the rev-254 back-port, whose pre-port `ondemand.go`, `ondemand_test.go`, `server_accept.go` and `req.go` were byte-identical to this branch's. M-1: `[login]`/`[logout]` trigger calls are wrapped in `recoverPlayer` so one player's panic force-disconnects only that player instead of taking the world down (DEVIATION SEC1-D1); `runTickLoop` recovers an unhandled tick panic, fires `crashSaveAll` — a per-player best-effort `crashSavePlayers` pass that waits on `waitForSaveFlush` — then re-panics so process-exit crash semantics for supervisors are unchanged (DEVIATION SEC1-D2). The per-tick body was extracted from `runTickLoopWithRate` into `tickOnce()` behind the `tickBodyFn` seam (quit-select, nextTick/drift/sleep and cycle-stat bookkeeping stay in the loop, and the move was verified line-for-line pure); the `[logout]` call site moved from `s.runScript` to the existing `s.runScriptFn` seam, same default behaviour (`2ca162c7`). M-8: the account portal gained anonymous-form double-submit CSRF (`goscape_csrf` cookie) via `publicForm()`, session rotation on login, security headers, server read/write/idle timeouts, a 64 KiB request body cap surfaced as 413, and a two-step `/verify-email` flow (GET renders a confirmation form, POST consumes the token) (`289176ec`). M-12: a new `ondemand.debug_status_enabled` config flag (default `false`) gates `/debug/status`; the Helm chart now sets non-root/read-only-rootfs/drop-ALL `securityContext`s, `fsGroupChangePolicy`, pod `resources`, a `tcpSocket` liveness probe, an unmounted service-account token, an `emptyDir` for `/tmp`, `image.digest` support, unconditional `--config.expand-env=true`, and scopes the hiscore `NetworkPolicy` to the Kong proxy when `createGatewayConfig` is set; the Dockerfile pins `USER 65532:65532` (`35ad5f40`). [BACKPORT-FIDELITY] Every item is goscape-own process/deployment hardening with no Engine-TS counterpart to diverge from; wire format is untouched, and the batch transplants no later-revision TS behaviour onto this pin. **User-owed smoke test**: confirm a throttled OnDemand cache download is served slowly rather than disconnected under the new queue.
- 🧩 **Pack ID index generation (2026-08-31, back-ported from `rev-274` @ `47eb0ed7`)**: goscape could not pack a fresh `git clone` of Content. Content gitignores fourteen `pack/*.pack` ID indexes ("since these are generated for the server only, we don't have to share them"), so a clone carries 19 of 30. goscape **read** those indexes and never wrote them: `PackFile.Register`/`Save` were ported but wired only into `pkg/unpack`, and the pack direction ported just the orphan half of TS `validateConfigPack` (as `ValidateConfigPackNames`, itself dead code until `2cfec7ea`). The failure was invisible for as long as every tree the project packed already had the indexes — a developer's prepared checkout — and it surfaced only when CI cloned Content fresh. Same shape as the T30 catches: unported, and vacuous on the content actually tested.
  ...
  - **Ported**: `validateConfigPack`'s register-and-save half for the ten non-transmitted config families, `regenScriptPack` (`.rs2`, brackets retained — the compiler loads this index as its `runescript` symbol table, and without it `script.dat` is 8 bytes), and `validateCategoryPack` (categories have no source files of their own; names come from `category=` references in `.loc`/`.npc`/`.obj`, and `CrawlConfigCategories` is new). Transmitted families are deliberately NOT auto-assigned — their ids are baked into the client cache. rev-274 also gave them TS's missing-id error; see `PIG-D3` below for why this pin does not.
  ...
  - **Ordering**: TS validates each pack on first access, so its family order is irrelevant; goscape's is fixed, and npc resolves `huntmode=` through an index that packs later. `generateConfigPackIndexes` therefore builds all eleven up front, before any family packs.
  - **Deviations**: `PIG-D1` — ids are assigned from `CrawlConfigNames`' ordered slice, never from ranging the `cfgs` map; ids are written into the packed `.dat`, so map order would make two packs of identical content differ. Pinned by `TestRegisterMissingNames_DeterministicAcrossRuns`, mutation-verified on `rev-274` by reintroducing map-order registration. `PIG-D2` — TS's third `validateCategoryPack` gate arm (`didFileSetChange` on a revalidate stamp) is dev-mode hot-reload, which goscape does not model — same reasoning as the existing `revalidatePack` row. `PIG-D3` — this pin has no transmitted/non-transmitted split in `readAndValidate`; that arrives with rev-254's T30 correspondence audit at TS pin `2e3bcf43`. Generation therefore lives entirely in `generateConfigPackIndexes`, `readAndValidate` keeps its orphan-only `validatePackNamesAgainstCfgs`, and neither the self-healing per-family seam nor the transmitted missing-id error is back-ported — adopting them here would import rev-254-era TS behaviour into a rev-245.2 pin.
  - **Verified on this branch**: a bare clone of Content `cbcfe670` — the revision `rev-245.2` pins — goes 18 → 29 indexes and packs successfully with **no Engine-TS involved**, emitting a 38 MB tree and two benign `missing model` warnings alongside the pre-existing `Value is discarded` compiler warnings. `script.dat` is 4,346,779 bytes. The index-for-index byte-parity comparison against Engine-TS was run once, on `rev-274`.
- 🛡 **Pack staleness guard (2026-08-31, back-ported from `rev-274` @ `82772f13`)**: incremental packing was keyed on SOURCE mtimes only, so it could not see that the PACKER's byte layout had changed. That was observed on `rev-274`, where an engine sync moved npc opcodes and a plain `make pack` afterwards left **2,909 artifacts unrebuilt**, including a `hunt.dat` carrying pre-sync opcodes and `mapview/worldmap.jag`; the server refused to boot on the loud path and would have served wrong bytes on the quiet one. Nothing about the hazard is specific to that sync or that branch — freshness is keyed on source mtimes here too — so the guard is back-ported rather than left where it was found. Spec: `docs/superpowers/specs/2026-08-31-pack-staleness-guard-design.md`.
  ...
  - **Enforceability** is the load-bearing half. A format version is only as good as remembering to bump it, and that is exactly the discipline that failed during rev-274's sync. `TestPackFormatVersionGolden` hashes the packers' output over fixed fixtures and fails with a message naming the constant and both required steps. Its fixtures cover the fields this revision actually has — no `check_invcat`, no seq `reachforward`/`duplicatebehaviour` — so both the fixture set and the golden hash are this branch's own. Mutation-verified on this branch: reverting `wanderrange` from server opcode 200 to 26 trips it.
  ...
  - **Not applicable at this pin**: `packMaps` carries no worldmap gate (the worldmap was a standalone tool outside `packAll` until rev-254), so neither the `rebuildWorldmap` hunk nor the fixtures' worldmap stubs are back-ported. The `packall` and CLI pack fixtures need no format stamp either: an unstamped tree forces a rebuild that is harmless here, and they pass unchanged.
  ...
  - **Verified on this branch** against a pack of Content `cbcfe670`: an unstamped tree (one built by the pre-guard packer) forces a full rebuild with the INFO line, then carries `format=1`. With the format unchanged, a second consecutive pack rewrites only **37 of 2,728** artifacts — the unconditional stages — so incrementality is intact. Bumping the version rebuilds the deliberately back-dated artifacts (2 stale → 0) and advances the stamp.
