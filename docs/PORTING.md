...
| ℹ LOW | `pkg/gamedb/migrations/{sqlite,postgres}/000002_drop_chat.up.sql` + `modules/world/handlers_game.go` (`MESSAGE_PUBLIC` handler) + `modules/friends/handler.go` (`PrivateMessage` handler) | `FriendServer.ts:286-297` (public chat persist) + `FriendServer.ts:266-284` (PM persist) | ✅ EXCEPTION-DOCUMENTED | — | **Chat persistence (public_chat / private_chat) — DIVERGENCE.** TS persists public chat (`FriendServer.ts:286-297`) and PMs (`FriendServer.ts:266-284`) to central-DB tables. goscape retires both tables (migration `000002_drop_chat`) and emits `PublicChatEvent` (world module, `MESSAGE_PUBLIC` handler) / `PrivateChatEvent` (friends `PrivateMessage` handler) telemetry events instead — Kafka-only, fire-and-forget. The TS resolve-both-accounts silent-drop for PMs is preserved (`ResolvePrivateMessageEndpoints`). A deployment without a telemetry emitter records chat nowhere. Approved in `docs/superpowers/specs/2026-07-07-chat-kafka-only-design.md`. |
...
| ID | Area | Size | Notes |
|---|---|---|---|
| _(none — ARCH-1 CLOSED 2026-06-13: tick error-recovery now TS-faithful (panic-only retry at NPC lifecycle + world-script queue). Closure row in [`docs/PORTING-CLOSED.md`](docs/PORTING-CLOSED.md))_ |
...
- 🚧 **friends `public_chat` account_id resolution NO-LANDING-SITE**
  ...
  **RETIRED (2026-07-05):** DB-2 federation is retired — see the "DB-2
  federation RETIRED" entry in Recent audit history. `public_chat` now
  writes TS's exact row shape (`session_uuid`, `timestamp`, `coord`,
  `message`); there is no more account_id resolution to land.
- ℹ **FriendServerRepository internals NO-OP/N-A** — the orderBy
  ...
  **RETIRED (2026-07-05):** DB-2 federation is retired — see the "DB-2
  ...
  now account-id-keyed like TS, so these lines DO have a corresponding
  Go form: the cap is members-aware (`friendListLimit`/`membersFriendListLimit`,
  100/200) and `account.members` IS selected to compute it.
...
- Strange Plant (triffid) `opnpc`/`apnpc` `npc_delay` continuation fix (2026-06-14, ORIGIN): the player-side `resumeOrFinish` switch was missing a `case script.NpcSuspended` arm, so a player-anchored script that suspends via `npc_delay`/`npc_arrivedelay` (every `opnpc`/`apnpc` carries an ActiveNpc) had its continuation DROPPED at the `default` "unsupported execution state" arm — whose comment wrongly asserted "player-side scripts cannot reach NpcSuspended". `macro_event_triffid.rs2`'s `[opnpc1]` pick handler ends `npc_delay(22); npc_del;` — the dropped `npc_del` left the picked plant alive, so its hostile `ai_timer` resumed and attacked the player after they had picked the fruit (user-reported). The bug affected every `opnpc`/`apnpc` that delays-then-acts on its NPC. Fix stores the continuation on the active NPC (`state.ActiveNpc.StoreActiveScript`), mirroring TS `Player.executeScript` (`Player.ts:2220-2221 @c86cfd23`: `script.activeNpc.activeScript = script`); pin test `TestOpNpcScriptNpcDelayStoresContinuationOnNpc`. NPC_SUSPENDED arm unchanged across the pinned Engine-TS range (introduced once at `9d771f26`), so this restores fidelity. Commit `59d007f1`; backported to rev-254 (`e8c18897`), rev-245.2 (`43fca517`), rev-244 (`d8c74211`), rev-225 (`adf4b8bf`).
- NPC-dispatcher player-suspend continuation fix (2026-06-14, ORIGIN): sibling of the Strange Plant fix on the NPC side, surfaced by a follow-up audit of all three post-`Execute` dispatch sites for this bug class (player `resumeOrFinish`, world-queue `resumeOrFinishWorld`, npc `resumeOrFinishNpc`). The other two handle every reachable suspension state; `resumeOrFinishNpc`'s `default` arm DROPPED player-suspend states (Suspended/PauseButton/CountDialog) where TS `Npc.executeScript` stores the continuation on the active player (`Npc.ts:225-226 @c86cfd23`: `} else { script.activePlayer.activeScript = script; }`). An NPC-anchored script can carry an active player (`buildNpcScriptState` binds an ActivePlayer target → `state.Self`, e.g. `ai_opplayer`/`ai_applayer`). Fix mirrors TS: store on `state.Self` when present, else clear defensively. **LATENT** — unreachable in the pinned content (player-suspend opcodes require ProtectedActivePlayer, which NPC-anchored scripts don't grant; combat `ai_opplayer` suspends via `npc_delay`→NpcSuspended); mirrored for fidelity so a future content path can't silently lose the continuation. (TS's protect-cleanup tail at `Npc.ts:232-239` was a separate concern at the time — since closed, see the next entry.) Pin test `TestResumeOrFinishNpcStoresPlayerSuspendOnActivePlayer`. Commit `5d3dcf4d`; backported to rev-254 (`c6310917`), rev-245.2 (`2431d840`), rev-244 (`4847913d`), rev-225 (`7bd3f61e`).
- NPC-dispatcher protect-cleanup tail (2026-06-14, ORIGIN): completes the `resumeOrFinishNpc` TS-fidelity port — the last unmodelled piece of TS `Npc.executeScript`. Ported its tail (`Npc.ts:232-239 @c86cfd23`) that releases protected access held on the active player(s) after the dispatch switch: `script._activePlayer[2].protect = false` + `pointerRemove(ProtectedActivePlayer[2])`, gated on the active player being present. Without it, an NPC-anchored script holding protected access on its active player (an `ai_opplayer`/`ai_applayer` run with protect) would leave a stale protect flag (protectedScriptActive stays true) blocking the player's later interactions. The player dispatcher (`resumeOrFinish`) needs no equivalent: it clears the protagonist's own protect, and for a player-anchored script the protagonist IS the active player — TS `Player.executeScript` has no such tail (verified, no parallel gap). **LATENT** — `buildNpcScriptState` never sets the ProtectedActivePlayer pointer in npc context; mirrored for fidelity. Pin test `TestResumeOrFinishNpcReleasesProtectedActivePlayer`. Commit `54ecab8a`; backported to rev-254 (`5e79082f`), rev-245.2 (`11dde6b2`), rev-244 (`2f12813a`), rev-225 (`0197edea`).
- Arc 30 — **Architecture fix arc** (2026-07-02 five-agent architecture review). Plan `docs/superpowers/plans/2026-07-02-arch-fix-arc.md`; arc base `dcb04092` on `rev-274`. Six HIGH findings fixed serially, one branch per fix, each merged `--no-ff` back to `rev-274`; all six are Go-side infrastructure hardening with **no wire/gameplay behavior change**, except `world.tcp-write-timeout`'s default dropping 30s→2s (documented in `examples/full-config-reference.yaml`). `arch-28.1` (sqlite pool left unpooled/unpragma'd, risking serialization issues under concurrent access) **FIXED** `2320a18c` on branch `fix/arch28-sqlite`: serialized the sqlite pool + per-connection `busy_timeout`/`foreign_keys` pragmas. Merged `d81f2732`. `arch-28.2` (a failed module left the process exit 0, masking failures from orchestration/systemd) **FIXED** `73ba3977` on branch `fix/arch28-exit-code`: `app` now exits non-zero when a module fails. Merged `0b064390`. `arch-28.3` (the WS-bridge path bypassed `serveConn`'s panic recovery — a panicking WS-bridge connection could crash the whole process — and the quit gate had a TOCTOU window) **FIXED** `ec60a488` on branch `fix/arch28-ws-recover`: routed WS-bridge connections through `serveConn`'s `recover()` (TS `TcpServer.ts` parity) and closed the gate's TOCTOU with an atomic admission mutex. Merged `0e637ba7`. `arch-28.4` (connection-teardown cluster, 6 commits `78d9b0d9`..`668e273a` on branch `fix/arch28-conn-teardown`, merged `abfd9c48`) bundles four sub-fixes: (a) a guaranteed non-lossy removal queue for disconnects (`78d9b0d9`); (b) refcounted pooled-buffer release across the conn+tick goroutine teardown so the last owner out returns the buffers — previously the conn goroutine recycled `bufw` into a new connection while the tick goroutine was still flushing the old player's frames into it (`2ea5e1de`); (c) `Shutdown` closing live conns + `NewWorldService`'s `serverDone` plumbing restructured (the `run()` spawn moved from `runFn` to the end of `startingFn`) so shutdown no longer hangs when a service is stopped before `runFn` ever executes (`e4141eb7`), plus synchronous `trackConn`/`tcpWg.Add` pairing and the `admissionGateMu` accept-loop admission gate (renamed from `wsGateMu`, now guarding both admission sites) closing the last untracked-conn window (`8733646d`, `668e273a`); (d) a 2s per-flush write budget on the tick goroutine + immediate conn close on flush error, replacing the old unconditional 30s flush that could freeze every player's tick for up to 30s (`418bdc8a`). `arch-28.5` (a failed logout save silently dropped player state with no retry) **FIXED** `c2836731` on branch `fix/arch28-logout-retry`: bounded retry for logout saves. Merged `72838b7d`. `arch-28.6` (no CI signal for rev-* branches) **FIXED** `12cf33a2` on branch `fix/arch28-ci`: build/test/`-race` GitHub Actions workflow. Merged `1d317218`. **Known residuals (not fixed in this arc):** (a) the OnDemand pump goroutine (`onDemand.serveClient` → `clientODAdapter.send`) is a third `bufw` writer outside the conn+tick refcount model introduced by 28.4b — a pure-OnDemand connection's in-flight pump send can race the conn goroutine's pool-return defer on teardown; this race pre-dates the cluster (the old code released the buffers unconditionally at the same point) and is intentionally not fixed here, documented on `client.go`'s `teardownRefs` field comment (closed by Arc 31 arch-29.1); (b) the review's MED/LOW findings remain open, tracked in auto-memory `arch_review_2026_07_02_findings.md`. **Numbering note:** this section already contains an unrelated `Arc 28` (2026-06-01, CategoryType subsystem, two entries above) and `Arc 29` (2026-06-01, hiscore write-path, one entry above) predating this arc's plan; the `arch-28.*` fix-ID prefix, the six `fix/arch28-*` branch names, and the `client.go` `teardownRefs` comment's "see PORTING.md Arc 28" cross-reference all inherit that earlier (mistaken, pre-dated-by-Arc-29) numbering from when the plan was drafted — they refer to this entry, filed here as Arc 30 to avoid a duplicate arc number in this list.
- Arc 31 — **Architecture backlog arc** (2026-07-02, closing the MED/LOW findings and residuals the Arc 30 five-agent review left open). Plan `docs/superpowers/plans/2026-07-02-arch-backlog-arc.md`; arc base `f668bbd3` on `rev-274`. Eleven fixes, one branch per fix (`fix/arch29-<slug>`), each merged `--no-ff` back to `rev-274`. `arch-29.1` (the OnDemand pump goroutine was a third untracked `bufw` writer racing the conn+tick refcount teardown Arc 30's `arch-28.4b` introduced) **FIXED** `47d72a4b` on branch `fix/arch29-pump-ref`: a transient pump reference closes the OnDemand third-writer race. **This CLOSES the Arc 30 known residual** (cross-referenced in that entry above). Merged `0a61c567`. `[BACKPORT-FIDELITY]`. `arch-29.2` (bridge gRPC clients/servers between world/login/friends had no keepalive enforcement, so a half-dead TCP path could wedge silently) **FIXED** `9a4a55cf` on branch `fix/arch29-keepalive`: keepalive probes on bridge clients + enforcement on servers. Merged `43566c3b`. `[BACKPORT-FIDELITY]`. `arch-29.3` (a failed WorldStartup/WorldConnect registration at boot was never retried, permanently stranding crashed-out players at `ALREADY_LOGGED_IN` since nothing else clears `account_login.logged_in`) **FIXED** `b7ff6a5f` on branch `fix/arch29-startup-retry`: `Server.retryBridgeRegistration` retries WorldStartup/WorldConnect every 5s in a `bridgesCtx`-scoped background goroutine until it succeeds once. **Reviewer caught a CRITICAL design flaw before merge:** making the registration async reopened a login-integrity race — `WorldStartup`'s blanket `clearWorldSessions` wipe could land on a retry *after* a player had already logged in during the outage window, falsifying that player's live `logged_in` flag. **FIXED** `750ded55`: gates `callPlayerLoginRPC` behind a `Server.worldStartupDone` atomic, giving pre-registration logins the same offline reject (opcode 8) as an unreachable login server; the gate opens only strictly after a successful WorldStartup call, so every admitted login postdates the one-time wipe (the steady-state variant of the flaw is structurally unreachable once the retry succeeds). Also joins the retry goroutines on `Server.bridgeWg` so `Shutdown` can't return with a retrier still live. With the login gate, a reachable login server whose `WorldStartup` persistently errors now blocks ALL logins on that world (opcode-8 rejects, retried every 5s, logged on both sides) — deliberate: a world that cannot clear stale `logged_in` rows must not admit logins; pre-arc it would have admitted them against stale state. Merged `84f241f2`. `[BACKPORT-FIDELITY]`. `arch-29.4` (friends' gRPC `GracefulStop` had no bound, so a stuck bidi subscriber stream could hang friends shutdown indefinitely) **FIXED** `f1955711` + test-bound tightening `d061d9b9` on branch `fix/arch29-friends-stop`: closes subscriber streams at stop and bounds `GracefulStop` (an e2e test proved a looser 6s bound couldn't distinguish the fix from `GracefulStop`'s own backstop, so it was tightened to 2s with red/green proof). Merged `8c93d02a`. `[BACKPORT-FIDELITY]`. `arch-29.5` (`Config.Validate` only fanned out to `World.Validate`, so `--config.verify` green-lit login/friends/ondemand configs that then failed at boot; the hand-duplicated ondemand/world node-identity fields could drift with no warning; four config keys decoded strictly but had zero consumers) **FIXED** `fc19df09` on branch `fix/arch29-config`: full four-module `Validate` fan-out, an ondemand/world node-field drift warning in `CheckConfig`, hard-fail on an invalid config outside `--config.verify` (previously logged and limped forward to fail again later), and a dead-key purge — see **Compat notes** below. Merged `e17d023f`. `arch-29.6` (the Helm readiness probe only proved the world's TCP listener was accepting connections, not that its tick loop was alive) **FIXED** `8fe1a457` on branch `fix/arch29-healthz`: `GET /healthz` (tick-liveness, boot-grace aware so it doesn't 503 the entire startup window) and `GET /debug/status` on the ondemand HTTP mux; Helm SingleBinary/World probes switched `tcpSocket`→`httpGet /healthz`. Merged `fc0a3131`. `arch-29.7` (several `pkg/io` signatures promised safety they didn't deliver: a truncated RSA login block panicked through `GData` instead of erroring, `PBit`'s grow checks compared the wrong offset, `filestream.New`/`cache.MakeCRCs` panicked on I/O failure instead of returning an error) **FIXED** `35c0a245` on branch `fix/arch29-io-api`: honest error contracts across `RSADec`/`GData`/`PBit`/`filestream.New`/`MakeCRCs`, with ~25 caller sites repo-wide rippled to propagate the new errors instead of panicking. Merged `cf26327e`. `arch-29.8` (constructors acquired real resources — the world TCP listener, the friends-bridge world-events subscriber goroutine — outside the service's starting phase with no rollback path on init failure, and disabled modules masqueraded as Running via a vacuous `IdleService`) **FIXED** `26455ebe` on branch `fix/arch29-lifecycle`: extracts the listener bind and subscriber spawn into `Server.Listen()`/`startWorldEventsSubscriber()`, called from `world.go`'s `startingBody`; the four disabled-module branches in `cmd/goscape/app/modules.go` now return `(nil, nil)` instead of `services.NewIdleService(nil, nil)`; standalone ondemand now builds its own CRC table via `cache.MakeCRCs` instead of serving an empty snapshot. Fix wave `fadbfa6c` closed a reviewer-caught starting-failure bridge-client leak (`BasicService` skips `stoppingFn` entirely when `startingFn` errors, so the bridge-client close funcs had to fire from the starting closure itself, proven exclusive with the stopping-path closes) before merging at `69f0646d`. **Noted behavior change (correct per goals):** `--target all` with everything disabled now fails with dskit's `"no services"` error instead of silently no-opping. `arch-29.9` (dead write-only/discarded code, plus no way to check a running binary's version without a full config load) **FIXED** `76218c76` on branch `fix/arch29-deadcode`: removed the write-only `shutdownRequested` atomic, `world.New`'s discarded local `signals.Handler`, the unused `NewLoginService`/`NewFriendsService` wrappers, and stale ondemand subservices scaffolding; added a `--version` flag that short-circuits before any config file is read, backed by `pkg/util/build.String()`. Merged `9577e06f`. `arch-29.10` (the Makefile carried targets referencing long-deleted `pkg/storage`/`./integration`/`tools/doc-generator`/`production/goscape-mixin` paths, plus a hardcoded `rev-225` `GIT_TARGET_BRANCH`) **FIXED** `1abfdf5d` on branch `fix/arch29-makefile`: pruned the dead targets and `faillint`, derived `GIT_TARGET_BRANCH` from the current branch instead of a stale hardcode. Orphaned-variable follow-up `77295709` (dropped `GOTEST` and the `DOC_*` block whose only consumers were the deleted targets) merged together at `ecd38e94`. `arch-29.11` (two test-debt items: `sendPlayerLogoutWithRetry`'s bridge-cancel abort path had no dedicated pin, and `TestHandleConn_ShutdownRace` mirrored the admission-gate logic by hand instead of racing real `Shutdown`) **FIXED** `daca491d` on branch `fix/arch29-test-debt`: added `TestLogoutSaveAbortsOnBridgesCancel` (RED/GREEN traced to the `bridgesCtx.Err()` give-up guard, not the retry-delay select branch originally suspected) and rebuilt `TestHandleConn_ShutdownRace` on a real live listener + `serveTCP` + `s.Shutdown()` (does not by itself discriminate an `admissionGateMu`-removal fault at default scheduling speed — flagged as a follow-up below). Merged `478af061`. `arch-29.13` (goscape fired one independent goroutine per friends-server mutation RPC — `grpcFriendsBridge`'s Add/RemoveFriend, Add/RemoveIgnore, `SetChatMode`, Private/PublicMessage; the inline `PlayerLogin` call in `tick.go`'s `processLogins`; the inline `PlayerLogout` call in `server.go`'s `removePlayerOnTick` — so gRPC gave no cross-RPC ordering guarantee, unlike TS's single `World.friendThread.postMessage` channel, which is strictly FIFO across every player: a login/logout pair for one player could apply to the friends server as logout-then-login, leaving that player permanently "online" against the friends server's world cap until a fresh login re-registered them, and an add-then-delete pair could apply as delete-then-add; this is the 2026-07-02 five-agent architecture review's MED finding "friends RPC fan-out loses TS FIFO ordering") **FIXED** on branch `fix/arch29-fifo-dispatch`: a new `friendsMutationDispatcher` (`modules/world/friends_dispatcher.go`) is a SINGLE global unbounded FIFO queue served by exactly ONE worker goroutine — the faithful translation of TS's single `postMessage` queue (deliberately NOT per-player: TS has no per-player partitioning, and per-player queues would not even fix the single-player login/logout race that motivated this fix, since that player's login and logout must serialize through the very same queue). Every friends-mutation call site named above now enqueues a closure instead of firing its own goroutine; the call-site survey found all of them are tick-goroutine producers, so enqueue order equals game order — exactly TS. The worker wraps each dequeued action in its own `context.WithTimeout(bridgesCtx, callTimeout)`, so a hung friends-server cannot wedge the queue forever, but a slow call DOES head-of-line-block every mutation queued behind it for up to that timeout — deliberate and TS-faithful, since TS's own friend-thread also processes `postMessage` entries strictly one at a time; arch-29.2's keepalives handle the total-outage fast-fail case upstream of this. The queue is intentionally unbounded (mirroring TS's own unbounded `postMessage` queue — a wedged friend thread backs both up the same way) and warn-logs once per depth threshold (256, 1024) crossed rather than dropping or reordering. The worker is started from `NewWorldService`'s `startingBody` (arch-29.8 acquisition-in-starting convention) and folded into `Server.bridgeWg`, so it exits promptly when `bridgesCancel` fires and Shutdown's existing `bridgeWg.Wait()` joins it with no separate wait needed; entries still queued at that point are abandoned rather than drained, matching TS's own loss of its unprocessed `postMessage` queue on process exit. Two ordering tests (`TestFriendsDispatchPreservesLoginLogoutOrder`, `TestFriendsDispatchAddThenDeleteOrder`) pin the guarantee with a deterministic (channel-gated, not sleep-based) mechanism proven to fail against the old per-call goroutine fan-out before the fix landed. `[BACKPORT-FIDELITY]`.

  **Compat notes:** strict YAML decoding now rejects configs carrying `world.log_format`, `world.enable_tcp_server`, `world.graceful_shutdown_timeout`, or `login.graceful_shutdown_timeout` — all four keys decoded but had zero consumers (verified by grep) and are deleted outright; none of the shipped example configs carried them except `login.graceful_shutdown_timeout` in `examples/bundled/goscape.yaml`, now removed. `friends.graceful_shutdown_timeout` is the opposite case — declared but previously unwired — and is now genuinely wired to bound friends' `GracefulStop` (flag default bumped 30s→5s to match `defaultGracefulStopBound`); `examples/full-config-reference.yaml` is updated to match. **New operational surface:** `GET /healthz` + `GET /debug/status` on the ondemand HTTP mux (arch-29.6), and a `--version` flag (arch-29.9). **`[BACKPORT-FIDELITY]`:** `arch-29.1`/`29.2`/`29.3`/`29.4`/`29.13` are candidates for a later backport pass to rev-254/rev-245.2/rev-244/rev-225 (same worktree machinery as the Arc 28→all-branches backport, on request); `arch-29.5`..`29.11` are rev-274-only operational/tooling improvements and stay off the backport list per the no-forward-port policy (memory: `no_forward_port_deviations.md`) — they hardened goscape's own process/config/build surface rather than restoring TS fidelity, so there is nothing to backport. **Open follow-ups (not fixed in this arc):** double-prefixed `Validate` fan-out errors (e.g. `"login: login: ..."` when a module's own `Validate` already self-prefixes its error) — left for a later polish wave, cosmetic only; `ondemand.New`'s failure branches (`NewSourceIPs`, the optional cache `filestream.New`) still leak the `pkg/dskit/server.Server` listener passed in already-bound as `serv` — needs `pkg/dskit/server` to export a pre-Running close/shutdown hook, noted in arch-29.8's task report; a scheduling-hook test harness idea from arch-29.11, to make `TestHandleConn_ShutdownRace` discriminate an `admissionGateMu`-removal fault at default scheduling speed instead of only under an artificial diagnostic delay. The friends `handler.go` `WorldConnect` doc comment flagged stale by Task 3's review — it claimed a same-world re-`WorldConnect` resets that world's player counter to 0, but `Repository.InitializeWorld` actually preserves the existing count (`TestRepository_InitializeWorld_PreservesExistingCount`) — is fixed in this commit, not deferred. **Numbering note:** the `arch-29.*` fix-ID prefix and the `fix/arch29-*` branch names inherit the plan's original numbering, drafted when the plan author expected the prior arc to land as "Arc 29"; it landed as Arc 30 instead (see that entry's own numbering note), so this entry is filed as Arc 31 to avoid a duplicate arc number — the same prefix-vs-arc-number split Arc 30 used.
 - Architecture follow-up batch (2026-07-03, closing/declining the Arc 31 "Open follow-ups" plus a few review nits). Branch `fix/followups-batch` off `rev-274`, merged `--no-ff` at `c0e482dc`; five commits, all rev-274-only with **no wire/gameplay change**. **Closes** the Arc 31 open follow-up "`ondemand.New`'s failure branches leak the already-bound `pkg/dskit/server.Server` listener": adds `server.Server.Close()` (releases `httpListener` for the construct-then-abort path — the pre-Running close hook that follow-up asked `pkg/dskit/server` to export) and calls `serv.Close()` on `initOnDemand`'s `ondemand.New` error branch (`086df9c2`; red/green-pinned by `TestServerClose_ReleasesListener`; impact is small since an init failure exits the process, but it is the correct close-what-you-bind fix and guards future pre-Run callers). **Declines** the Arc 31 scheduling-hook test-harness follow-up as YAGNI — hooking goroutine scheduling into production shutdown purely to let `TestHandleConn_ShutdownRace` discriminate an `admissionGateMu`-removal fault is not worth the production complexity; the race stays structurally covered and documented in-file. **New hardening:** an `arch-29.6` follow-up gives `GET /healthz` a boot deadline (`healthzBootGrace = 30s`) — a world wired but with no completed first tick (`LastTick.Unix() <= 0`) previously returned 200 forever, hiding a tick loop wedged *during* startup; past the grace it now returns 503 "no first tick" (`eb43c055`, comments corrected by review at `384701c2`; decision extracted into a pure `healthzStatus(snap, hasWorld, sinceBoot)` helper, red/green-pinned by `TestHealthzStatus`). The 30s window is measured from ondemand module init (≈ process start) and spans the world's cache load, so a slow cold-cache boot can 503 transiently — acceptable because `/healthz` is wired only as a readiness probe (never liveness), self-healing on the first tick. `friends.Config.Validate` now rejects a non-positive `graceful_shutdown_timeout` (`c0b80a98`) — it was silently coerced to the 5s default (`newGRPCServer` keeps that coercion as defense-in-depth), masking an operator typo; an omitted key still gets the default. And the world-side `LoginClient.PlayerForceLogout` / `grpcLoginClient.PlayerForceLogout` wrapper — dead since the disconnect-without-save path was removed for TS parity (Arc 30 `arch-28.4`) — is documented as retained-but-uncalled (kept to mirror the live login RPC surface; the proto RPC and login-server handler are untouched) rather than removed (`707f5b12`). Final whole-batch review (opus) READY TO MERGE; both Minor findings (an inverted "never a false positive" claim and a "both branches clock-free" imprecision in the healthz comments) fixed in `384701c2`.
 - Tech-debt cleanup batch (2026-07-03, from a three-agent debt audit of the mature post-Arc-31 tree). **Behavior-preserving** across **all 5 rev branches** — no wire/gameplay/cache-output change. 9 items landed (one, the `fire{Op,Ap}Trigger{Npc,Loc,Obj}` consolidation, was reviewed and **deliberately skipped** at a user go/no-go: ~180 LOC not worth the hot-path fidelity risk of merging six funcs with documented per-type deviations). Items: **A1** `chan interface{}`→`chan struct{}` for the world `quit` signal channel; **A2** removed 5 stale/dead `server.go` TODOs + documented the `net.ErrClosed`/listener-close contracts (`// TODO: WS support` deliberately kept); **A3** new `pkg/pathfinder/loc/layer_test.go` pinning all 23 `Shape→Layer` mappings + a completeness guard against iota reordering; **A4** extracted `readAndValidate` + `saveTransmittedConfig` helpers across the `packAndSave*` funcs in `pack_configs.go` (cache output byte-identical, verified via CRC/parity); **A5** split the 2240-LOC `server.go` god-file into `server_login/accept/players/lookup.go` (proven a **pure move** per branch via a package/import-stripped multiset line-diff = blank-line deltas only); **A6** clamp helpers → builtin `min`/`max`; **A8** extracted `resolveInvOrErr` + `resolveLocType/NpcType/ObjType` + `pushStringOrNull`/`pushNameOrDebugOrNull` in `pkg/script` (error ORDER preserved; `handleInvTotal`/`handleNcOp`/the `*Param` delegators excluded); **A9** `invButtonHandler(n)` factory + `clampCount`/`clampInt32`/`lookupOrReport` cheat helpers (`::kick` kept inline — different miss message); **A10** `math/rand`→`math/rand/v2`, `interface{}`→`any`, and 8 audit-safe `for i:=0;i<N;i++`→`for range N` conversions. Executed subagent-driven (fresh implementer + task review per item), with an opus final whole-branch review on rev-274 (ready-to-merge; one Minor — a `clampInt32` caller-precondition doc note — fixed in `850882c2`). Ported to the 4 older branches (pilot rev-254 → parallel fanout rev-245.2/244/225), each controller-verified (compile-all, gofmt, full `go test ./...` = 0 FAIL, world `-race`, A5 pure-move proof, commit hygiene) and adaptation-reviewed (approved). Per-branch adaptations: **A4** on rev-254/245.2/244 has one fewer helper arg (their `validatePackNamesAgainstCfgs` predates the `transmitted bool` gate; rev-254 keeps `varbit` in the transmitted set, older branches don't); **A4 on rev-225 is a partial port** (extracts only the `saveTransmittedConfig` tail — rev-225 predates `validatePackNamesAgainstCfgs`/BuildVerify entirely, so there is no shared read/validate head to dedupe); **A1 on rev-225** touches fewer sites (no `modules/world/ondemand.go` at that rev); **A5 on rev-225** additionally relocates 5 package-level decls (`errWorldFull`, `logoutSaveAttempts`, `playerSaveFlushTimeout`, `PlayerSaveRate`, `expectedRevision`) to their primary-usage split file + adds a 2-line `expectedRevision` doc comment (behavior-identical, reviewer-judged an improvement, disclosed here as a deviation from the strict pure-move the other branches took). Merge tips: rev-274 `7863557b`, rev-254 `7cf3e702`, rev-245.2 `684eff7d`, rev-244 `f92061cd`, rev-225 `d5cb03c5`. NOT pushed (all 5 branches local; CI fires on push). NOTE: this is a rev-274-originated cleanup ported by explicit user request across all branches (none restores TS fidelity — same forward-port override as the 2026-07-03 healthz campaign, not the default no-forward-port policy).
 - ✅ **DB-2 federation RETIRED (2026-07-05)** — login+friends now share one central database (spec `2026-07-05-central-db-consolidation-postgres`); friends tables re-keyed to TS 274 account-id shape with FK+CASCADE goscape extensions. Restored TS behaviors previously excepted: addFriend/addIgnore owner+target account resolution (`FriendServerRepository.ts:204-296`) incl. members-aware 200 cap, PM account-existence check (`FriendServer.ts:270-284` — the `NAI-S4A-D-FED-NO-ACCOUNT-EXISTENCE-CHECK` exception blocks are deleted), TS-shape `public_chat` `{session_uuid, timestamp, coord, message}` (profile/world recovered via session join). Deliberate non-FKs: `ignorelist.value` (TS allows ignoring nonexistent players), `public_chat.session_uuid` (headless uuids have no session row). Supersedes the rev-244 B5 "friends `public_chat` account_id resolution NO-LANDING-SITE" and "FriendServerRepository internals NO-OP/N-A" rows above, both marked retired with a pointer back to this entry. Phase 2 (postgres backend via pgx/v5, timestamptz schema, gated pg test suites, Helm postgres values) landed 2026-07-05.
 - Reboot process-exit parity fix (2026-07-06, ORIGIN): a production `::reboot`/`::slowreboot` that finishes draining players must stop the WHOLE process — TS `World.ts` `processShutdown` calls `process.exit(0)` once the last player drains (`src/engine/World.ts:1216` @dee467c8), so an external supervisor restarts a fully-dead node. goscape's world runFn returned `nil` on the graceful-exit branch, so the world service Terminated cleanly and left its sibling modules (ondemand/login/friends) running half-alive under `target: all`. Fixed by returning `modules.ErrStopProcess` from the graceful branch (`modules/world/world.go`) — the service manager's Failure listener turns that sentinel into `sm.StopAsync()`, tearing down every module, while `app.go`'s `isRequestedStop` still classifies it as a requested shutdown (process exits 0, logged info not error). `TestWorldServiceFnsRunFnOutcomes` updated from the old nil contract to pin the `ErrStopProcess` return (RED→GREEN). Landed on rev-274 in this commit; ported to all five rev branches.
- ➕ **Hiscores read API (2026-08-19, EXTENSION — no Engine-TS counterpart)**: `modules/hiscore` serves public JSON player-card and leaderboard reads over the `hiscore`/`hiscore_large` central-database tables that `modules/login`'s `updateHiscores` already populates on logout (write path ported at Arc 29, `login-server-9`/`gap-db-datastruct-9`). Engine-TS has no hiscore serving endpoint at any pinned revision — `hiscore` appears there only in `db/types.ts`, the Prisma migrations, and the write path in `LoginServer.ts`. This subsystem therefore sits **outside the TS fidelity ledger**; a future parity audit must not report it as an unclosed divergence. The only central-database change is additive indexes — migration `000004_hiscore_indexes` (both `sqlite` and `postgres` variants under `pkg/gamedb/migrations/`), which adds `idx_hiscore_rank`/`idx_hiscore_account`/`idx_hiscore_large_rank`/`idx_hiscore_large_account` and changes no existing table shape. `modules/login`'s write path is untouched. Spec: `docs/superpowers/specs/2026-08-19-hiscore-api-design.md`. Operator guide: `main:docs/admin/hiscores-api.md` (published on the docs site; it replaced this branch's `docs/hiscores-api.md`, which was deleted once the site page shipped for all five revisions).
- 🔒 **SEC1 — Security hardening batch 1 (2026-08-22)**: five independent hardening fixes landed together on `sec1-hardening` (plan `docs/superpowers/plans/2026-08-22-sec1-hardening.md`, spec `docs/superpowers/specs/2026-08-22-sec1-hardening-design.md`). M-7: `GameLogin.LogValue` redacts the password, ISAAC seed and CRC table from the login debug log (a6bf68c0). M-1: `[login]`/`[logout]` trigger calls are wrapped in `recoverPlayer` so one player's panic force-disconnects only that player instead of taking the world down (59ab2e03, DEVIATION SEC1-D1); `runTickLoop` recovers an unhandled tick panic, fires `crashSaveAll` — a per-player best-effort `crashSavePlayers` pass that waits on `waitForSaveFlush` — then re-panics so process-exit crash semantics for supervisors are unchanged (e181ad7e, a170f4ac, DEVIATION SEC1-D2). M-2: outbound socket writes move off the tick goroutine onto a per-client `outboundWriter` (`modules/world/outbound.go`) behind a bounded async queue, with `client.closeConn()` draining-then-closing everywhere; caps are 512 frames / 1 MiB, and a zero/negative `tcp_server_write_timeout` stamps a 250 ms `fallbackDrainTimeout` on the socket instead of leaving per-write deadlines off indefinitely (4acad516, 417db03c, 5cd18d93, DEVIATION SEC1-D3 — see spec for the OnDemand-pump rationale behind the 512/1 MiB caps and the 250 ms fallback; **user-owed smoke test**: confirm a throttled OnDemand cache download is not disconnected under the new queue); because a ~506-byte OnDemand chunk occupies a whole slot, the OnDemand pump no longer relies on those caps at all — `serveClient` consults `outboundWriter.Backlogged()` (soft high-water: half of each cap) and ends the client's slice with its queue and pending requests intact, retrying on a later pump pass, so a slow downloader is served slowly instead of disconnected (6c3aea7b). M-8: the account portal gained anonymous-form double-submit CSRF (`goscape_csrf` cookie), session rotation on login, security headers, server read/write/idle timeouts, a 64 KiB request body cap surfaced to the client as 413, and a two-step `/verify-email` flow (GET renders a confirmation form, POST consumes the token) so email verification is no longer a state-changing GET (7b2c60a0, 646c1368, 479ddfc7, ad73e0f5). M-12: a new `ondemand.debug_status_enabled` config flag (default `false`) gates the `/debug/status` endpoint (9a6e13a4); the Helm chart now sets non-root/read-only-rootfs/drop-ALL `securityContext`s, pod `resources`, a `tcpSocket` liveness probe, an unmounted service-account token, an `emptyDir` for `/tmp`, `image.digest` support, unconditional `--config.expand-env=true`, and scopes the hiscore `NetworkPolicy` to the Kong proxy when `createGatewayConfig` is set; the Dockerfile pins `USER 65532:65532` (image base stays `distroless/static:debug-nonroot`) (5e7b42c7). [BACKPORT-FIDELITY] M-1 and M-2 are goscape-own process-hardening changes with no Engine-TS counterpart to diverge from and are off the backport list per `no_forward_port_deviations`; M-7, M-8 and M-12 touch goscape-only surfaces (log redaction, account portal, Helm chart) and may be backported to any rev branch that carries the account portal / Helm chart.
- 🔄 **rev-274 engine sync — Engine-TS `4c95f87e`→`1d25566c`, Content `37607266`→`2b62ae68d` (2026-08-31)**: ports upstream `8139461a` "Synced engine with 289 improvements" (94 files, +2,455/−6,716). Spec `docs/superpowers/specs/2026-08-31-rev274-engine-sync-289-design.md`, plan `docs/superpowers/plans/2026-08-31-rev274-engine-sync-289.md`. The companion commit `1d25566c` touches only `public/client/*.js` (the minified JS-client bundle) and is a no-op here — goscape serves the Java client. **Baseline note:** the `274-GOSCAPE` bookmarks in the Engine-TS and Content checkouts had been 4 commits stale since the 2026-07-16 pin update, so the apparent diff `dee467c8..1d25566c` overstated the work by four already-ported commits; `main:REFERENCES.md`, not the bookmark, is the authority on the porting baseline.
  - **Config decode/encode**: hunt `check_invcat` (new server opcode 18, pushing `extracheck_var` from 18–20 to 19–21); npc `wanderrange`/`maxrange` move to server opcodes 26/27 with `maxrange` defaulting to `wanderrange + 2` via a new `PostDecode` that must run after BOTH decode passes; seq `stretches`→`reachforward` and `duplicatebehavior`→`duplicatebehaviour`; loc/spotanim `hasalpha`, npc `hasanim` and obj `code9` retired (they now **reject** rather than parse-and-ignore); spotanim `orientation`→`angle`; obj `ManWearOffsetY`→`ManWearOffset`. The `maxrange` default is the one entry with runtime effect — it changes hunt/retreat geometry for every npc that omits the key.
  - **Collision restructure**: `FlagBlockNPCs`→`FlagNpcOcc`, `FlagBlockPlayers`→`FlagBlockNpcAndPlayers` (both keep their bit), new `FlagPlayerOcc` at bit 22, `FlagRoof` pinned explicitly at bit 31. New `BlockWalk.PLAYER` with a deliberately asymmetric movement case (clear both markers on the old tile, set only `PLAYER_OCC` on the new). `Npc.blockWalkFlag` rebuilt around two orthogonal opt-outs over an always-present hard block. `breakRouteFinding` dropped from the routefinder API; `ChangePlayer` split into `ChangeBlock` + `ChangePlayerOcc`.
  - **Correctness**: `Entity.SetLifecycle` clamps non-sentinel ticks to ≥1; `removeNpc` is idempotent; the npc-turn panic recovery evicts with duration 0 rather than the `-1` sentinel; `Npc.Cleanup`/respawn clear the `activeScript`/`delayed`/`delayedUntil` trio; `Zone.UpdatedThisTick` replaces a lifecycle-tick test when deciding obj visibility on a full zone refresh; the IPv6 login-bucket key is now the full left-packed address instead of "third hextet mod 256".
  - **Script**: 49 player opcodes renumbered (`P_TEMPRUN` at 2088 and `P_TRANSMOGRIFY` at 2092 are inserted mid-enum, not appended), so every compiled script byte moves; four new opcodes; six new pointer-table rows plus `FINDHERO` require and `OBJ_FIND` conditional; transmogrification in the appearance block (**wire-visible**); `map_findsquare` rewritten to collect-then-roll with a 100-tile cap.
  - **Pack**: `PixPack` derives crop bounds by scanning for non-magenta and chooses pixel order by transition count with a flipped polarity; the `.opt` sidecar is now tiling-only; title/textures jagfile member order changed; `versionlist` `anim_index` carries real frame→animset bases; five archive CRC gates moved onto the saved file bytes (title `410306098`, interface `2135735991`, textures `915347346`, wordenc `1386621111`, sounds `-759577225`); `data/raw/wordenc` refreshed to 13,463 bytes.
  - **Networking**: JS-client archive HTTP routes are CRC-gated and 404 on mismatch (`/crc` stays ungated).
  - **Verification**: pack parity is byte-exact — every one of the 56 artifacts the reference and goscape both produce is identical, verified file-by-file rather than by stage counters. The reference emits 76 files; the other 20 are TS incremental-build bookkeeping under `.stamps/`/`.cache/`. Full `go test ./...` green; `-race` clean on the touched packages.
  - **Deviations**: `SYNC289-D1` — goscape does NOT adopt upstream's removal of the 1000 ms `setTimeout` before socket close; the SEC1-D3 bounded outbound drain provides the flush guarantee that timeout was crudely approximating, and adopting the TS shape would reintroduce the truncated-final-packet bug SEC1-D3 fixed. `SYNC289-D2` — `PORTING-EXCEPTION (unpack-tools-go-only)`: upstream deleted `tools/unpack/**` and `tools/render/sound/**` (~4,900 lines); goscape **keeps** `pkg/unpack` + the `goscape-cli unpack` subcommand, following the `symbols-export-go-only` precedent. The deletion is repo housekeeping with no runtime, packed-byte or wire consequence, while goscape's counterpart is a shipped capability with 16 unpack families and a decoder-conformance fixture corpus. Consequence: from this pin forward `pkg/unpack` is goscape-owned and has no upstream counterpart to track, so a future audit must not report its divergence as drift. `SYNC289-D3` — `src/web.ts`'s Express→Fastify migration is out of scope (goscape's HTTP surface is `pkg/dskit/server`); only its CRC-gated archive-route behaviour is ported. `SYNC289-D5` — goscape KEEPS its runtime pointer checks; upstream deleted `checkedHandler`/`ScriptState.pointerCheck` and now enforces only at compile time via `ScriptOpcodePointers`. goscape's copy of that table is compile-time-only too (consumed solely by `pkg/pack/compiler`), so the `require*` helpers in `pkg/script` are a separate runtime layer with no TS counterpart left. Dropping them would mean trusting that every executed script came from this compiler — but bytecode is read from the cache at startup, so a hand-edited or corrupt `script.dat` would nil-deref in a handler instead of returning a clean script error, converting a contained failure into a panic for SEC1-D1's `recoverPlayer` to catch. `SYNC289-D4` — the `routeBlockerFind*` pathfinder variants and the `useRouteBlockerFlags` branch are deleted along with the flags they read; the subsystem was write-only on both sides (`changeLocCollision` hardcoded `breakroutefinding=false`, and goscape's `pkg/gamemap` did the same), upstream never had those variants at all, and goscape reached them only from one test.
  - **Pre-existing divergence recorded here for the first time** (surfaced, not caused, by this sync's parity run): goscape emits 2,900 loose map files under `client/maps` (966) and `server/maps` (1,934) that the TS reference does not produce at all — TS writes maps into `main_file_cache` idx4, while goscape additionally serves per-zone files over the goscape-specific HTTP `/maps/` fallback route (`modules/ondemand/handler.go`, no TS analog). `smoke-pack --reference-dir` reports these as `MISS`; they are expected and are the only non-empty diff class remaining.
  - **Compiler**: `@lostcityrs/runescript` `0.9.6`→`0.9.7` (plan task H2). One concrete delta found and ported: the `logout` trigger no longer declares a boolean return, and Content `687b6a1a1` rewrote `logout.rs2` from `[logout,_]()(boolean)` + `return(true)` to a bare `[logout,_]` to match (the old file even carried a `// TODO: Change compiler to remove return value`). The engine never read the value — `World.ts:780-790` executes the script and discards the result at both pins. Separately, upstream fixed `Compiler.ts:144` writing the `corrupt2` arm into `commandInfo.corrupt`; goscape had reproduced that bug and now follows the fix. Byte parity of `server/script.dat` against the 0.9.7-built reference is the evidence that no further codegen delta exists.
- 🛡 **Pack staleness guard (2026-08-31, ORIGIN — follow-up to the rev-274 engine sync)**: incremental packing was keyed on SOURCE mtimes only, so it could not see that the PACKER's byte layout had changed. After the 289 sync moved npc `wanderrange`/`maxrange` to opcodes 26/27 and `extracheck_var` to 19-21, a plain `make pack` left **2,909 artifacts unrebuilt** — including a `hunt.dat` still carrying pre-sync opcodes from June, and `mapview/worldmap.jag`. The server refused to boot on the loud path (`unrecognized npc config code 201`) and would have served wrong bytes on the quiet one. Spec: `docs/superpowers/specs/2026-08-31-pack-staleness-guard-design.md`.
  - **Mechanism**: `pack.FormatVersion` (one global version) is recorded to `<outDir>/.stamps/pack-format.txt` after a successful pack. `pack.BeginPack` compares the recorded version against the running binary's and, on a mismatch — or on an unstamped tree, which cannot be identified and is therefore assumed stale — latches `pack.ForceRebuild()` for the run. The three `ShouldBuild*` helpers consult the latch, which covers the config pipeline, `maps`, and `clientinterface`. The one presence-only gate (`rebuildWorldmap` in `pkg/pack/maps`) consults it explicitly; `graphics`/`midi`/`sprites`/`audio`/`versionlist` rebuild unconditionally and need nothing. The guard lives in `PackAll`, not the CLI, because `modules/world/rebuild_worker.go` calls `PackAll` in-process for the `::rebuild` cheat.
  - **Enforceability** is the load-bearing half. A format version is only as good as remembering to bump it, and that is exactly the discipline that failed during the sync. `TestPackFormatVersionGolden` hashes the packers' output over fixed fixtures and fails with a message naming the constant and both required steps. Mutation-verified: reverting a single opcode (`wanderrange` 26 → 200) trips it.
  - **`make pack` now wipes `$(CACHE_OUT_DIR)` first**; `make pack-incremental` is the opt-in fast path. Belt-and-braces over the stamp, covering what the stamp cannot see (an interrupted half-finished pack).
  - **Closes a documented PERMANENT omission**: `pkg/pack/maps/pack.go` carried "TS Pack.js:130 `packerUpdated = shouldBuildFile(__filename, mapFile)`; Go has no `__filename` analog; omit packerUpdated (permanent — no script mtime)". The format stamp supplies the identity that comment said was unavailable.
  - **Deviations**: `PSG-D1` — coverage extends to the config families, which TS leaves exposed. Upstream stamps only four stages via `didFileSetChange` (`FsCache.ts:165` @1d25566c: `graphics/pack.ts:15`, `map/Pack.js:244`, `midi/pack.ts:14`, `PackFile.ts:204,327`), and in `packConfigs` only `category` checks the packer sources (`PackShared.ts:360`) while the other twenty families use `shouldBuildConfigOutput`, which does not. **The bug therefore exists upstream too**, masked because TS's documented workflow is `npm run clean && npm run build` whereas goscape's is a bare `make pack`. `PSG-D2` — the stamp records a format version rather than TS's `<packer source>=<mtime>`, forced by goscape shipping a compiled binary with no `.go` files at runtime. Hashing the executable was considered and REJECTED: `make pack` injects a fresh `BuildDate` via ldflags, so every build yields a different binary and the guard would have permanently disabled incrementality while appearing to work.
  - Verified: `make pack-incremental` with an unchanged format leaves deliberately back-dated artifacts alone (incrementality intact); bumping the version rebuilds them (2 stale → 0) with an INFO line naming the change; byte parity against `Server274-ref` unchanged at 54/56 shared artifacts, the two exceptions being `script.dat`/`script.idx` whose embedded content path differs by invocation.
- 🧩 **Pack ID index generation (2026-08-31, ORIGIN — unported hunk found by packing a bare Content clone)**: goscape could not pack a fresh `git clone` of Content. Content gitignores fourteen `pack/*.pack` ID indexes ("since these are generated for the server only, we don't have to share them"), so a clone carries 19 of 30. goscape **read** those indexes and never wrote them: `PackFile.Register`/`Save` were ported but wired only into `pkg/unpack`, and the pack direction ported just the orphan half of TS `validateConfigPack` (as `ValidateConfigPackNames`, itself dead code until `2cfec7ea`). The failure was invisible for as long as every tree the project packed already had the indexes — a developer's prepared checkout — and it surfaced only when CI cloned Content fresh. Same shape as the T30 catches: unported, and vacuous on the content actually tested.
  - **Symptom**: `PackConfigs: invalid property value in <file>: param=pyre_level,5` — an `int` param assigned `5`, apparently failing to parse. It never reached parsing: `parseStructConfigFor` missed `paramTypes.ConfigNames[name]`, and `read_typed.go` reports the generic envelope while **discarding** the underlying `unknown param`. Every fix advanced the failure to the next unindexed family, across two revisions and three different config kinds.
  - **Ported**: `validateConfigPack`'s register-and-save half for the ten non-transmitted config families, `regenScriptPack` (`.rs2`, brackets retained — the compiler loads this index as its `runescript` symbol table, and without it `script.dat` is 8 bytes), and `validateCategoryPack` (categories have no source files of their own; names come from `category=` references in `.loc`/`.npc`/`.obj`, and `CrawlConfigCategories` is new). Transmitted families are deliberately NOT auto-assigned — their ids are baked into the client cache — and instead gained TS's missing-id error, which goscape had never checked in that direction.
  - **Closes `NAI-194-D-PACKFILE-SINGLETONS-DEFERRED`.** TS keeps `EnumPack`/`ObjPack`/etc. as module-level singletons; `loadParamLookups` built its own instances via `NewPackFile`, snapshotting each index at construction. Harmless while the files always predated packing, wrong once they are generated mid-run: `ensureLk` runs before anything packs, so a lookup captured an empty `enum.pack` and dbrow then failed with `double-check the reference exists`. The lookups now take the `Registry`'s instances, which `varpPF` already did — `registry.go:52` is the only `PackFile` construction site left in the pack path. `Registry` gained `Enum` and `Script`.
  - **Ordering**: TS validates each pack on first access, so its family order is irrelevant; goscape's is fixed, and npc resolves `huntmode=` through an index that packs later. `generateConfigPackIndexes` therefore builds all eleven up front, before any family packs. The per-family seam remains as a self-healing no-op.
  - **Deviations**: `PIG-D1` — ids are assigned from `CrawlConfigNames`' ordered slice, never from ranging the `cfgs` map; ids are written into the packed `.dat`, so map order would make two packs of identical content differ. Pinned by `TestSyncConfigPackNames_DeterministicAcrossRuns`, mutation-verified by reintroducing map-order registration. `PIG-D2` — TS's third `validateCategoryPack` gate arm (`didFileSetChange` on a revalidate stamp) is dev-mode hot-reload, which goscape does not model — same reasoning as the existing `revalidatePack` row.
  - **Verified**: a bare clone of Content `2b62ae68` goes 19 → 30 indexes and packs successfully with **no Engine-TS involved**, emitting the same 39 MB tree and the same three benign `missing model` warnings. All twelve generated indexes are **byte-identical** to those Engine-TS `1d25566c` produces from the same commit, including the 10,940-line `script.pack` and 1,291-line `dbrow.pack`. Packed output matches the Engine-TS-assisted pack file-for-file except `script.dat`/`script.idx`, whose 10,940-byte delta is exactly one byte per script — the embedded absolute source path, the invocation-dependent difference the staleness-guard entry above already records. Packing one tree twice is byte-identical.
