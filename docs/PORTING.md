...
- ℹ **FriendServerRepository internals NO-OP/N-A** — the orderBy
  ...
  **RETIRED (2026-07-06):** DB-2 federation is retired — see the "DB-2
  ...
  Go form: the cap is members-aware (`friendListLimit`/
  `membersFriendListLimit`, 100/200) and `account.members` IS selected
  to compute it.
...
- Strange Plant (triffid) `opnpc`/`apnpc` `npc_delay` continuation fix (2026-06-14): the player-side `resumeOrFinish` switch was missing a `case script.NpcSuspended` arm, so a player-anchored script that suspends via `npc_delay`/`npc_arrivedelay` (every `opnpc`/`apnpc` carries an ActiveNpc) had its continuation DROPPED at the `default` "unsupported execution state" arm. `macro_event_triffid.rs2`'s `[opnpc1]` pick handler ends `npc_delay(22); npc_del;` — the dropped `npc_del` left the picked plant alive, so its hostile `ai_timer` resumed and attacked the player after they had picked the fruit. Fix stores the continuation on the active NPC (`state.ActiveNpc.StoreActiveScript`), mirroring TS `Player.executeScript` (`script.activeNpc.activeScript = script`); pin test `TestOpNpcScriptNpcDelayStoresContinuationOnNpc`. Verified faithful vs rev-254 pin `2e3bcf43` (`Player.ts:2218-2219`; NPC_SUSPENDED arm unchanged since `9d771f26`), so this restores fidelity rather than forward-porting. Backported from rev-274 `59d007f1`; local commit `e8c18897`.
- NPC-dispatcher player-suspend continuation fix (2026-06-14): sibling of the Strange Plant fix on the NPC side, found by auditing all three post-`Execute` dispatch sites for this bug class. An NPC-anchored script that carries an active player (`buildNpcScriptState` binds an ActivePlayer target → `state.Self`, e.g. `ai_opplayer`/`ai_applayer`) and suspends ON THAT PLAYER had its continuation DROPPED in `resumeOrFinishNpc`'s `default` arm (clear+warn). TS `Npc.executeScript` stores it on the active player (`Npc.ts:225-226 @2e3bcf43`: `} else { script.activePlayer.activeScript = script; }`); fix mirrors that (store on `state.Self`, else clear defensively). LATENT — unreachable in pinned content (the player-suspend opcodes require ProtectedActivePlayer, which NPC-anchored scripts don't grant; combat `ai_opplayer` uses `npc_delay`→NpcSuspended), mirrored for fidelity. Pin test `TestResumeOrFinishNpcStoresPlayerSuspendOnActivePlayer`. Origin rev-274 `5d3dcf4d`; local commit `c6310917`.
- NPC-dispatcher protect-cleanup tail (2026-06-14): completes the `resumeOrFinishNpc` TS-fidelity port (follow-on to the player-suspend fix above). Ported the tail of TS `Npc.executeScript` (`Npc.ts:232-239 @2e3bcf43`) that releases protected access held on the active player(s) — `script._activePlayer[2].protect = false` + `pointerRemove(ProtectedActivePlayer[2])` — so a stale protect flag (protectedScriptActive) can't block the player's later interactions. The player dispatcher needs no equivalent (it clears the protagonist's own protect, which IS the active player there; TS `Player.executeScript` has no such tail — verified, no parallel gap). LATENT — `buildNpcScriptState` never sets the ProtectedActivePlayer pointer in npc context; mirrored for fidelity. Pin test `TestResumeOrFinishNpcReleasesProtectedActivePlayer`. Origin rev-274 `54ecab8a`; local commit `5e79082f`.
- Arc 28 fidelity backport (2026-07-02): six goscape-vs-TS bug fixes ported from rev-274's arch-28 review (arc base `dcb04092`, head `25f554eb`), restoring behavior TS's single-threaded/in-process model had that the gRPC-split, connection-pooled Go server had lost. All six had a direct landing site on rev-254 — no N/A items. (28.1) sqlite: `openDB` now serializes the pool (`SetMaxOpenConns(1)`) and carries `busy_timeout`/`foreign_keys` as per-connection DSN pragmas instead of a one-shot `PRAGMA` exec, matching TS better-sqlite3's single-connection posture; origin `2320a18c`, local `bde123bb`. (28.2) exit code: `App.Run` now inspects `ServicesByState()[Failed]` after `AwaitStopped` and returns a joined error (ignoring `ErrStopProcess`/`context.Canceled`), so a crashed module exits non-zero instead of 0; origin `73ba3977`, local `4b0c889f`. (28.3) WS recover: `HandleConn` now runs the same `admissionGateMu`-gated quit-check/`tcpWg.Add` admission gate and routes through `serveConn` (not `handleTCPConn` directly), so a malformed WS-framed login hits the gap-login-wire-1 recover instead of crashing the process — implemented directly under the mutex's final name (`admissionGateMu`) rather than the transitional `wsGateMu` rev-274 started with; origin `ec60a488`, local `5d97d233`. (28.4) connection-teardown cluster, four commits mirroring rev-274's split: (a) guaranteed non-lossy `removalQueue`/`enqueueRemoval`/`drainRemovals` for disconnect removals, replacing the lossy `relayActionQueue` (origin `78d9b0d9`, local `5b96c51e`); (b) `client.teardownRefs` refcounts the conn+tick goroutines' shared pooled buffers so neither side's `handleTCPConn` defer nor `removePlayerOnTick` releases them out from under the other, with the `sendLoginOK` ref-before-publish ordering folded in from the arc's final-review polish `9f644730` (origin `2ea5e1de`, local `99790179`); (c) `liveConns`/`trackConn`/`closeLiveConns` let `Shutdown` force-close a chatty connection instead of hanging on `tcpWg.Wait()` forever, plus `worldServiceFns` moves the `serverDone`-feeding goroutine from `runFn` to `startingFn` so a stop-during-Starting transition can't deadlock — folds in the admission-site `trackConn` relocation and `closeLiveConns` snapshot-outside-lock fix-waves (`8733646d`, `668e273a`) directly rather than as separate transitional commits, including that pair's `9f644730` refusal-log text and the OnDemand-pump (`onDemand.cycle` → `clientODAdapter.send`) third-writer residual note on `teardownRefs` — rev-254 has that pump writing into `c.bufw`, so the residual applies here too (origin `e4141eb7`, local `effd5505`); (d) `flushWriteOrClose` + a 2s default `TCPServerWriteTimeout` (was 30s) bound the tick goroutine's per-flush stall and close a dead conn on write failure instead of leaving it to the read-side timeout (origin `418bdc8a`, local `ca69c861`). (28.5) logout retry: `sendPlayerLogoutWithRetry` gives a graceful logout's `PlayerLogout` RPC up to 3 attempts (2s apart, aborting early on `bridgesCtx` cancellation) instead of one, closing the restart-blip data-loss window the gRPC split introduced versus TS's in-process login worker queue; the `9f644730` singular-`"attempt"` slog-key polish is folded in on both the Warn and Error branches; origin `c2836731`, local `c87ce295`. Not backported: arch-28.6 (CI workflow — repo tooling, not fidelity) and the gofmt-stable doc-comment commit `25f554eb` (rev-274-specific file states). Gates: `go build`, compile-all (`go test -run '^$' ./...`), the arc's targeted tests, `-race` on `modules/login`, `modules/friends`, and the world subset, and the full `modules/world` suite (36s, no hang — this branch's tip already carries the `drainConn` 1s→100ms test-deadline fix) all green. Follow-up (2026-07-02): arch-28.6 lands separately — `.github/workflows/go.yml` copied verbatim from rev-274's final state (`25f554eb`), preceded by a repo-wide gofmt sweep (23 safe files) plus a 5-file gofmt-stable doc-comment conversion (`inv_test.go`, `namemap_test.go`, `packfile_test.go`, `worldmap.go`, `symbols_export.go`) so the branch is CI-format-clean on first push.
- Arc 29 fidelity backport (2026-07-02): five goscape-vs-TS bug fixes ported from rev-274's arch-29 review (arc base `f668bbd3`), continuing the arch-28 backport's restoration of behavior the gRPC-split, connection-pooled Go server had lost versus TS's single-threaded/in-process model. All five had a direct landing site on rev-254 — no N/A items (this branch's arch-28 backport already recorded that the OnDemand pump writes into `c.bufw`, so 29.1's residual applies here, unlike rev-225 which has no OnDemand conn integration). (29.1) OnDemand pump transient ref: `client.tryRef()` (CAS acquire-if-live) lets the OnDemand pump (`onDemand.cycle` → `clientODAdapter.send`) take a transient buffer ref around each write+flush instead of being an unmodeled third writer outside `teardownRefs`; `clientODAdapter.send` now refuses with `net.ErrClosed` once the last standing ref (conn/tick) has released the pooled buffers, and `handleTCPConn`'s defer skips the pre-login flush for `ClientStateOndemand` conns (the pump co-owns `bufw` there); the `teardownRefs` doc comment naming the pump as a residual third writer is rewritten to describe its transient participation; `client_teardown_test.go` gets the origin's send-refused-after-teardown / -race send-vs-dropConnRef stress / tryRef-lifecycle coverage verbatim (that file was byte-identical to the arc base); origin `47d72a4b`, local `006e8a1c`. (29.2) gRPC keepalives: new `modules/world/grpc_keepalive.go` (`clientKeepaliveParams`/`worldClientKeepalive`, 30s probe / 10s timeout / `PermitWithoutStream=true`) wired into both `NewLoginClient`/`NewFriendsClient` dial options, and a mirrored `grpc.KeepaliveEnforcementPolicy` (`MinTime: 15s`, `PermitWithoutStream: true`) added to both `modules/login/server.go` and `modules/friends/server.go`'s `newGRPCServer` — closes a NAT/firewall silent-drop gap where subscriber streams could block in `Recv()` forever; origin `9a4a55cf`, local `de50f66c`. (29.3) WorldStartup/WorldConnect retry + login gate, folding the reviewer-caught fix wave into one local commit: `LoginClient.WorldStartup`/`FriendsClient.WorldConnect` now return `error`; `Server.retryBridgeRegistration(name, call)` retries an idempotent registration call every `bridgeRetryDelay` (5s default, tests shrink it) on a goroutine parented to `bridgesCtx` until it succeeds once; `NewWorldService`'s `startingBody` calls it instead of blocking inline, so boot no longer stalls on the ~20s gRPC min-connect timeout when login/friends are unreachable. The fold-in closes the reviewer-caught Critical: an async retry reopened a login-integrity race (a login admitted while a failed `WorldStartup` was still retrying could have its live session's `logged_in` flag wiped by the eventually-successful retry's blanket `UPDATE`), so `Server.worldStartupDone` (atomic.Bool) now gates `callPlayerLoginRPC` — while closed, logins get this branch's own byte-identical login-server-offline reject (`loginresp.OpLoginServerOffline.Opcode`, wire opcode 8), the same as the login-server-unreachable path; `worldStartupCall` opens the gate only after a nil `WorldStartup` error (strictly after the wipe); `initLoginGate` opens it immediately for standalone worlds (nil login client). `Server.bridgeWg` (`WaitGroup.Go`) tracks the retry goroutines so `Shutdown` joins them right after `bridgesCancel`. **Operator note: a persistently-failing `WorldStartup` now blocks logins by design (opcode 8) instead of silently admitting them against a not-yet-cleared session table** — this is a deliberate fidelity/safety tradeoff, not a regression. Origin `b7ff6a5f` + `750ded55`, local `835b46b4`. (29.4) friends shutdown, folding the bound-tightening follow-up into one local commit: `(*subscriptions).closeAll`/`(*worldSubscriptions).closeAll` close every registered subscriber's `done` channel and delete its entry under one lock acquisition (deleting while closing prevents `register`'s close-prior path from double-closing a later registration for the same key); `Friends.running`'s `ctx.Done` path calls both before `f.srv.shutdown()` so a standalone `--target friends` node with an attached world's subscriber stream no longer hangs forever on `GracefulStop`; `grpcServer.shutdown` races `GracefulStop` against a grace window (`defaultGracefulStopBound = 5s`, injectable via the new `grace` field) and forces `Stop()` if it elapses, as a backstop for stragglers `closeAll` cannot reach. The fold-in tightens the new e2e test's `AwaitTerminated` bound from 6s to 2s (meaningfully below the 5s backstop) so the test can actually catch a wiring regression instead of only a total revert (measured ~5.02s with just the `closeAll` calls reverted, which passed at the original 6s bound). Origin `f1955711` + `d061d9b9`, local `91937b1d`. (29.13) friends FIFO dispatch (goscape fired one independent goroutine per friends-server mutation RPC — `grpcFriendsBridge`'s Add/RemoveFriend, Add/RemoveIgnore, `SetChatMode`, Private/PublicMessage; the inline `PlayerLogin` call in `tick.go`'s `processLogins`; the inline `PlayerLogout` call in `server.go`'s `removePlayerOnTick` — so gRPC gave no cross-RPC ordering guarantee, unlike TS's single `World.friendThread.postMessage` channel, which is strictly FIFO across every player: a login/logout pair for one player could apply to the friends server as logout-then-login, leaving that player permanently "online" against the friends server's world cap until a fresh login re-registered them, and an add-then-delete pair could apply as delete-then-add): new `friendsMutationDispatcher` (`modules/world/friends_dispatcher.go`) is a SINGLE global unbounded FIFO queue served by exactly ONE worker goroutine — checked out verbatim from the origin commit, unchanged by branch shape since it has no dependency on arch-29.8 (which this branch does not carry; its Lifecycle doc comment is adapted to say so). Every friends-mutation call site named above now enqueues a closure instead of firing its own goroutine; the worker is started from `NewWorldService`'s `startingBody` (alongside the existing `retryBridgeRegistration` calls) and folded into `Server.bridgeWg`, so `Shutdown`'s existing `bridgeWg.Wait()` join covers it too. Two ordering tests (`TestFriendsDispatchPreservesLoginLogoutOrder`, `TestFriendsDispatchAddThenDeleteOrder`) pin the guarantee with a deterministic channel-gated mechanism; RED was captured locally with a throwaway variant of both tests using bare per-call goroutines (the pre-fix shape) in place of the dispatcher, which failed deterministically (`[logout login]` / `[del add]`) across repeated runs before being deleted ahead of the fix commit. Origin `adc2fecf`, local `6cc460bc`. Gates: `go build`, compile-all (`go test -run '^$' ./...`), the arc's targeted tests, `-race` on the `modules/world` targeted subset and the full `modules/friends` suite, and the full `modules/world`+`modules/friends` suites (world ~42s, no hang) all green. `gofmt -l .` (outside `.claude/`) stays empty throughout.
- USER-DIRECTED follow-up forward-port (2026-07-03): four rev-274-only operational/lifecycle/config/doc improvements ported onto rev-254 at the user's explicit request ("port it too"). **This is a deliberate DEPARTURE from goscape's normal no-forward-port policy** — unlike the arch-28/arch-29 entries above, NONE of these restore TS fidelity; they replicate later-rev engine behavior onto an older branch on request. (FFP-5) `docs(world)`: the `grpcLoginClient.PlayerForceLogout` wrapper + its `LoginClient` interface entry are documented as retained-but-uncalled (the disconnect-without-save caller was removed for TS parity; the stale "used on disconnect" comment is corrected to say it is kept to mirror the login RPC surface); comment-only, origin `707f5b12`, local `c0de0527`. (FFP-2) `fix(ondemand)`: `initOnDemand` binds the HTTP listener via `server.New` then constructs the OnDemand module; when `ondemand.New` errored the bound socket leaked until process exit (no service ever runs to `Shutdown` it). Added `server.Server.Close()` (releases the bound listener for the construct-then-abort path) and call it on the `ondemand.New` error branch; red-green pinned by `TestServerClose_ReleasesListener`; origin `086df9c2`, local `a06a0024`. (FFP-3) `feat(friends)`: `friends.graceful_shutdown_timeout` was declared but UNWIRED — `newGRPCServer` hardcoded `defaultGracefulStopBound` (5s) and the flag defaulted to 30s. Wired `newGRPCServer` to consume `Config.GracefulShutdownTimeout` (coercing a non-positive value to `defaultGracefulStopBound`), changed the flag default to `defaultGracefulStopBound` so an unset key stays 5s (**behavior-preserving: effective default shutdown timing is unchanged**), and added a `Validate` rule rejecting a non-positive `graceful_shutdown_timeout`; the one enabled-config test that built a zero-grace `Config` (`TestFriends_Running_StopsWithOpenSubscriberStream`) now sets it explicitly to `defaultGracefulStopBound`, mirroring rev-274. New `config_test.go` red-green pins the rule. rev-254 `newGRPCServer` already took `cfg Config`, so no signature/call-site threading was needed. Origin `fc19df09` (grace-wiring slice) + `c0b80a98` (Validate rule), local `0a42a4e6`. (FFP-1) `feat(observability)`: the full arch-29.6 `/healthz` (tick-liveness) + `/debug/status` feature with the boot deadline. `world.Server` gains three atomics (`lastTickNano`/`currentTickAtomic`/`lastCycleMillis`) stamped once per tick by `stampTick()` right after `s.currentTick++`, plus `HealthSnapshot()`; `modules/ondemand/health.go` defines a mirrored `ondemand.HealthSnapshot` + `RegisterHealthRoutes(mux, snap)` (import-free of `modules/world`), with the app wiring a field-by-field adapter in `initOnDemand` (coexisting with the FFP-2 `serv.Close()` change in the same function). The boot deadline treats a non-positive `LastTick` as "starting" within `healthzBootGrace` (30s), only flipping to 503 once a real timestamp goes stale (>10s) or the grace elapses with no first tick. Helm `_helpers.tpl` swaps the readiness probe from `tcpSocket` to `httpGet /healthz` on `ondemand-http` for SingleBinary/World; Management keeps `tcpSocket` on `login-grpc` (no HTTP port) — verified by `helm template` for all three modes. Origin `8fe1a457` + `eb43c055` (boot deadline) + `384701c2` (review fixup), local `a91c4795`. Gates: `go build ./...`, compile-all (`go test -run '^$' ./...`), `go test ./modules/{ondemand,world,friends}/ ./pkg/dskit/server/ ./cmd/goscape/app/`, `-race` on the touched non-world packages, and `gofmt -l .` (outside `.claude/`) empty — all green.
- Tech-debt cleanup batch (2026-07-03), ported from rev-274's three-agent debt audit of the mature post-Arc-31 tree. **Behavior-preserving** — no wire/gameplay/cache-output change; A7 (the `fire{Op,Ap}Trigger{Npc,Loc,Obj}` consolidation) was reviewed and **deliberately skipped** at the rev-274 go/no-go (~180 LOC not worth the hot-path fidelity risk). Items A1-A6 and A8-A10 landed per the rev-274 template (`chan struct{}` quit signal, dead-TODO cleanup, `pathfinder/loc` Shape→Layer pin test, `pack_configs.go` read/validate/save helper extraction, `server.go` god-file split, `min`/`max` builtins, `pkg/script` resolve/push helpers, `invButtonHandler` cheat factory, `math/rand/v2`+`any`+`for range` sweep). **rev-254 divergences**: A5 keeps 2 more funcs in `server.go` than rev-274 (this branch has no `Listen()`/`startWorldEventsSubscriber()` to split out); A4's `validatePackNamesAgainstCfgs` predates the `transmitted bool` gate (one fewer helper arg than rev-274) and still keeps `varbit` in the transmitted set. Merge tip `7cf3e702`. NOT pushed. Cross-ref rev-274 `1953b743` for the full 5-branch record.
- ✅ **DB-2 federation RETIRED (2026-07-06)** — login+friends now share one central database (`pkg/gamedb`, spec `2026-07-05-central-db-consolidation-postgres`), ported to rev-254 per the phase audit (`.superpowers/sdd/audit-port254.md`, `2e3bcf43` pin). **Audit finding:** rev-254's own TS pin (`2e3bcf43`) is behaviorally identical to rev-274's pin (`dee467c8`) across the entire friends surface (`git diff 2e3bcf43 dee467c8` on `FriendServer.ts`/`FriendServerRepository.ts`/`friend.ts`/both `schema.prisma` files shows only cosmetic Prettier rewrap + an `Environment` object restructuring, zero logic delta) — so rev-274's Go contract could be ported essentially verbatim, only re-citing line numbers at the 2e3bcf43 pin. `pkg/gamedb` (both sqlite and postgres backends, pgx/v5) copied byte-identical from rev-274 (source tip `311e0502`; landed here as `65aae0ed`); friends `repository.go` re-keyed from username37-keyed federated tables to account-id-keyed `friendlist`/`ignorelist`/`private_chat`/`public_chat` against the shared `account` table (FK + ON DELETE CASCADE, a goscape-only extension beyond prisma's un-related schema). Restored TS behaviors previously excepted: addFriend/addIgnore owner+target account resolution (`FriendServerRepository.ts:210-294` @2e3bcf43) incl. members-aware 200/100 cap with no profile filter on the count, PM account-existence check (`FriendServer.ts:256-285` @2e3bcf43 — the `NAI-S4A-D-FED-NO-ACCOUNT-EXISTENCE-CHECK` exception blocks in `repository.go`/`handler.go` are deleted, `LogPrivateMessage` now returns `errAccountMissing` and `PrivateMessage` maps it to a silent success matching TS's throw-inside-catch drop), TS-shape `public_chat` `{session_uuid, timestamp, coord, message}` (the federation-era extra `profile`/`world` columns are dropped; a central DB can recover them via a session join if ever needed, unlike the old federated friends DB). Deliberate non-FKs (matching TS): `ignorelist.value` (TS allows ignoring nonexistent players — no target-account resolution in `addIgnore`), `public_chat.session_uuid` (headless uuids have no session row). Deletes the federation-era `modules/friends/migrations/`, `db.go`, `db_test.go` (two public_chat re-key generations, `public_chat_legacy_225`/`public_chat_legacy_244`); `friends.go`/`config.go` retire `SQLiteDSN` in favor of `gamedb.Config` (independent-clients model, same posture as login); `cmd/goscape/app/modules.go`'s `initFriends` now passes `g.cfg.Database`. Supersedes the "friends `public_chat` account_id resolution NO-LANDING-SITE" and "FriendServerRepository internals NO-OP/N-A" rows above, both marked retired with a pointer back to this entry. Full audit/citation/test record: `.superpowers/sdd/port254-t2-report.md`..`t5-report.md`. Phase commits `65aae0ed`..`43ea9765` (direct on rev-254, no merge). NOT pushed.
- Reboot process-exit parity fix (2026-07-06, ORIGIN): a production `::reboot`/`::slowreboot` that finishes draining players must stop the WHOLE process — TS `World.ts` `processShutdown` calls `process.exit(0)` once the last player drains (`src/engine/World.ts:1208` @2e3bcf43), so an external supervisor restarts a fully-dead node. goscape's world runFn returned `nil` on the graceful-exit branch, so the world service Terminated cleanly and left its sibling modules (ondemand/login/friends) running half-alive under `target: all`. Fixed by returning `modules.ErrStopProcess` from the graceful branch (`modules/world/world.go`) — the service manager's Failure listener turns that sentinel into `sm.StopAsync()`, tearing down every module, while `app.go`'s `failedServicesError` still classifies it as a requested shutdown (process exits 0, logged info not error). `TestWorldServiceFnsRunFnOutcomes` updated from the old nil contract to pin the `ErrStopProcess` return (RED→GREEN). Landed on rev-254 in this commit; ported to all five rev branches.
- ➕ **Hiscores read API (2026-08-20, EXTENSION — no Engine-TS counterpart)**: `modules/hiscore` serves public JSON player-card and leaderboard reads over the `hiscore`/`hiscore_large` central-database tables that `modules/login`'s `updateHiscores` already populates on logout (write path ported at Arc 29, `login-server-9`/`gap-db-datastruct-9`, above). Engine-TS has no hiscore serving endpoint at any pinned revision — `hiscore` appears there only in `db/types.ts`, the Prisma migrations, and the write path in `LoginServer.ts`. This subsystem therefore sits **outside the TS fidelity ledger**; a future parity audit must not report it as an unclosed divergence. The only central-database change is additive indexes — migration `000004_hiscore_indexes` (both `sqlite` and `postgres` variants under `pkg/gamedb/migrations/`), which adds `idx_hiscore_rank`/`idx_hiscore_account`/`idx_hiscore_large_rank`/`idx_hiscore_large_account` and changes no existing table shape. `modules/login`'s write path is untouched. Backported from `rev-274` (`a4283a78`).
- 🔒 **SEC1 — Security hardening batch 1 (2026-08-22, back-ported from `rev-274` @ `33f61845`)**: five independent hardening fixes, user-directed onto this branch (spec `docs/superpowers/specs/2026-08-22-sec1-hardening-design.md`, which carries a rev-254 back-port note). M-7: `GameLogin.LogValue` redacts the password, ISAAC seed and CRC table from the login debug log (`72f7b809`). M-2: outbound socket writes move off the tick goroutine onto a per-client `outboundWriter` (`modules/world/outbound.go`) behind a bounded async queue, with `client.closeConn()` draining-then-closing at every `*client` teardown site; caps are 512 frames / 1 MiB, and a zero/negative `tcp_server_write_timeout` stamps a 250 ms `fallbackDrainTimeout` on the socket instead of leaving per-write deadlines off indefinitely (`868e34e3`, DEVIATION SEC1-D3). **rev-254 adaptation**: this branch's OnDemand is the pre-274 scheduler (`OnDemand.ts` @9aadcec4 — a fixed 50 ms cycle draining urgent/extra/ingame in full), not 274's round-robin pump, so the producer-side yield lives in `onDemand.cycle`/`sendBatch`: a client whose `odClient.backlogged()` reports the soft high-water mark has its pending requests put back at the HEAD of their queue, untouched and in order, and retried on the next 50 ms cycle — the connection is never closed for being slow. 274's `serveClient` slice-end, `pump` per-pass progress bound and `backloggedRetryInterval` have no counterpart here (the ticker already re-arms); the check is per request, never mid-file, because this scheduler has no resumable per-client cursor. M-1: `[login]`/`[logout]` trigger calls are wrapped in `recoverPlayer` so one player's panic force-disconnects only that player instead of taking the world down (DEVIATION SEC1-D1); `runTickLoop` recovers an unhandled tick panic, fires `crashSaveAll` — a per-player best-effort `crashSavePlayers` pass that waits on `waitForSaveFlush` — then re-panics so process-exit crash semantics for supervisors are unchanged (DEVIATION SEC1-D2). The per-tick body was extracted from `runTickLoopWithRate` into `tickOnce()` behind the `tickBodyFn` seam (quit-select, nextTick/drift/sleep and cycle-stat bookkeeping stay in the loop); the `[logout]` call site moved from `s.runScript` to the existing `s.runScriptFn` seam, same default behaviour (`0a0e3648`). M-8: the account portal gained anonymous-form double-submit CSRF (`goscape_csrf` cookie) via `publicForm()`, session rotation on login, security headers, server read/write/idle timeouts, a 64 KiB request body cap surfaced as 413, and a two-step `/verify-email` flow (GET renders a confirmation form, POST consumes the token) (`6db8dadd`). M-12: a new `ondemand.debug_status_enabled` config flag (default `false`) gates `/debug/status`; the Helm chart now sets non-root/read-only-rootfs/drop-ALL `securityContext`s, `fsGroupChangePolicy`, pod `resources`, a `tcpSocket` liveness probe, an unmounted service-account token, an `emptyDir` for `/tmp`, `image.digest` support, unconditional `--config.expand-env=true`, and scopes the hiscore `NetworkPolicy` to the Kong proxy when `createGatewayConfig` is set; the Dockerfile pins `USER 65532:65532` (`2533c588`). [BACKPORT-FIDELITY] Every item is goscape-own process/deployment hardening with no Engine-TS counterpart to diverge from; wire format is untouched, and the batch transplants no later-revision TS behaviour onto this pin. **User-owed smoke test**: confirm a throttled OnDemand cache download is served slowly rather than disconnected under the new queue.
- 🧩 **Pack ID index generation (2026-08-31, back-ported from `rev-274` @ `47eb0ed7`)**: goscape could not pack a fresh `git clone` of Content. Content gitignores fourteen `pack/*.pack` ID indexes ("since these are generated for the server only, we don't have to share them"), so a clone carries 19 of 30. goscape **read** those indexes and never wrote them: `PackFile.Register`/`Save` were ported but wired only into `pkg/unpack`, and the pack direction ported just the orphan half of TS `validateConfigPack` (as `ValidateConfigPackNames`, itself dead code until `2cfec7ea`). The failure was invisible for as long as every tree the project packed already had the indexes — a developer's prepared checkout — and it surfaced only when CI cloned Content fresh. Same shape as the T30 catches: unported, and vacuous on the content actually tested.
  ...
  - **Verified on this branch**: a bare clone of Content `caee3f2e` — the revision `rev-254` pins — goes 19 → 30 indexes and packs successfully with **no Engine-TS involved**, emitting a 41 MB tree and two benign `missing model` warnings. `script.dat` is 5,026,103 bytes. The index-for-index byte-parity comparison against Engine-TS was run once, on `rev-274` — this branch takes the same code path and the same seven tests.
- 🛡 **Pack staleness guard (2026-08-31, back-ported from `rev-274` @ `82772f13`)**: incremental packing was keyed on SOURCE mtimes only, so it could not see that the PACKER's byte layout had changed. That was observed on `rev-274`, where an engine sync moved npc opcodes and a plain `make pack` afterwards left **2,909 artifacts unrebuilt**, including a `hunt.dat` carrying pre-sync opcodes and `mapview/worldmap.jag`; the server refused to boot on the loud path and would have served wrong bytes on the quiet one. Nothing about the hazard is specific to that sync or that branch — freshness is keyed on source mtimes here too — so the guard is back-ported rather than left where it was found. Spec: `docs/superpowers/specs/2026-08-31-pack-staleness-guard-design.md`.
  ...
  - **Verified on this branch** against a pack of Content `caee3f2e`: an unstamped tree (one built by the pre-guard packer) forces a full rebuild with the INFO line, then carries `format=1`. With the format unchanged, a second consecutive pack rewrites only **41 of 2,803** artifacts — the unconditional stages — so incrementality is intact. Bumping the version rebuilds the deliberately back-dated artifacts (2 stale → 0) and advances the stamp.
