...
- 🔒 **SEC1 — Security hardening batch 1 (2026-08-22)**: five independent hardening fixes landed together on `sec1-hardening` (plan `docs/superpowers/plans/2026-08-22-sec1-hardening.md`, spec `docs/superpowers/specs/2026-08-22-sec1-hardening-design.md`). M-7: `GameLogin.LogValue` redacts the password, ISAAC seed and CRC table from the login debug log (07efa157). M-1: `[login]`/`[logout]` trigger calls are wrapped in `recoverPlayer` so one player's panic force-disconnects only that player instead of taking the world down (a2d2e35f, DEVIATION SEC1-D1); `runTickLoop` recovers an unhandled tick panic, fires `crashSaveAll` — a per-player best-effort `crashSavePlayers` pass that waits on `waitForSaveFlush` — then re-panics so process-exit crash semantics for supervisors are unchanged (02904d72, 8232ab98, DEVIATION SEC1-D2). M-2: outbound socket writes move off the tick goroutine onto a per-client `outboundWriter` (`modules/world/outbound.go`) behind a bounded async queue, with `client.closeConn()` draining-then-closing everywhere; caps are 512 frames / 1 MiB, and a zero/negative `tcp_server_write_timeout` stamps a 250 ms `fallbackDrainTimeout` on the socket instead of leaving per-write deadlines off indefinitely (e4c6178c, 8bc83236, a5500054, DEVIATION SEC1-D3 — see spec for the OnDemand-pump rationale behind the 512/1 MiB caps and the 250 ms fallback; **user-owed smoke test**: confirm a throttled OnDemand cache download is not disconnected under the new queue); because a ~506-byte OnDemand chunk occupies a whole slot, the OnDemand pump no longer relies on those caps at all — `serveClient` consults `outboundWriter.Backlogged()` (soft high-water: half of each cap) and ends the client's slice with its queue and pending requests intact, retrying on a later pump pass, so a slow downloader is served slowly instead of disconnected (c75f2269). M-8: the account portal gained anonymous-form double-submit CSRF (`goscape_csrf` cookie), session rotation on login, security headers, server read/write/idle timeouts, a 64 KiB request body cap surfaced to the client as 413, and a two-step `/verify-email` flow (GET renders a confirmation form, POST consumes the token) so email verification is no longer a state-changing GET (1bb109ad, 321c7e08, eeb69a70, d5d4a6a5). M-12: a new `ondemand.debug_status_enabled` config flag (default `false`) gates the `/debug/status` endpoint (92f518c4); the Helm chart now sets non-root/read-only-rootfs/drop-ALL `securityContext`s, pod `resources`, a `tcpSocket` liveness probe, an unmounted service-account token, an `emptyDir` for `/tmp`, `image.digest` support, unconditional `--config.expand-env=true`, and scopes the hiscore `NetworkPolicy` to the Kong proxy when `createGatewayConfig` is set; the Dockerfile pins `USER 65532:65532` (image base stays `distroless/static:debug-nonroot`) (d79f5f81). [BACKPORT-FIDELITY] M-1 and M-2 are goscape-own process-hardening changes with no Engine-TS counterpart to diverge from and are off the backport list per `no_forward_port_deviations`; M-7, M-8 and M-12 touch goscape-only surfaces (log redaction, account portal, Helm chart) and may be backported to any rev branch that carries the account portal / Helm chart.
