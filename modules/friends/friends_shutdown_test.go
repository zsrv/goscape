...
//
// arch-29.3-fix-wave (rev-274 d061d9b9): the AwaitTerminated bound is
// tightened to 2s, meaningfully below defaultGracefulStopBound (5s), so
// this test can only pass if the closeAll wiring in Friends.running
// actually runs — a bound at or above the backstop would also pass with
// only the GracefulStop grace-window fallback intact.
func TestFriends_Running_StopsWithOpenSubscriberStream(t *testing.T) {
	...
	// Pre-migrate a real on-disk DB (the module opens its OWN pool in
	// starting() via gamedb.Open; schema migration is normally the
	// database module's job, which this standalone test stands in for).
	dbCfg := testGamedbConfig(t)
	pre, err := gamedb.Open(dbCfg, noopLogger())
	if err != nil {
		t.Fatalf("pre-open: %v", err)
	}
	if err := pre.Migrate(t.Context()); err != nil {
		t.Fatalf("pre-migrate: %v", err)
	}
	if err := pre.Close(); err != nil {
		t.Fatalf("pre-close: %v", err)
	}
	f, err := New(cfg, dbCfg, noopLogger())
	if err != nil {
		t.Fatalf("New: %v", err)
	}
