...
// errAccountMissing is ResolvePrivateMessageEndpoints' sentinel for "an
// endpoint account does not exist". Mirrors TS FriendServer.ts:270-271,
// where executeTakeFirstOrThrow throws and the outer catch drops the
// PM. The handler maps it to a silent drop (no delivery, success RPC).
...
// (FriendServerRepository.ts:231). ignoreListLimit stays a flat 100
// (FriendServerRepository.ts:268).
const (
	friendListLimit        = 100
...
// (TS FriendServerRepository.ts:222-223/258; FriendServer.ts:270-271).
...
// (FriendServerRepository.ts:210-247): either account missing → silent
// no-op; duplicate → no-op; cap members ? 200 : 100. TS counts the cap
// across ALL profiles (no profile filter on the count query,
// FriendServerRepository.ts:230) — quirk mirrored. The whole
...
// FriendServerRepository.ts:197-207). No-op when either username has
...
// asc (FriendServerRepository.ts:357-371).
...
// AddIgnore mirrors TS addIgnore (FriendServerRepository.ts:249-294):
...
// quirk, :266). ON CONFLICT DO NOTHING matches TS's sqlite branch
// (:282-283) and is valid on both goscape backends.
...
// (TS FriendServerRepository.deleteIgnore, :308-316: where profile,
...
// (TS loadIgnores, :373-386: join owner account, select i.value,
...
// (FriendServerRepository.ts:177-181); goscape keeps its established
...
// Mirrors TS playerStaff membership (FriendServerRepository.ts:83-84 @2e3bcf43).
...
// (TS playerIgnores[other].includes(viewer), FriendServerRepository.ts:340).
