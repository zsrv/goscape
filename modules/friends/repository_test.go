...
// endpoints must exist under the re-keyed TS 274 schema (AddFriend now
...
// order) is caught. Re-keyed for the TS 274 account-id schema: friendlist rows
...
// Only the owner needs an account under TS 274 semantics — AddIgnore never
// resolves the target.
...
// an OFF player. TS FriendServerRepository.ts:83-84 @2e3bcf43 registers a
...
// TestIsVisibleTo_StaffGateThreshold pins the fidelity fix: TS gates
// playerStaff membership at staffLvl > 1, not > 0
// (FriendServerRepository.ts:83-84 @2e3bcf43). A viewer registered with
// staffLvl 1 is NOT staff and must not bypass an OFF player; staffLvl 2
// IS staff and bypasses.
func TestIsVisibleTo_StaffGateThreshold(t *testing.T) {
	r, _ := newTestRepo(t)
	r.InitializeWorld(1, 10)
	r.Register(1, 0xAAAA, 2, 0) // other: privateChat OFF
	r.Register(1, 0xBBBB, 0, 1) // viewer: staffLvl 1 — NOT staff at 2e3bcf43
	visible, err := r.IsVisibleTo(t.Context(), 0xBBBB, 0xAAAA)
	if err != nil {
		t.Fatalf("IsVisibleTo: %v", err)
	}
...
// (no dedupe). Both endpoints must exist under the TS 274 re-key —
...
// pre-274 schema, which stored raw username37 ints with no encoding).
...
// list (TS addIgnore, FriendServerRepository.ts:266), counted across ALL
// profiles with no profile filter (TS quirk).
...
// --- TS 274 re-key: new-behavior tests (account resolution, members-aware
// cap, TS-exact private/public chat row shapes) ---
...
// members-aware friend cap, FriendServerRepository.ts:231).
...
func TestAddFriend_MissingTarget_NoInsert(t *testing.T) {
	// TS FriendServerRepository.ts:225-228: `if (!account || !friendAccount) return`.
	r, db := newTestRepo(t)
	seedAccount(t, db, 1)
...
func TestAddFriend_NonMemberCap100(t *testing.T) {
	// TS: limit = account.members ? 200 : 100 (FriendServerRepository.ts:231).
	r, db := newTestRepo(t)
	seedAccount(t, db, 1)
	targets := distinctUsername37s(2, 101) // 100 friends + 1 over-cap probe
	for _, target := range targets {
		seedAccount(t, db, target)
	}
	for _, target := range targets[:100] {
		if err := r.AddFriend(t.Context(), 1, target); err != nil {
			t.Fatalf("AddFriend #%d: %v", target, err)
		}
	}
	if err := r.AddFriend(t.Context(), 1, targets[100]); err != nil {
...
func TestAddFriend_MemberCap200(t *testing.T) {
	...
	targets := distinctUsername37s(2, 102) // 102 friends — over the non-member cap
	for _, target := range targets {
		seedAccount(t, db, target)
	}
	for _, target := range targets {
...
func TestAddIgnore_NonexistentTarget_Succeeds(t *testing.T) {
	...
	// ignoring a player who doesn't exist works. 1000 (not 999 — a multiple
	// of 37, which jstring.FromBase37 maps to the sentinel "invalid_name")
	// is used so the round-trip through the ignorelist.value column is
	// exact.
	r, db := newTestRepo(t)
	seedAccount(t, db, 1)
...
func TestAddIgnore_MissingOwner_NoOp(t *testing.T) {
	// TS resolves the OWNER and returns on miss (FriendServerRepository.ts:258-262).
	r, _ := newTestRepo(t)
