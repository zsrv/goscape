...
// world.node_profile on every PrivateMessageRequest. This friends server
// holds no profile of its own — every RPC routes by req.Profile — so the
// request is the only source the event can read.
