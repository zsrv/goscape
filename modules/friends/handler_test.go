...
// keyed by the RESOLVED account ids (TS FriendServer.ts:275-276 @3c16994c
...
// (FriendServer.ts:275-276 @3c16994c: either endpoint unresolvable → no
...
func TestPrivateMessage_NoSubscription(t *testing.T) {
	h := newTestHandler(t)
	// Both endpoints need accounts under the TS 3c16994c re-key —
	// ResolvePrivateMessageEndpoints resolves from/to against the central database.
	seedAccount(t, h.repos.db, 0xAAAA)
	seedAccount(t, h.repos.db, 0xBBBB)
...
func TestHandler_PrivateMessage_ResolveErrorBlocksSend(t *testing.T) {
	...
	r.InitializeWorld(1, 100)
	r.Register(1, 200, 0, 0)
	// Seed both endpoints so ResolvePrivateMessageEndpoints would
	// otherwise succeed. db.Close() below fails the very first query it
	// issues — resolving `from`'s account id; the point of this test is
	// that a non-errAccountMissing failure anywhere in resolution still
	...
	// Force a resolution failure: close the underlying *gamedb.DB.
...
// TestPrivateMessage_MissingTarget_DroppedSilently pins the TS 3c16994c
// re-key's restored account-existence check (TS FriendServer.ts:275-276
// @3c16994c: executeTakeFirstOrThrow on either endpoint throws, the
// outer catch swallows it — no record, no delivery, socket stays
// healthy). goscape: the RPC succeeds, no PrivateChatEvent is emitted
// (chat is Kafka-only, spec
// docs/superpowers/specs/2026-07-07-chat-kafka-only-design.md), and
// nothing is delivered to the target's stream, even though the target is
...
func TestPrivateMessage_MissingTarget_DroppedSilently(t *testing.T) {
	...
	if n := len(cap.snapshot()); n != 0 {
		t.Errorf("emitted %d envelopes, want 0 (silent drop leaves no record)", n)
	}
...
// TestPrivateMessage_BothExist_EmittedAndDelivered dual-pins the
// presence side of TestPrivateMessage_MissingTarget_DroppedSilently:
// when both endpoints resolve, the PM emits exactly one PrivateChatEvent
// (chat is Kafka-only, spec
// docs/superpowers/specs/2026-07-07-chat-kafka-only-design.md) and is
// delivered to the target's stream.
func TestPrivateMessage_BothExist_EmittedAndDelivered(t *testing.T) {
	cap := &captureEmitter{}
	telemetry.Set(cap)
	t.Cleanup(telemetry.Reset)
	...
	envs := cap.snapshot()
	if len(envs) != 1 || envs[0].GetPrivateChat() == nil {
		t.Fatalf("PrivateChatEvent emissions: got %d, want 1", len(envs))
	}
...
// (public_chat audit handler tests retired — chat is Kafka-only, spec
// docs/superpowers/specs/2026-07-07-chat-kafka-only-design.md; the
// PublicMessage RPC no longer exists.)
