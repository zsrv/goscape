...
func (h *handler) PlayerLogin(ctx context.Context, req *loginpb.PlayerLoginRequest) (*loginpb.PlayerLoginResponse, error) {
	...
	// here.) Runs identically for BOTH auth modes: it is an account-keyed
	// anti-bruteforce guard independent of how the credential itself was
	// verified, so account mode (whose account row is loaded above) gets
	// the same protection as local mode.
	...
	// 4. Password check (local mode only — account mode already verified
	// credentials via VerifyGameLogin above).
	if h.cfg.AuthMode != AuthModeAccount {
		// A row whose password is the portal's sentinel game password
		// cannot possibly be a bcrypt hash (always exactly 60 bytes) —
		// this happens when a portal-created character logs in after a
		// rollback to auth_mode=local. bcrypt.CompareHashAndPassword on a
		// too-short hash returns an unexported hash-too-short error, NOT
		// ErrMismatchedHashAndPassword, so the generic error branch below
		// would surface codes.Internal instead of INVALID_CREDENTIALS.
