...
// LastTick/CurrentTick/LastCycleMillis from stampTick, and PlayersOnline from
// playerList.count, which is an atomic.Int32 mutated only at the two guarded
// write sites (playerList.set/remove, called from addPlayer /
// removePlayerInternal; the latter's slot-identity check makes the decrement
// idempotent under double-removal). Safe to call from any goroutine, with no
// locks — HealthSnapshot runs on the ondemand HTTP handlers, never the tick
// goroutine.
