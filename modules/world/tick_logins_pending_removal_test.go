...
// processLogins has not registered it yet (slot still -1).
...
// World.removePlayer guard (`if (player.slot === -1) return;`,
// World.ts:1602-1605 @1d25566c) for a Player that is still only queued in
// s.newPlayers.
//
...
// removePlayerInternal's slot-identity guard made the cleanup a no-op but
