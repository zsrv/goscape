...
// ref274CacheDir is defined in testdata_path_test.go — it resolves the
// Server274-ref reference cache (274 component layout) and skips when
// the reference checkout is unavailable. The repo's own data/pack is not
// used here because it may be a stale pack from another revision that
// LoadComponentTypes can no longer decode.

func TestReload_FreshLoad_PopulatesAllRegistries(t *testing.T) {
	s := newTestServerWithCachePath(t, ref274CacheDir(t))
...
func TestReload_ScriptCount_NodeDebug_FailureBroadcast_PartialCache(t *testing.T) {
	cacheDir := copyCacheExcept(t, ref274CacheDir(t), "server/script.dat", "server/script.idx")
...
func TestReload_MidPipelineLoaderError_LeavesHalfSwapped_SkipPin(t *testing.T) {
	...
	cacheDir := copyCacheExcept(t, ref274CacheDir(t), "server/dbrow.dat")
