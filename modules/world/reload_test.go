...
// ref244CacheDir is defined in testdata_path_test.go — it resolves the
// Server244-ref reference cache (rev-244 component layout) and skips when
...
// cache (e.g. 245.2-format) that LoadComponentTypes can no longer decode.

func TestReload_FreshLoad_PopulatesAllRegistries(t *testing.T) {
	s := newTestServerWithCachePath(t, ref244CacheDir(t))
...
func TestReload_ScriptCount_NodeDebug_FailureBroadcast_PartialCache(t *testing.T) {
	cacheDir := copyCacheExcept(t, ref244CacheDir(t), "server/script.dat", "server/script.idx")
...
func TestReload_MidPipelineLoaderError_LeavesHalfSwapped_SkipPin(t *testing.T) {
	...
	cacheDir := copyCacheExcept(t, ref244CacheDir(t), "server/dbrow.dat")
