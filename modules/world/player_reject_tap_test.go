...
func TestRejectedInboundTap_UnknownOpcode(t *testing.T) {
	...
	// Opcode 3 is not in the 274 Ops table.
...
// the last one, was deleted at 274 — so the oversized branch is unreachable
// from a real frame on THIS branch and the decoder is driven directly from the
// state a 2-byte-length op leaves it in: opcode read, length prefix pending.
// The branch is live on the older revisions this change is ported to.
