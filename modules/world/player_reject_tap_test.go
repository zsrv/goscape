...
// opcEventTracking is this revision's ONLY 2-byte-length opcode
// (pkg/io/protocol/game/client/prot.go — set(81, "EVENT_TRACKING", -2, r)), so
// it is the only opcode that can reach readPacket's oversized-frame rejection
// from a real frame. Named here because this revision's packet table exposes
// no per-opcode constants of its own; the preflight below pins it to the
// table rather than trusting the number.
const opcEventTracking = 81
...
// TestRejectedInboundTap_OversizedFrame covers the other rejection, driving
// the decoder directly from the state a 2-byte-length op leaves it in: opcode
// read, length prefix pending. TestRejectedInboundTap_OversizedFrameRealFrame
// below reaches the same branch from a real EVENT_TRACKING frame, which this
// revision's packet table still has.
func TestRejectedInboundTap_OversizedFrame(t *testing.T) {
	p, _ := newTestPlayer(t)
	tp := attachTap(p.client)

	p.client.opcode = opcEventTracking
	p.client.waiting = -2
	...
	if got[0].opcode != opcEventTracking {
		t.Errorf("opcode = %d, want %d", got[0].opcode, opcEventTracking)
	}
	if want := payload[:maxRejectedTapBytes]; !bytes.Equal(got[0].payload, want) {
		t.Errorf("payload = %v, want the first %d buffered bytes %v",
			got[0].payload, maxRejectedTapBytes, want)
...
// TestRejectedInboundTap_OversizedFrameRealFrame drives the same rejection the
// way a client actually reaches it on this revision: a genuine EVENT_TRACKING
// frame whose 2-byte length prefix declares more than the 1600-byte cap. The
// opcode therefore comes off the wire ISAAC-encrypted and the decoder reads
// the length prefix itself, rather than being handed that state.
func TestRejectedInboundTap_OversizedFrameRealFrame(t *testing.T) {
	if op := gameclient.Ops[opcEventTracking]; op.PayloadSize != -2 {
		t.Fatalf("preflight: Ops[%d] (%q).PayloadSize = %d, want -2 (a 2-byte length prefix)",
			opcEventTracking, op.Name, op.PayloadSize)
	}
	...
	payload := rampBytes(100)
	p.client.in.Write([]byte{encryptOpcode(enc, opcEventTracking)})
	p.client.in.Write([]byte{0x07, 0xD0}) // declared length 2000 > the 1600 cap
	p.client.in.Write(payload)
	...
	if got[0].opcode != opcEventTracking {
		t.Errorf("opcode = %d, want the DECRYPTED %d", got[0].opcode, opcEventTracking)
	}
	if want := payload[:maxRejectedTapBytes]; !bytes.Equal(got[0].payload, want) {
		t.Errorf("payload = %v, want the first %d buffered bytes %v",
			got[0].payload, maxRejectedTapBytes, want)
