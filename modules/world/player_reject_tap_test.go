...
// read, length prefix pending. That is the only way to reach this branch on
// rev-274, whose packet table has no 2-byte-length opcode at all
// (EVENT_TRACKING, the last one, was deleted there). This revision still has
// one, so TestRejectedInboundTap_OversizedFrameFromWire drives the same
// rejection off a real frame as well.
func TestRejectedInboundTap_OversizedFrame(t *testing.T) {
	p, _ := newTestPlayer(t)
	tp := attachTap(p.client)

	p.client.opcode = int(gameclient.OpcEventTracking)
	p.client.waiting = -2
	...
	if got[0].opcode != gameclient.OpcEventTracking {
		t.Errorf("opcode = %d, want %d", got[0].opcode, gameclient.OpcEventTracking)
	}
	if want := payload[:maxRejectedTapBytes]; !bytes.Equal(got[0].payload, want) {
		t.Errorf("payload = %v, want the first %d buffered bytes %v",
			got[0].payload, maxRejectedTapBytes, want)
...
// TestRejectedInboundTap_OversizedFrameFromWire drives the oversized-frame
// rejection the way a real client reaches it on this revision: EVENT_TRACKING
// is the one opcode whose payload size is -2, so a genuine encrypted frame
// declaring more than the 1600-byte cap walks the whole decode path — opcode
// lookup, 2-byte length prefix, rejection — instead of being staged from a
// half-decoded client state.
func TestRejectedInboundTap_OversizedFrameFromWire(t *testing.T) {
	...
	buf = append(buf, 0x07, 0xD0) // declared length 2000 > the 1600 cap
