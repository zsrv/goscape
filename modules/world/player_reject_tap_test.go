...
// read, length prefix pending.
//
// rev-274's packet table has no 2-byte-length opcode at all — EVENT_TRACKING,
// the last one, was deleted at 274 — so on that branch this is the only way in.
// It IS reachable from a real frame here, which
// TestRejectedInboundTap_OversizedFrameFromWire covers as well.
...
// TestRejectedInboundTap_OversizedFrameFromWire drives the same rejection the
// way a real client reaches it: this revision still has a 2-byte-length opcode
// (EVENT_TRACKING), so the whole frame — encrypted opcode, oversized length
// prefix, payload — can go in off the wire and readPacket decodes its own way
// to the rejection.
