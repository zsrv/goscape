...
func newTestPlayerForLoadSave(t *testing.T) (*Player, *objtype.InvTypeConfigs) {
	...
	p := &Player{
		...
		// (initPlayerVarps; TS Player.ts:418) and LoadSave only overlays
...
// extra slots keep their initPlayerVarps seeds. TS contract: Player.ts:418
// allocates vars registry-sized with per-type seeds (:424-432);
