...
// False for a Player that is still only queued in s.newPlayers (slot -1,
// TS Player.ts: "slot = -1 until login assigns one") and for one already
// removed. Callers must hold playersMu.
func (s *Server) registeredLocked(p *Player) bool {
	return p.slot >= 1 && p.slot < len(s.players.entities) && s.players.get(p.slot) == p
}
...
func (s *Server) removePlayerOnTick(p *Player) {
	// TS World.removePlayer (World.ts:1602-1605 @1d25566c) opens with
