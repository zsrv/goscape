...
// Lock-free read — count is atomic (playersMu guards the slot array).
...
// registeredLocked reports whether p currently occupies its own pid in
// s.players, i.e. whether processLogins has registered it into the world.
// False for a Player that is still only queued in s.newPlayers (pid stays
// -1 until login assigns one) and for one already removed. Callers must
// hold playersMu.
func (s *Server) registeredLocked(p *Player) bool {
	return p.pid >= 1 && p.pid < len(s.players.entities) && s.players.get(p.pid) == p
}
...
func (s *Server) removePlayerOnTick(p *Player) {
	// TS World.removePlayer (World.ts:1629-1632 @3c16994c) opens with
	// `if (player.pid === -1) return;`: a Player that sendLoginOK
	// published to s.newPlayers but processLogins has not registered yet
	// has no pid, so removal is a complete no-op — no save, no logout
