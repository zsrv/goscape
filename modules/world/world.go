...
func worldServiceFns(
	...
	starting := func(ctx context.Context) error {
		if err := startingBody(ctx); err != nil {
			...
			// failure (Listen, ...) would leak both gRPC connections
			// without this. No double-close is possible: the state machine
			// makes the two paths exclusive — starting failure means
			// stopping never runs (closes happen HERE); starting success
			// means this branch never ran (closes happen in stopping).
			if lcClose != nil {
	...
	runFn := func(ctx context.Context) error {
		...
		case err := <-serverDone:
			...
			if gracefulExit() {
				...
				// last player has drained (World.ts:1242 @9aadcec4), killing
