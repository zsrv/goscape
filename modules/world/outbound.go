...
// frames are large, trips the byte cap (1 MiB) first. The OnDemand cycle is
...
// That is why the OnDemand cycle no longer relies on the caps at all — it
...
// now consults Backlogged and defers a backlogged client's requests to a
// later cycle instead. Backpressure, not the hard cap, governs OnDemand;
// the caps remain the last-resort bound for a peer that has stopped
// reading altogether.
//
// DEVIATION SEC1-D3 (continued): the OnDemand cycle yields at the soft
// high-water mark and retries on the next 50ms cycle, so a slow downloader
// is served slowly instead of disconnected.
const (
...
// tick goroutine post-login, the OnDemand cycle in state 2 — each already
...
// the hard cap. Only optional producers consult it (the OnDemand cycle);
