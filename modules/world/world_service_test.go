...
// pkg/dskit/services/basic_service.go:44), and the bridge client closes
// used to live only in stopping — so a startingBody failure (Listen, ...)
// leaked both gRPC connections. The starting closure must now close both
// clients exactly once before propagating the error.
