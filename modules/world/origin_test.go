...
func TestPacketReceivedEnvelopeCarriesOrigin(t *testing.T) {
	...
	// MOVE_GAMECLICK: opcode 6 on this revision, 1-byte length prefix.
...
// TestMouseMoveEnvelopeCarriesOrigin covers handler_events.go.
func TestMouseMoveEnvelopeCarriesOrigin(t *testing.T) {
	...
	if err := handleEventMouseMove(p, []byte{0x01, 0x02}); err != nil {
		t.Fatalf("handleEventMouseMove: %v", err)
	}
