...
// opcMoveGameClick is this revision's MOVE_GAMECLICK client opcode. This
// branch exports no per-opcode constants — gameclient.Ops is the table — so
// the value is pinned here and cross-checked against that table before use.
const opcMoveGameClick = 181
...
func TestPacketReceivedEnvelopeCarriesOrigin(t *testing.T) {
	if got := gameclient.Ops[opcMoveGameClick].Name; got != "MOVE_GAMECLICK" {
		t.Fatalf("gameclient.Ops[%d] = %q, want MOVE_GAMECLICK", opcMoveGameClick, got)
	}
	s, cap := newOriginServer(t)
	...
	// MOVE_GAMECLICK: opcode 181 on this revision (gameclient.Ops[181]),
	// 1-byte length prefix. Payload: ctrlHeld(1) + startX G2 + startZ G2.
	payload := []byte{0, 0x0C, 0xA4, 0x0C, 0x8B}
	buf := []byte{encryptOpcode(enc, opcMoveGameClick), byte(len(payload))}
...
// windowed EVENT_TRACKING handler that emits the MouseMoveEvent on this
// revision. The gates are the handler's own happy path (see
// TestHandleEventTrackingHappyPathRecords): a non-empty payload under
// inputTrackingMaxBlobBytes, an open tracking window, submitInput set, and
// recordedBlobsSizeTotal under the per-session cap.
func TestMouseMoveEnvelopeCarriesOrigin(t *testing.T) {
	...
	if len(p.input.recordedBlobs) != 1 {
		t.Fatalf("recordedBlobs: got %d, want 1 (handler gates did not open)", len(p.input.recordedBlobs))
	}
