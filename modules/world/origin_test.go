...
// EVENT_TRACKING (opcode 217) site that emits the MouseMoveEvent envelope on
// this revision. The gates mirror TestHandleEventTrackingHappyPathRecords: an
// active tracking window, submitInput on, and room under the per-session
// byte cap — the emit sits past all three, inside the handler's existing
// guard chain.
func TestMouseMoveEnvelopeCarriesOrigin(t *testing.T) {
	...
	if err := handleEventTracking(p, []byte{0x01, 0x02}); err != nil {
		t.Fatalf("handleEventTracking: %v", err)
	}
	if got := len(p.input.recordedBlobs); got != 1 {
		t.Fatalf("recordedBlobs: got %d, want 1 (the emit site sits past Record)", got)
	}
