...
// a yielding producer consults so a slow reader gets served slowly instead
// of being pushed into the hard cap and disconnected. rev-225 has no such
// producer in-world (no OnDemand TCP path), but the signal is kept and
// pinned: it must go true well before the cap and back to false once the
// peer catches up.
