...
//
// NewServer performs full cache loading, so this shares the Server225_2-ref
...
func TestNewServerStampsBootTime(t *testing.T) {
	// rev-225 loads the wordenc filter out of the cache pack itself
	// (encfilter.Load(cfg.CachePath)), so the cache path is the only
	// config this needs.
	cfg := Config{
