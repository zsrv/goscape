...
//
// TS Player.executeScript stores the NpcSuspended continuation ON THE ACTIVE
// NPC:
//
...
// still attacked. Fix backported from rev-274 (59d007f1).
func TestOpNpcScriptNpcDelayStoresContinuationOnNpc(t *testing.T) {
	s := newTestServer(t)
	s.worldVars = worldVarsView{s: s} // production wiring (server.go New)
	s.currentTick = 100
