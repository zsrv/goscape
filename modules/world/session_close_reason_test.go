...
// TestSessionCloseReason_InputTrackingNoReport covers the idle logout this
// revision's windowed InputTracking raises when the client never submits its
// tracking report (TS InputTracking.ts:150). It is the same class of end as
// the IDLE_TIMER opcode — the client stopped answering — so it reports a
// timeout.
func TestSessionCloseReason_InputTrackingNoReport(t *testing.T) {
