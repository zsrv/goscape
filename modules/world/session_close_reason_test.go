...
// TestSessionCloseReason_MissingInputTrackingReport covers the idle logout
// InputTracking.submitEvents raises when the client never sent the report it
// was asked for (NAI-74) — the same "the player stopped answering" verdict the
// IDLE_TIMER opcode and processLogouts reach.
func TestSessionCloseReason_MissingInputTrackingReport(t *testing.T) {
