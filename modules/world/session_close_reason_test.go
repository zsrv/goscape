...
// TestSessionCloseReason_MissedInputTrackingReport covers submitEvents' kick
// arm: outside debug mode a client that never sent an input tracking report
// is logged out idle. It is a decision site this revision has and rev-274
// does not, so it needs the same timeout reason the other idle paths record.
func TestSessionCloseReason_MissedInputTrackingReport(t *testing.T) {
	tt, p, _, _ := inputTrackingTestSetup(t)
	tp := attachTap(p.client)

	tt.hasSeenReport = false
	p.client.server.cfg.NodeDebug = false
	tt.submitEvents()

	if !p.requestIdleLogout {
		t.Fatal("preflight: submitEvents did not flag the idle logout")
	}
