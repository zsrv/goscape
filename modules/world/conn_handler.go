...
func (s *Server) HandleConn(conn net.Conn) {
	...
	// under admissionGateMu. Either this connection registers in tcpWg before
	...
	// to closeLiveConns. Lock order is admissionGateMu → liveConnsMu (trackConn
	// takes only the leaf liveConnsMu; nothing acquires admissionGateMu while
	// holding liveConnsMu).
	s.trackConn(conn)
	s.admissionGateMu.Unlock()
