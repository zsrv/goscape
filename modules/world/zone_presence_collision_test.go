...
// port (Engine-TS@e1dea19f PathingEntity.ts:160-185): an entity's collision
...
//	          changeNpcCollision(width, new..., true)        (TS :166-169)
//	case ALL: same PLUS the changePlayerCollision pair        (TS :170-175)
//
...
func TestNpcStep_CollisionFlagFollows(t *testing.T) {
	...
	if s.gamemap.Pathfinder.Flags.IsFlagged(3200, 3200, 0, collision.FlagBlockNPCs) {
		t.Errorf("old tile (3200,3200) still carries FlagBlockNPCs after step; footprint must follow (TS PathingEntity.ts:167)")
	}
	if !s.gamemap.Pathfinder.Flags.IsFlagged(3201, 3200, 0, collision.FlagBlockNPCs) {
		t.Errorf("new tile (3201,3200) missing FlagBlockNPCs after step (TS PathingEntity.ts:168)")
	}
}
...
func TestNpcTeleport_CollisionFlagFollows_AcrossLevels(t *testing.T) {
	...
	if s.gamemap.Pathfinder.Flags.IsFlagged(3200, 3200, 0, collision.FlagBlockNPCs) {
		t.Errorf("source tile (3200,3200,0) still carries FlagBlockNPCs after teleport; removal must use prevX/prevZ/prevLevel (TS PathingEntity.ts:167,290)")
	}
	if !s.gamemap.Pathfinder.Flags.IsFlagged(3232, 3232, 2, collision.FlagBlockNPCs) {
		t.Errorf("dest tile (3232,3232,2) missing FlagBlockNPCs after teleport (TS PathingEntity.ts:168)")
	}
}
...
func TestPlayerStep_CollisionFlagFollows(t *testing.T) {
	...
	// step must move it (Player constructs with BlockWalk.NPC, Player.ts:411).
	...
	if s.gamemap.Pathfinder.Flags.IsFlagged(3200, 3200, 0, collision.FlagBlockNPCs) {
		t.Errorf("old tile (3200,3200) still carries FlagBlockNPCs after player step (TS PathingEntity.ts:167)")
	}
	if !s.gamemap.Pathfinder.Flags.IsFlagged(3201, 3200, 0, collision.FlagBlockNPCs) {
		t.Errorf("new tile (3201,3200) missing FlagBlockNPCs after player step (TS PathingEntity.ts:168)")
	}
}
...
func TestNpcBlockWalkNone_StepMovesNoFlags(t *testing.T) {
	...
	if s.gamemap.Pathfinder.Flags.IsFlagged(3201, 3200, 0, both) {
		t.Errorf("blockwalk=none: new tile gained entity flags after step (TS switch has no NONE case, PathingEntity.ts:165-176)")
	}
}
...
func TestNpcBlockWalkAll_StepMovesBothFlagFamilies(t *testing.T) {
	...
	if s.gamemap.Pathfinder.Flags.IsFlagged(3200, 3200, 0, both) {
		t.Errorf("blockwalk=all: old tile still carries entity flags after step (TS PathingEntity.ts:171,173)")
	}
	if !s.gamemap.Pathfinder.Flags.IsFlagged(3201, 3200, 0, collision.FlagBlockNPCs) ||
		!s.gamemap.Pathfinder.Flags.IsFlagged(3201, 3200, 0, collision.FlagBlockPlayers) {
		t.Errorf("blockwalk=all: new tile missing one or both flag families after step (TS PathingEntity.ts:172,174)")
	}
}
...
// Player.ts:1886); a subsequent step must not move/plant any flags.
...
// addNpc seeds the flag (TS World.ts:1271-1279), removeNpc clears it at the
// CURRENT position (TS World.ts:1302-1310).
...
// TestPlayerStepThenLogout_CollisionSeedBalance: per TS@e1dea19f there is NO
// login-time player collision seed — the flag materialises on the first
// refreshZonePresence move (PathingEntity.ts:168) and is cleared
// unconditionally at logout (World.ts:1601, goscape removePlayerInternal).
func TestPlayerStepThenLogout_CollisionSeedBalance(t *testing.T) {
	...
	// no changeNpcCollision in its player-add path at e1dea19f).
	if s.gamemap.Pathfinder.Flags.IsFlagged(3200, 3200, 0, collision.FlagBlockNPCs) {
	...
	if !s.gamemap.Pathfinder.Flags.IsFlagged(3201, 3200, 0, collision.FlagBlockNPCs) {
		t.Fatalf("step: tile (3201,3200) missing FlagBlockNPCs (first move materialises the flag, TS PathingEntity.ts:168)")
	}

	s.removePlayerInternal(p)

	if s.gamemap.Pathfinder.Flags.IsFlagged(3201, 3200, 0, collision.FlagBlockNPCs) {
		t.Errorf("logout: tile (3201,3200) still carries FlagBlockNPCs after removePlayerInternal (TS World.ts:1601)")
	}
}
