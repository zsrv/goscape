...
//
// arch-29.8 backfill: this used to rely on all four production modules
// being disabled (each returning an IdleService) to populate the service
// map with harmless no-op services. Disabled modules now contribute NO
// service (see TestDisabledModulesYieldNoService in
// modules_disabled_test.go), so an all-disabled --target=all now correctly
// yields zero services and NewManager fails fast with "no services" — that
// degenerate case is not what this test is about. A scoped ModuleManager
// with one explicit idle module stands in for "a module that starts, runs,
// and stops cleanly on signal," independent of the disabled-module
// masquerade this fix removes.
// COV-1 (Arc 18).
...
//
// arch-29.8 backfill: previously relied on the disabled-module IdleService
// masquerade to populate serviceMap[OnDemand] with a harmless no-op (see
