...
func runSmokePack(args []string, stdout, stderr io.Writer) int {
	...
	refDir := fs.String("reference-dir", "",
		"Reference pack output to byte-diff against (typically Engine-TS/data/pack). Empty → diff disabled. "+
			"Note: server/script.dat records each script's path relative to the scripts root it was found "+
			"under, so the diff no longer depends on where --content-dir lives. Engine-TS records the literal "+
			"path it was handed, so expect script.dat to differ from a TS reference in that header string.")
