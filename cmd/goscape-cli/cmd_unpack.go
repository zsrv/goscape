...
func unpackUsage(w io.Writer) {
	...
	fmt.Fprintln(w, "  -revision   string   Revision tag embedded in config output path (default: 274; config only).")
...
func parseUnpackFlags(name string, args []string, stderr io.Writer, showPackDir, showRevision, showType bool) (*unpackFlags, int) {
	...
	if showRevision {
		// TS hardcodes the revision tag at the unpackConfigs call site:
		// '274' at Unpack.ts:356 @dee467c8 ('254' at Unpack.ts:356
		// @43e02957; '245' at Unpack.ts:368 @3c16994c).
		fs.StringVar(&f.revision, "revision", "274",
