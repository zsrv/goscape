...
func unpackUsage(w io.Writer) {
	...
	fmt.Fprintln(w, "  -revision   string   Revision tag embedded in config output path (default: 254; config only).")
...
func parseUnpackFlags(name string, args []string, stderr io.Writer, showPackDir, showRevision, showType bool) (*unpackFlags, int) {
	...
	if showRevision {
		...
		// '254' at Unpack.ts:356 @43e02957 (was '245' at Unpack.ts:368 @3c16994c).
		fs.StringVar(&f.revision, "revision", "254",
