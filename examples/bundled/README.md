...
modules persist to local SQLite, and the `ondemand` (HTTP) and `world` (TCP)
modules are plain network listeners. **No external services are required to run
it.**
...
| 8082 | hiscore | read-only hiscores JSON API (see `docs/hiscores-api.md`) |
