...
// ref225FontPackDir resolves the rev-225 reference cache pack dir from
// GOSCAPE_REF225_DIR, or "" when unset/unavailable (see the cross-revision
// cache note on the fonttype tests).
func ref225FontPackDir() string {
...
func TestSplitInitFontWrap_BreaksOnMaxWidth(t *testing.T) {
	packDir := ref225FontPackDir()
	if packDir == "" {
		t.Skip("Server225-ref cache not available; set GOSCAPE_REF225_DIR")
	}
