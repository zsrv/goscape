...
// values + ScriptOpcodeMap keys, including the four '*' vararg keys). 404
// entries (enum and map counts are equal at this pin; the dee467c8 table had
// 400 — 8139461a adds P_TEMPRUN, P_TRANSMOGRIFY, DATE_MINUTES and
// DATE_RUNEDAY).
//
// P_TEMPRUN (2088) and P_TRANSMOGRIFY (2092) are inserted into the MIDDLE of
// the const enum, not appended, so 49 existing player opcodes shift:
// P_STOPACTION/P_TELEJUMP/P_TELEPORT by +1 and P_WALK (2091->2093) through
// WEIGHT (2136->2138) by +2. Every compiled script byte moves with them.
