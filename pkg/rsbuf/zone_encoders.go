...
// buffer delivered via UpdateZonePartialEnclosed (opcode 195, -2).
// Values mirror ServerGameZoneProt.ts at Engine-TS rev 274 (dee467c8).
...
// copies, so the 245.2 renumber sailed past it and shipped stale 244 bytes
...
const (
	...
	ZoneOpObjDel       = 52
	ZoneOpObjReveal    = 219
	ZoneOpLocAddChange = 138
	ZoneOpMapProjAnim  = 107
	ZoneOpLocDel       = 173
	ZoneOpObjCount     = 95
	ZoneOpMapAnim      = 85
	ZoneOpObjAdd       = 81
...
// packet (opcode 153, fixed 2). The opcode is emitted by writeOut.
...
// UpdateZonePartialFollows packet (opcode 32, fixed 2).
...
// precomputed shared-data bytes for UpdateZonePartialEnclosed (opcode 195, -2).
