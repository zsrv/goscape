...
// createNew=true, which TRUNCATES dat+idx on every run, then calls Pack. On a
// second pack the intermediate out/client/interface already exists and is fresh,
// so the ShouldBuild gate says "no rebuild" — but TS PackClient.ts:31 still does
// cache.write(0,3,...) UNCONDITIONALLY (the gate wraps only the rebuild of the
// intermediates). Gating the write too (as goscape did) left interface empty on
// every repack into a populated out-dir, which 404'd the OnDemand /interface route.
func TestPack_RepackWritesArchiveIntoTruncatedCache(t *testing.T) {
	...
	// but out/client/interface already exists and is fresh ⇒ no rebuild.
	pack.ClearFsCache()
