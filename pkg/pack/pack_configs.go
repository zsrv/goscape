...
// see the transmitted set documented on validatePackNamesAgainstCfgs):
// BuildVerify the client buffer against crc (log-not-throw), save the
// server pack, then queue the client pack into clientJag.
//
...
// readAndValidate, which lets the per-family seam self-heal and lets
// transmitted packs raise TS's missing-id error. That split arrives with the
// T30 correspondence audit at TS pin 2e3bcf43 and does not exist here, so
// generation lives entirely in this function and readAndValidate keeps its
// orphan-only check.
//
