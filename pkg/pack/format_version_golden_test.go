...
// is exactly the discipline that failed during the rev-274 sync: the config
// opcodes moved, the per-family byte tests correctly failed and were updated,
// and nothing connected that to "the cached outputs are now wrong". This
// closes the loop — CI names the constant instead of relying on memory.
const packFormatGolden = "178aaa4f94dd0e6365405de214962c1285f5bfe27394f4a12fecc2d0da255a0a"
...
// per-family tests and the byte-parity gate against Server274-ref are for;
// this test only has to notice that SOMETHING moved.
func TestPackFormatVersionGolden(t *testing.T) {
	...
	// --- hunt: exercises check_invcat (opcode 18) and the renumbered
	// extracheck_var window (19-21), both moved by the rev-274 sync.
	{
		pf := buildHuntPF("g_hunt")
		cfgs := map[string][]ConfigLine{
			"g_hunt": {
				{Key: "type", Value: objtype.HuntModePlayer},
				{Key: "check_invcat", Value: huntCheckInvCat{inv: 1, category: 2, condition: ">", val: 3}},
			},
		}
		pd, err := packHuntConfigs(cfgs, pf, nil)
		if err != nil {
			t.Fatalf("hunt: %v", err)
		}
		add("hunt", pd.Dat.Data)
	}
	{
	...
	// --- npc: exercises wanderrange/maxrange, which moved 200/201 -> 26/27.
	{
		...
		server, client, err := packNpcConfigs(cfgs, npcPack, nil)
		if err != nil {
			t.Fatalf("npc: %v", err)
		}
	...
	// --- seq: exercises the reachforward / duplicatebehaviour renames.
	{
		cfgs := map[string][]ConfigLine{
			"g_seq": {
				{Key: "reachforward", Value: true},
				{Key: "duplicatebehaviour", Value: 2},
				{Key: "priority", Value: 4},
			},
		}
		server, client := packSeqConfigs(cfgs, seqOneSlotPack("g_seq"), nil)
