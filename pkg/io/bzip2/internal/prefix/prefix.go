...
func (pc PrefixCodes) checkCanonical() bool {
	...
	for _, c := range pc {
		if c.Len > 0 {
			c.Val = dscommon.ReverseUint32N(c.Val, uint(c.Len))
...
func GenerateLengths(codes PrefixCodes, maxBits uint) error {
	...
	for _, c := range codes[1:] {
		if c.Cnt < cntLast {
			return errorf(dserrors.Invalid, "non-monotonically increasing symbol counts")
		}
		cntLast = c.Cnt
	}
	...
	if dscommon.Debug && !codes.checkLengths() {
...
func GeneratePrefixes(codes PrefixCodes) error {
	if len(codes) <= 1 {
		if len(codes) == 1 {
			if codes[0].Len != 0 {
				return errorf(dserrors.Invalid, "degenerate prefix tree with one node")
			}
			codes[0].Val = 0
		}
		return nil
	...
	for _, c := range codes[1:] {
		if c.Sym <= symLast {
			return errorf(dserrors.Invalid, "non-unique or non-monotonically increasing symbols")
		}
		if minBits > c.Len {
			minBits = c.Len
		}
	...
	if minBits == 0 {
		return errorf(dserrors.Invalid, "invalid prefix bit-length")
	}
	...
	if code != 1<<maxBits {
		return errorf(dserrors.Invalid, "degenerate prefix tree")
	}

	// Assign the code to each symbol.
	for i, c := range codes {
		codes[i].Val = dscommon.ReverseUint32N(uint32(nextCodes[c.Len]), uint(c.Len))
		nextCodes[c.Len]++
	}

	if dscommon.Debug && !codes.checkPrefixes() {
		panic("overlapping prefixes detected")
	}
	if dscommon.Debug && !codes.checkCanonical() {
