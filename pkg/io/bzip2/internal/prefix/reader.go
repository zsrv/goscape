...
import (
	...
	"github.com/zsrv/goscape/pkg/io/bzip2/internal/compressapi"
...
// compressapi.ByteReader or compressapi.BufferedReader interface, then it also
...
// compressapi.BufferedReader interface. If the input does not satisfy either
// compressapi.ByteReader or compressapi.BufferedReader, then it will be internally
// wrapped with a bufio.Reader.
type Reader struct {
	Offset int64 // Number of bytes read from the underlying io.Reader

	rd     io.Reader
	byteRd compressapi.ByteReader     // Set if rd is a ByteReader
	bufRd  compressapi.BufferedReader // Set if rd is a BufferedReader
	...
	// These fields are only used if rd is a compressapi.BufferedReader.
	...
	*pr = Reader{
	...
	case *strings.Reader:
		...
		*pr.sr = stringReader{Reader: rr}
		pr.bufRd = pr.sr
	case compressapi.BufferedReader:
		pr.bufRd = rr
	case compressapi.ByteReader:
		pr.byteRd = rr
	default:
		if pr.bu == nil {
...
func (pr *Reader) Read(buf []byte) (cnt int, err error) {
	if pr.numBits > 0 {
		if pr.numBits%8 != 0 {
			return 0, errorf(dserrors.Invalid, "non-aligned bit buffer")
		}
		for cnt = 0; len(buf) > cnt && pr.numBits > 0; cnt++ {
			if pr.bigEndian {
				buf[cnt] = dscommon.ReverseLUT[byte(pr.bufBits)]
			} else {
...
func (pr *Reader) ReadSymbol(pd *Decoder) uint {
	if len(pd.chunks) == 0 {
		panicf(dserrors.Invalid, "decode with empty prefix tree")
	}
...
// If reader is a compressapi.BufferedReader, then this calls Discard to update
// the read offset.
...
// If the underlying reader is a compressapi.BufferedReader, then this will fill
