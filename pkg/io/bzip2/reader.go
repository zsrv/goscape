...
func (zr *Reader) Read(buf []byte) (int, error) {
	for {
		...
		func() {
			...
			if zr.rdHdrFtr%2 == 0 {
				...
				if zr.rd.ReadBitsBE64(16) != hdrMagic {
					panicf(dserrors.Corrupted, "invalid stream magic")
				}
				if ver := zr.rd.ReadBitsBE64(8); ver != 'h' {
					if ver == '0' {
						panicf(dserrors.Deprecated, "bzip1 format is not supported")
					}
					panicf(dserrors.Corrupted, "invalid version: %q", byte(ver))
				}
				lvl := int(zr.rd.ReadBitsBE64(8)) - '0'
				if lvl < BestSpeed || lvl > BestCompression {
					panicf(dserrors.Corrupted, "invalid block size: %d", lvl*blockSize)
				}
				zr.level = lvl
				zr.rdHdrFtr++
			} else {
				...
				if zr.blkCRC != zr.crc.val {
					panicf(dserrors.Corrupted, "mismatching block checksum")
				}
		...
		if zr.err != nil {
			zr.err = errWrap(zr.err, dserrors.Corrupted)
			return 0, zr.err
		}
	}
}
...
func (zr *Reader) decodeBlock() []byte {
	if magic := zr.rd.ReadBitsBE64(48); magic != blkMagic {
		if magic == endMagic {
			...
			if zr.endCRC != endCRC {
				panicf(dserrors.Corrupted, "mismatching stream checksum")
			}
			zr.endCRC = 0
			zr.rd.ReadPads()
			zr.rdHdrFtr++
		...
		panicf(dserrors.Corrupted, "invalid block or footer magic")
	}
	...
	if zr.rd.ReadBitsBE64(1) != 0 {
		panicf(dserrors.Deprecated, "block randomization is not supported")
	}
	...
	if ptr >= len(buf) {
		panicf(dserrors.Corrupted, "origin pointer (0x%06x) exceeds block size: %d", ptr, len(buf))
	}
...
func (zr *Reader) decodePrefix(numSyms int) (syms []uint16) {
	...
	if numSyms < 3 {
		panicf(dserrors.Corrupted, "not enough prefix symbols: %d", numSyms)
	}
	...
	if numTrees < minNumTrees || numTrees > maxNumTrees {
		panicf(dserrors.Corrupted, "invalid number of prefix trees: %d", numTrees)
	}
	...
	for i := range treeSels {
		...
		if int(sym) >= numTrees {
			panicf(dserrors.Corrupted, "invalid prefix tree selector: %d", sym)
		}
	...
	for {
		if blkLen == 0 {
			...
			if selIdx >= len(treeSels) {
				panicf(dserrors.Corrupted, "not enough prefix tree selectors")
			}
		...
		if int(sym) >= numSyms {
			panicf(dserrors.Corrupted, "invalid prefix symbol: %d", sym)
		}
		if len(syms) >= zr.level*blockSize {
			panicf(dserrors.Corrupted, "number of prefix symbols exceeds block size")
		}
