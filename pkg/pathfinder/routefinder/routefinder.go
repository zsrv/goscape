...
func (pf *RouteFinder) FindRoute(level, srcX, srcZ, destX, destZ, srcSize, destWidth, destLength, locAngle, locShape int,
	...
	// DEVIATION SYNC289-D4 (closing an older one): the route-blocker
	// pathfinding branch is gone. It selected routeBlockerFind* variants that
	// navigated by the WALL_*_ROUTE_BLOCKER / LOC_ROUTE_BLOCKER masks, and
	// TS 8139461a deleted those flags outright. Upstream never had this
	// branch at all — PathFinder.ts has no route-blocker path at either
	// 4c95f87e or 1d25566c — and nothing ever set the bits it read, so on the
	// goscape side it was unreachable in production too: the only constructor
	// used outside tests is NewRouteFinderDefault, which passes
