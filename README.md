...
- **`hiscore`** (`modules/hiscore/`) serves the `hiscore`/`hiscore_large` tables that
  ...
  `127.0.0.1:8082`. Endpoints, parameters and rate-limit knobs are documented in
  the Administrator's Guide's Hiscores API page on the docs site (source:
  `main:docs/admin/hiscores-api.md`).
