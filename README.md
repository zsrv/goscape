...
**revision 274** and compatible with the Lost City Java client.

The server is self-contained by default: login, friends, account and hiscore persist to a
central database (SQLite by default, or PostgreSQL via `database.backend`), and the
ondemand (HTTP) and world (TCP) modules are plain network listeners — **no external
services are required to run it with the default SQLite backend**.
...
| `world`    | TCP game server (RS2 protocol, rev 274)       |
| `login`    | gRPC login service (central DB: SQLite/Postgres) |
| `friends`  | friends/private-message server (central DB: SQLite/Postgres) |
...
- **`hiscore`** (`modules/hiscore/`) serves the `hiscore`/`hiscore_large` tables that
  ...
  read time as well as write time. Default listener `127.0.0.1:8082`; see
  `docs/hiscores-api.md` for endpoints, parameters and rate-limit knobs.
...
- `docs/hiscores-api.md` documents the hiscores read API; the account module's design
  notes live in `docs/superpowers/specs/2026-07-19-account-management-design.md`.
