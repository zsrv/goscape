...
**revision 254** and compatible with the Lost City Java client.

The server is self-contained: login and friends persist to a central database (SQLite by
default; PostgreSQL also supported via `database.backend`), and the ondemand (HTTP) and
...
| `world`    | TCP game server (RS2 protocol, rev 254)       |
