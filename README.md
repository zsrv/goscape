...
The server is self-contained by default: login and friends persist to a central database
(SQLite by default, or PostgreSQL via `database.backend`), and the ondemand (HTTP) and
world (TCP) modules are plain network listeners — **no external services are required to
run it with the default SQLite backend**.
